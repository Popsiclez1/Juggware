...
# In frozen builds (PyInstaller/auto-py-to-exe), ensure bundled GLFW DLLs are discoverable.
def _configure_glfw_dll_search_path():
    try:
        ...
        for dll_name in ("glfw3.dll", "glfw.dll", "libglfw.dll"):
            dll_path = os.path.join(base_dir, dll_name)
            if os.path.exists(dll_path):
                # pyGLFW reads this override and loads the exact DLL path.
                os.environ["PYGLFW_LIBRARY"] = dll_path
                break
    except Exception:
        pass
...
GLFW_AVAILABLE = True
GLFW_IMPORT_ERROR = ""
_configure_glfw_dll_search_path()
try:
    import glfw
    import imgui
...
# True only when launcher.py started this process (no temp/file markers required).
LAUNCHED_BY_LAUNCHER = os.environ.get("JUGGWARE_LAUNCHED_BY_LAUNCHER") == "1"

# Delay range in seconds before starting cheat after graphics reset
RESET_GRAPHICS_DELAY_MIN = 5
RESET_GRAPHICS_DELAY_MAX = 25
...
TITLE_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/CheatTitle"
STATUS_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/CheatStatus"
VERSION_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/CheatVersion"
FALLBACK_TITLE = "CS2 Cheat"

# Offsets download URL
OFFSETS_URL = "https://www.dropbox.com/scl/fi/zf0hjar0yo7qggbwnje45/offsets.exe?rlkey=bllys071bvfbdsvpo3dxhg3t0&st=w3ql2z21&dl=1"

# GitHub offsets URLs
GITHUB_OFFSETS_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/output/offsets.json"
GITHUB_CLIENT_DLL_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/output/client_dll.json"
...
SPECIALHIT_SOUND_URL = "https://www.dropbox.com/scl/fi/atfpkh3z2wyqp62gm61df/Nigger.wav?rlkey=4y8aiqbd3z85gaf5dyzd8ppgw&st=txdtk390&dl=1"

# Loader Info tab image
LOADER_INFO_IMAGE_URL = "https://cdn.discordapp.com/attachments/1466591782522323015/1523501136584114366/IMG_7564.jpg?ex=6a4c5680&is=6a4b0500&hm=26f17f5b620de94f7424771a3a5848413bcdb894706daa6e8657616cc5adf226&"
...
LOADER_INFO_IMAGE_PATH = os.path.join(TEMP_FOLDER, "loader_info_image.png")
LOADER_TEXTURE_REGISTRY_TAG = "loader_texture_registry"
LOADER_INFO_TEXTURE_TAG = "loader_info_texture"
LOADER_INFO_IMAGE_MAX_WIDTH = 170
...
# Flag set on first launch when the config folder did not exist.
FIRST_LAUNCH = False
...
SKELETON_BONE_NAMES = tuple(dict.fromkeys(bone_name for bone_pair in BONE_CONNECTIONS for bone_name in bone_pair))
SKELETON_BONE_IDS = {name: BONE_IDS[name] for name in SKELETON_BONE_NAMES if name in BONE_IDS}
SKELETON_BONE_MIN_ID = min(SKELETON_BONE_IDS.values()) if SKELETON_BONE_IDS else 0
SKELETON_BONE_MAX_ID = max(SKELETON_BONE_IDS.values()) if SKELETON_BONE_IDS else 0
SKELETON_BONE_BLOCK_SIZE = ((SKELETON_BONE_MAX_ID - SKELETON_BONE_MIN_ID + 1) * 0x20) if SKELETON_BONE_IDS else 0
...
# Global triggerbot settings that apply across presets.
DEFAULT_TRIGGERBOT_GLOBAL_SETTINGS = {
    "triggerbot_first_shot_delay": 0,
    "triggerbot_first_shot_cooldown": 0.0,
}
...
Default_Config = {
    "esp_enabled": True,
    "box_esp": True,
    "box_type": "2D Full",                   # "2D Full", "2D Corner", "3D Full", or "3D Corner"
    "line_esp": True,
    "skeleton_esp": True,
    "health_bar": True,
    "armor_bar": True,
    "health_position": "Horizontal Below",   # "Vertical Left", "Vertical Right", "Horizontal Above", "Horizontal Below"
    "healthbar_type": "Bars",            # "Bars" or "Text"
    "health_text_size": 14.0,            # Font size for health/shield text labels
    "name_esp": True,
    "weapon_esp": True,                 # Show weapon name label above players
    "distance_esp": False,
    "distance_text_size": 14.0,          # Font size for distance text labels
    "flashed_indicator_enabled": False,  # Show whether each player is currently flashed
    "flashed_indicator_text_size": 14.0, # Font size for flashed indicator labels
    "draw_head_hitbox": True,  # Draw circle showing head hitbox zone
    "head_hitbox_thickness": 2.0,  # Thickness of the head hitbox sphere lines
    ...
    "targeting_type": 0,                # Legacy global targeting fallback
    "esp_targeting_type": 0,            # 0 = enemies only, 1 = all players for ESP/radar
    "aimbot_targeting_type": 0,         # 0 = enemies only, 1 = all players for aimbot
    "triggerbot_targeting_type": 0,     # 0 = enemies only, 1 = all players for triggerbot
    "triggerbot_first_shot_delay": 0,   # Delay before first shot fires when triggerbot starts (ms)
    "triggerbot_first_shot_cooldown": 1.0,  # Time a target can be lost before first-shot delay is re-applied (s)
    "acs_targeting_type": 0,            # 0 = enemies only, 1 = all players for ACP
    # ESP Colors - Boxes
    "enemy_box_color": (255, 0, 0),       # Red for enemy boxes
    "team_box_color": (0, 255, 0),       # Green for team boxes
    # ESP Colors - Snaplines
    "enemy_snapline_color": (255, 0, 0),  # Red for enemy snaplines
    "team_snapline_color": (0, 255, 0),  # Green for team snaplines
    # ESP Colors - Skeleton
    ...
    "head_hitbox_color": (255, 255, 255),  # White for head hitbox circle
    # Menu Colorway
    ...
    "rounded_window_corners": False,         # Enable rounded window corners on Windows 11
    # ESP Thickness
    "box_thickness": 2.0,                   # Box outline thickness (1.0 - 5.0)
    "snapline_thickness": 2.0,              # Snapline thickness (1.0 - 5.0)
    "skeleton_thickness": 2.0,              # Skeleton line thickness (1.0 - 5.0)
    # Radar
    "radar_enabled": True,                 # Enable radar overlay
    "radar_external_mode": False,           # Render radar in a separate external window
    "radar_external_additional_info": False, # Show external radar text labels beside blips
    "radar_external_x": None,               # Persisted X position for standalone external radar window
    "radar_external_y": None,               # Persisted Y position for standalone external radar window
    "radar_size": 350,                      # Radar diameter in pixels
    "radar_scale": 25.0,                     # Scale factor (higher = zoomed out)
    "radar_x": 750,                           # X offset from screen center (negative = left, positive = right)
    "radar_y": -350,                           # Y offset from screen center (negative = up, positive = down)
    ...
    "radar_border_color": (255, 255, 255),  # Radar border color
    "radar_crosshair_color": (255, 255, 255),  # Radar crosshair color
    ...
    # Aimbot
    "aimbot_enabled": True,                # Enable aimbot
    "aimbot_radius": 100,                    # Aimbot FOV radius (pixels)
    "aimbot_smoothness": 0.0,               # Aimbot smoothness (1-100)
    "aimbot_movement_type": "Linear",      # Movement type: "Linear" or "Dynamic"
    "aimbot_use_humanization": False,       # Use a more human-like movement profile for aimbot
    "aimbot_humanization_strength": 50.0,    # Humanization strength percentage (0-100)
    "aimbot_show_radius": True,             # Show aimbot radius circle
    "aimbot_visible_check": False,          # Only aim at enemies visible to local player
    "aimbot_spotted_check": False,          # Legacy alias for aimbot_visible_check
    "aimbot_adjust_x_axis": True,           # Allow aimbot to adjust horizontal aim
    "aimbot_adjust_y_axis": True,           # Allow aimbot to adjust vertical aim
    ...
    "aimbot_deadzone_radius": 15,           # Deadzone radius (pixels)
    "aimbot_show_deadzone": True,          # Show deadzone circle
    "aimbot_snaplines": True,              # Show snaplines to aimbot targets
    # Aimbot Colors
    ...
    "aimbot_radius_transparency": 255,      # Aimbot radius circle transparency (0-255)
    "aimbot_deadzone_transparency": 255,    # Aimbot deadzone circle transparency (0-255)
    "aimbot_radius_thickness": 2.0,         # Aimbot radius circle thickness
    "aimbot_deadzone_thickness": 2.0,       # Aimbot deadzone circle thickness
    "aimbot_snapline_thickness": 2.0,       # Aimbot snapline thickness
    # Spotted ESP
    "hide_spotted_players": False,          # Hide ESP elements for spotted players
    "hide_while_spectated": False,         # Hide ESP while spectators are present
    ...
    "crosshair_use_targeting_color": False, # Use different color when aiming at target
    "crosshair_targeting_color": (255, 0, 0), # Crosshair color when targeting enemy (red)
    "name_text_size": 14.0,                 # Nickname text font size
    "weapon_text_size": 14.0,               # Weapon label text font size
    # FPS Cap
    "fps_cap_enabled": False,               # Enable FPS limiting
    "fps_cap_value": 144,                   # FPS cap value
    # Triggerbot
    "current_triggerbot_preset": "Default",  # Currently selected weapon preset
    ...
    "acs_target_bone": "Head",              # Target bone (Head, Neck, Chest)
    "acs_smoothness": 5,                    # Movement smoothness (1-50)
    "acs_deadzone": 5,                      # Deadzone in pixels
    "acs_draw_deadzone_lines": True,       # Draw deadzone visualization lines
    "acs_always_show_deadzone_lines": True, # Show deadzone lines without holding key
    "acs_line_width": 2,                    # Deadzone line width (1-10)
    "acs_line_transparency": 255,            # Deadzone line transparency (0-255)
    "acs_line_color": (255, 0, 0),           # Deadzone line color (RGB)
    # Misc Features
    "anti_flash_enabled": False,            # Prevent flashbang blindness
    "local_flashed_indicator_enabled": False,  # Show a centered label while the local player is flashed
    "hide_on_tabout": False,                # Hide overlay and menu when tabbing out of CS2
    "show_tooltips": True,               # Show tooltips on hover
    "show_status_labels": False,             # Show feature status labels in overlay
    "show_spectator_list_overlay": False,   # Show standalone spectator list overlay window
    "spectator_protections_disable_anti_flash": False,  # Disable Anti-Flash while spectators are present
    "spectator_protections_disable_aimbot": False,      # Disable Aimbot while spectators are present
    "spectator_protections_disable_rcs": False,         # Disable Recoil Control while spectators are present
    "spectator_list_x": None,               # Persisted X position for standalone spectator list window
    "spectator_list_y": None,               # Persisted Y position for standalone spectator list window
    "rcs_visualizer_enabled": False,       # Show recoil visualizer on overlay
    "rcs_visualizer_color": (0, 255, 0), # Recoil visualizer line and endpoint color
    "rcs_visualizer_transparency": 255,    # Recoil visualizer opacity (0-255)

    "show_overlay_fps": True,               # Show overlay FPS counter in overlay
    # Camera FOV Changer
    "fov_value": 90,                        # Desired FOV value (68-140)
    "fov_apply_while_scoped": False,        # If True, apply FOV changes while scoped
    # Hitsound
    ...
    "rcs_enabled": False,                  # Enable recoil compensation using aim punch angles
    "rcs_horizontal_smoothing": 0.0,       # Horizontal smoothing (0 = instant, 10 = more human-like)
    "rcs_vertical_smoothing": 0.0,         # Vertical smoothing (0 = instant, 10 = more human-like)
    "rcs_horizontal_compensation": True,   # Apply horizontal recoil compensation (mouse X)
    "rcs_vertical_compensation": True,     # Apply vertical recoil compensation (mouse Y)
    "rcs_horizontal_compensation_percentage": 100,  # Horizontal compensation strength (1-100%)
    "rcs_vertical_compensation_percentage": 100,    # Vertical compensation strength (1-100%)
    "rcs_smoothing_technique": "Instant",  # Smoothing technique: "Softened" or "Instant"
}

# Supported target bones for aimbot/ACP (Pelvis removed).
SUPPORTED_TARGET_BONES = ("Head", "Neck", "Chest")


def normalize_target_bone_value(value):
    """Normalize target bone string to supported values."""
    if value in SUPPORTED_TARGET_BONES:
        return value
    if value == "Pelvis":
        return "Chest"
    if isinstance(value, str):
        value = value.strip().lower()
        if value in {"head", "headbone"}:
            return "Head"
        if value in {"neck", "neckbone"}:
            return "Neck"
        if value in {"chest", "body", "torso"}:
            return "Chest"
    return "Head"


def get_target_bone_offset(target_bone_name, fallback=7):
    """Resolve the requested target bone to the correct bone index used by the game memory layout."""
    normalized_name = normalize_target_bone_value(target_bone_name)
    bone_map = {
        "Head": BONE_IDS.get("head", 7),
        "Neck": BONE_IDS.get("neck", 6),
        "Chest": 4,
    }
    return bone_map.get(normalized_name, fallback)


def apply_head_target_bone_offset(target_bone_name, screen_pos):
    """Shift the projected head target point upward by a few pixels for aiming consistency."""
    if target_bone_name == "Head" and screen_pos[0] != -999 and screen_pos[1] != -999:
        return (screen_pos[0], screen_pos[1] - 3.0)
    return screen_pos


def sanitize_target_bones_in_config(config):
    """Ensure aimbot/ACP target bone settings only use supported values."""
    if not isinstance(config, dict):
        return

    config["aimbot_target_bone"] = normalize_target_bone_value(
        config.get("aimbot_target_bone", "Head")
    )
    config["acs_target_bone"] = normalize_target_bone_value(
        config.get("acs_target_bone", "Head")
    )


def normalize_targeting_type_value(value):
    """Normalize targeting mode to 0 (enemies only) or 1 (all players)."""
    if isinstance(value, str):
        return 1 if value.lower() in {"all players", "all", "team", "teammates"} else 0
    try:
...
def get_targeting_mode_value(settings, key):
    """Safely read a feature-specific targeting mode, falling back to the legacy global setting."""
    if not isinstance(settings, dict):
        return 0
    if key in settings and settings.get(key) is not None:
        return normalize_targeting_type_value(settings.get(key))
    return normalize_targeting_type_value(settings.get("targeting_type", 0))


def sanitize_targeting_types_in_config(config):
    """Migrate legacy targeting settings to the new feature-specific keys."""
    if not isinstance(config, dict):
        return

    legacy_value = config.get("targeting_type")
    if legacy_value is not None:
        normalized = normalize_targeting_type_value(legacy_value)
        for key in ("esp_targeting_type", "aimbot_targeting_type", "triggerbot_targeting_type", "acs_targeting_type"):
            if key not in config:
                config[key] = normalized

    for key in ("esp_targeting_type", "aimbot_targeting_type", "triggerbot_targeting_type", "acs_targeting_type"):
        config[key] = normalize_targeting_type_value(config.get(key, 0))


def sanitize_radar_modes_in_config(config):
    """Ensure mutually exclusive radar modes are not enabled at the same time."""
    if not isinstance(config, dict):
        return

    if config.get("radar_external_mode", False) and config.get("radar_overlap_game", False):
        # Default to external mode when both are present from old/invalid configs.
        config["radar_overlap_game"] = False
...
UI_COLORWAYS = {
    ...
    "Emerald": {
        "window_bg": (16, 28, 24, 255),
        "title_bg": (12, 20, 18, 255),
        "title_bg_active": (40, 120, 95, 255),
        "frame_bg": (28, 70, 54, 255),
        "frame_bg_hovered": (45, 100, 78, 255),
        "frame_bg_active": (60, 130, 102, 255),
        "button": (45, 115, 90, 255),
        "button_hovered": (70, 150, 120, 255),
        "button_active": (30, 95, 72, 255),
        "tab": (24, 60, 48, 255),
        "tab_hovered": (52, 105, 82, 255),
        "tab_active": (40, 90, 70, 255),
        "text": (220, 255, 235, 255),
        "check_mark": (110, 255, 175, 255),
        "slider_grab": (85, 210, 135, 255),
        "header": (34, 80, 62, 255),
        "header_hovered": (52, 115, 90, 255),
        "header_active": (72, 145, 116, 255),
    },
    "Sunset": {
        "window_bg": (34, 22, 18, 255),
        "title_bg": (24, 16, 12, 255),
        "title_bg_active": (190, 95, 55, 255),
        "frame_bg": (115, 58, 36, 255),
        "frame_bg_hovered": (150, 78, 48, 255),
        "frame_bg_active": (190, 105, 70, 255),
        "button": (175, 90, 52, 255),
        "button_hovered": (220, 125, 80, 255),
        "button_active": (145, 72, 42, 255),
        "tab": (125, 62, 38, 255),
        "tab_hovered": (165, 88, 54, 255),
        "tab_active": (150, 78, 46, 255),
        "text": (255, 240, 230, 255),
        "check_mark": (255, 180, 120, 255),
        "slider_grab": (230, 150, 90, 255),
        "header": (140, 70, 42, 255),
        "header_hovered": (175, 95, 58, 255),
        "header_active": (205, 120, 76, 255),
    },
    "Ocean": {
        "window_bg": (16, 24, 34, 255),
        "title_bg": (10, 18, 26, 255),
        "title_bg_active": (35, 105, 155, 255),
        "frame_bg": (24, 64, 96, 255),
        "frame_bg_hovered": (38, 88, 126, 255),
        "frame_bg_active": (54, 118, 162, 255),
        "button": (45, 110, 155, 255),
        "button_hovered": (72, 145, 200, 255),
        "button_active": (32, 84, 124, 255),
        "tab": (24, 58, 90, 255),
        "tab_hovered": (46, 95, 140, 255),
        "tab_active": (36, 78, 118, 255),
        "text": (220, 240, 255, 255),
        "check_mark": (95, 205, 255, 255),
        "slider_grab": (80, 175, 240, 255),
        "header": (32, 76, 116, 255),
        "header_hovered": (48, 104, 148, 255),
        "header_active": (68, 132, 176, 255),
    },
    "Neon Lime": {
        "window_bg": (18, 24, 16, 255),
        "title_bg": (12, 18, 10, 255),
        "title_bg_active": (135, 220, 55, 255),
        "frame_bg": (70, 120, 40, 255),
        "frame_bg_hovered": (105, 170, 58, 255),
        "frame_bg_active": (140, 220, 78, 255),
        "button": (120, 200, 55, 255),
        "button_hovered": (155, 235, 90, 255),
        "button_active": (95, 165, 40, 255),
        "tab": (78, 135, 42, 255),
        "tab_hovered": (120, 190, 70, 255),
        "tab_active": (105, 170, 58, 255),
        "text": (240, 255, 220, 255),
        "check_mark": (210, 255, 120, 255),
        "slider_grab": (185, 255, 100, 255),
        "header": (90, 150, 48, 255),
        "header_hovered": (125, 195, 68, 255),
        "header_active": (155, 230, 90, 255),
    },
    "Frost": {
        "window_bg": (22, 28, 34, 255),
        "title_bg": (16, 22, 28, 255),
        "title_bg_active": (95, 145, 180, 255),
        "frame_bg": (62, 90, 120, 255),
        "frame_bg_hovered": (90, 122, 156, 255),
        "frame_bg_active": (122, 154, 188, 255),
        "button": (90, 130, 165, 255),
        "button_hovered": (124, 168, 210, 255),
        "button_active": (70, 110, 145, 255),
        "tab": (58, 92, 122, 255),
        "tab_hovered": (92, 128, 164, 255),
        "tab_active": (78, 112, 148, 255),
        "text": (236, 245, 255, 255),
        "check_mark": (170, 230, 255, 255),
        "slider_grab": (142, 205, 255, 255),
        "header": (72, 108, 142, 255),
        "header_hovered": (104, 142, 178, 255),
        "header_active": (132, 170, 210, 255),
    },
    "Rose Gold": {
        "window_bg": (30, 20, 24, 255),
        "title_bg": (22, 14, 18, 255),
        "title_bg_active": (170, 100, 120, 255),
        "frame_bg": (115, 68, 86, 255),
        "frame_bg_hovered": (145, 90, 112, 255),
        "frame_bg_active": (180, 120, 140, 255),
        "button": (165, 95, 115, 255),
        "button_hovered": (205, 130, 150, 255),
        "button_active": (135, 72, 90, 255),
        "tab": (110, 64, 84, 255),
        "tab_hovered": (155, 92, 118, 255),
        "tab_active": (138, 80, 104, 255),
        "text": (255, 239, 242, 255),
        "check_mark": (255, 180, 205, 255),
        "slider_grab": (230, 150, 175, 255),
        "header": (128, 78, 100, 255),
        "header_hovered": (160, 102, 126, 255),
        "header_active": (190, 126, 150, 255),
    },
    "Royal": {
        "window_bg": (18, 20, 34, 255),
        "title_bg": (12, 14, 24, 255),
        "title_bg_active": (92, 58, 185, 255),
        "frame_bg": (54, 44, 112, 255),
        "frame_bg_hovered": (80, 64, 148, 255),
        "frame_bg_active": (110, 90, 190, 255),
        "button": (92, 72, 170, 255),
        "button_hovered": (128, 108, 205, 255),
        "button_active": (72, 52, 142, 255),
        "tab": (60, 48, 120, 255),
        "tab_hovered": (88, 76, 160, 255),
        "tab_active": (76, 64, 144, 255),
        "text": (240, 240, 255, 255),
        "check_mark": (170, 180, 255, 255),
        "slider_grab": (140, 150, 240, 255),
        "header": (70, 58, 130, 255),
        "header_hovered": (100, 86, 168, 255),
        "header_active": (130, 112, 200, 255),
    },
    "Mint": {
        "window_bg": (20, 30, 26, 255),
        "title_bg": (14, 22, 18, 255),
        "title_bg_active": (58, 165, 135, 255),
        "frame_bg": (42, 112, 94, 255),
        "frame_bg_hovered": (62, 142, 118, 255),
        "frame_bg_active": (82, 172, 142, 255),
        "button": (55, 150, 122, 255),
        "button_hovered": (88, 185, 150, 255),
        "button_active": (40, 124, 102, 255),
        "tab": (42, 100, 88, 255),
        "tab_hovered": (72, 140, 118, 255),
        "tab_active": (58, 128, 108, 255),
        "text": (230, 255, 244, 255),
        "check_mark": (140, 255, 215, 255),
        "slider_grab": (110, 230, 185, 255),
        "header": (50, 118, 100, 255),
        "header_hovered": (74, 148, 126, 255),
        "header_active": (96, 176, 152, 255),
    },
    "Solar": {
        "window_bg": (30, 22, 16, 255),
        "title_bg": (22, 16, 10, 255),
        "title_bg_active": (220, 135, 35, 255),
        "frame_bg": (150, 95, 30, 255),
        "frame_bg_hovered": (190, 122, 46, 255),
        "frame_bg_active": (230, 155, 64, 255),
        "button": (210, 125, 40, 255),
        "button_hovered": (245, 165, 80, 255),
        "button_active": (175, 102, 30, 255),
        "tab": (140, 88, 28, 255),
        "tab_hovered": (185, 120, 48, 255),
        "tab_active": (165, 108, 38, 255),
        "text": (255, 245, 228, 255),
        "check_mark": (255, 210, 120, 255),
        "slider_grab": (245, 175, 90, 255),
        "header": (160, 102, 34, 255),
        "header_hovered": (200, 132, 54, 255),
        "header_active": (230, 160, 74, 255),
    },
    "Aurora": {
        "window_bg": (18, 24, 30, 255),
        "title_bg": (12, 18, 24, 255),
        "title_bg_active": (95, 70, 180, 255),
        "frame_bg": (60, 44, 112, 255),
        "frame_bg_hovered": (88, 70, 148, 255),
        "frame_bg_active": (120, 100, 188, 255),
        "button": (100, 80, 170, 255),
        "button_hovered": (136, 116, 210, 255),
        "button_active": (76, 58, 142, 255),
        "tab": (64, 52, 120, 255),
        "tab_hovered": (96, 84, 158, 255),
        "tab_active": (80, 68, 144, 255),
        "text": (235, 244, 255, 255),
        "check_mark": (160, 210, 255, 255),
        "slider_grab": (125, 180, 240, 255),
        "header": (74, 62, 132, 255),
        "header_hovered": (108, 92, 172, 255),
        "header_active": (140, 118, 206, 255),
    },
    "Midnight Bloom": {
        "window_bg": (22, 18, 32, 255),
        "title_bg": (16, 12, 24, 255),
        "title_bg_active": (122, 90, 200, 255),
        "frame_bg": (60, 42, 96, 255),
        "frame_bg_hovered": (90, 68, 138, 255),
        "frame_bg_active": (124, 100, 176, 255),
        "button": (110, 82, 170, 255),
        "button_hovered": (146, 118, 210, 255),
        "button_active": (86, 62, 140, 255),
        "tab": (64, 48, 100, 255),
        "tab_hovered": (100, 80, 148, 255),
        "tab_active": (84, 66, 128, 255),
        "text": (243, 238, 255, 255),
        "check_mark": (175, 210, 255, 255),
        "slider_grab": (142, 178, 240, 255),
        "header": (82, 66, 128, 255),
        "header_hovered": (116, 98, 168, 255),
        "header_active": (148, 130, 204, 255),
    },
    "Velvet Eclipse": {
        "window_bg": (18, 16, 22, 255),
        "title_bg": (12, 10, 16, 255),
        "title_bg_active": (132, 58, 165, 255),
        "frame_bg": (72, 34, 92, 255),
        "frame_bg_hovered": (104, 54, 126, 255),
        "frame_bg_active": (144, 82, 164, 255),
        "button": (122, 58, 152, 255),
        "button_hovered": (162, 90, 196, 255),
        "button_active": (96, 42, 122, 255),
        "tab": (64, 34, 84, 255),
        "tab_hovered": (104, 58, 126, 255),
        "tab_active": (88, 46, 108, 255),
        "text": (246, 232, 250, 255),
        "check_mark": (255, 170, 240, 255),
        "slider_grab": (220, 120, 210, 255),
        "header": (94, 48, 118, 255),
        "header_hovered": (130, 72, 154, 255),
        "header_active": (164, 96, 188, 255),
    },
    "Coral Drift": {
        "window_bg": (32, 22, 20, 255),
        "title_bg": (24, 16, 14, 255),
        "title_bg_active": (205, 95, 92, 255),
        "frame_bg": (145, 68, 60, 255),
        "frame_bg_hovered": (182, 95, 84, 255),
        "frame_bg_active": (224, 126, 112, 255),
        "button": (198, 98, 88, 255),
        "button_hovered": (232, 132, 118, 255),
        "button_active": (166, 78, 70, 255),
        "tab": (128, 58, 50, 255),
        "tab_hovered": (174, 92, 78, 255),
        "tab_active": (154, 80, 66, 255),
        "text": (255, 241, 236, 255),
        "check_mark": (255, 195, 175, 255),
        "slider_grab": (240, 160, 140, 255),
        "header": (158, 78, 68, 255),
        "header_hovered": (194, 112, 96, 255),
        "header_active": (224, 140, 120, 255),
    },
    "Icy Ember": {
        "window_bg": (22, 28, 34, 255),
        "title_bg": (16, 20, 26, 255),
        "title_bg_active": (92, 140, 190, 255),
        "frame_bg": (54, 92, 128, 255),
        "frame_bg_hovered": (84, 126, 164, 255),
        "frame_bg_active": (118, 160, 200, 255),
        "button": (96, 138, 178, 255),
        "button_hovered": (132, 176, 216, 255),
        "button_active": (72, 110, 148, 255),
        "tab": (56, 90, 124, 255),
        "tab_hovered": (90, 128, 164, 255),
        "tab_active": (76, 112, 148, 255),
        "text": (238, 248, 255, 255),
        "check_mark": (170, 236, 255, 255),
        "slider_grab": (138, 208, 255, 255),
        "header": (74, 112, 148, 255),
        "header_hovered": (108, 148, 186, 255),
        "header_active": (140, 180, 220, 255),
    },
    "Violet Pulse": {
        "window_bg": (24, 20, 32, 255),
        "title_bg": (16, 14, 24, 255),
        "title_bg_active": (155, 92, 205, 255),
        "frame_bg": (102, 58, 132, 255),
        "frame_bg_hovered": (136, 86, 168, 255),
        "frame_bg_active": (176, 122, 210, 255),
        "button": (150, 92, 188, 255),
        "button_hovered": (188, 126, 224, 255),
        "button_active": (126, 70, 160, 255),
        "tab": (88, 56, 116, 255),
        "tab_hovered": (128, 86, 162, 255),
        "tab_active": (112, 72, 144, 255),
        "text": (248, 236, 255, 255),
        "check_mark": (220, 166, 255, 255),
        "slider_grab": (194, 128, 240, 255),
        "header": (116, 76, 150, 255),
        "header_hovered": (150, 108, 186, 255),
        "header_active": (184, 138, 220, 255),
    },
    "Lime Mirage": {
        "window_bg": (20, 28, 20, 255),
        "title_bg": (14, 20, 14, 255),
        "title_bg_active": (138, 206, 72, 255),
        "frame_bg": (78, 128, 48, 255),
        "frame_bg_hovered": (108, 164, 76, 255),
        "frame_bg_active": (144, 206, 104, 255),
        "button": (126, 190, 70, 255),
        "button_hovered": (168, 232, 108, 255),
        "button_active": (100, 154, 52, 255),
        "tab": (72, 124, 46, 255),
        "tab_hovered": (112, 172, 78, 255),
        "tab_active": (96, 156, 66, 255),
        "text": (240, 255, 225, 255),
        "check_mark": (208, 255, 136, 255),
        "slider_grab": (176, 255, 108, 255),
        "header": (92, 144, 60, 255),
        "header_hovered": (124, 182, 88, 255),
        "header_active": (156, 220, 116, 255),
    },
    "Sapphire Dust": {
        "window_bg": (16, 24, 32, 255),
        "title_bg": (10, 16, 24, 255),
        "title_bg_active": (56, 120, 190, 255),
        "frame_bg": (36, 76, 122, 255),
        "frame_bg_hovered": (62, 108, 156, 255),
        "frame_bg_active": (92, 144, 194, 255),
        "button": (70, 122, 176, 255),
        "button_hovered": (102, 156, 214, 255),
        "button_active": (48, 92, 142, 255),
        "tab": (34, 70, 104, 255),
        "tab_hovered": (64, 108, 152, 255),
        "tab_active": (50, 92, 132, 255),
        "text": (228, 244, 255, 255),
        "check_mark": (120, 210, 255, 255),
        "slider_grab": (92, 182, 240, 255),
        "header": (46, 86, 132, 255),
        "header_hovered": (74, 122, 170, 255),
        "header_active": (102, 152, 202, 255),
    },
    "Amber Glow": {
        "window_bg": (30, 24, 18, 255),
        "title_bg": (22, 16, 12, 255),
        "title_bg_active": (228, 152, 54, 255),
        "frame_bg": (172, 112, 42, 255),
        "frame_bg_hovered": (208, 144, 66, 255),
        "frame_bg_active": (242, 176, 90, 255),
        "button": (212, 148, 58, 255),
        "button_hovered": (246, 182, 92, 255),
        "button_active": (180, 118, 42, 255),
        "tab": (150, 100, 36, 255),
        "tab_hovered": (196, 136, 66, 255),
        "tab_active": (176, 122, 50, 255),
        "text": (255, 246, 228, 255),
        "check_mark": (255, 220, 140, 255),
        "slider_grab": (242, 192, 106, 255),
        "header": (166, 118, 48, 255),
        "header_hovered": (202, 150, 76, 255),
        "header_active": (232, 178, 104, 255),
    },
    "Crimson Mist": {
        "window_bg": (28, 16, 18, 255),
        "title_bg": (20, 10, 12, 255),
        "title_bg_active": (180, 48, 70, 255),
        "frame_bg": (120, 30, 46, 255),
        "frame_bg_hovered": (152, 46, 66, 255),
        "frame_bg_active": (188, 70, 94, 255),
        "button": (168, 54, 76, 255),
        "button_hovered": (208, 84, 112, 255),
        "button_active": (134, 38, 58, 255),
        "tab": (110, 30, 44, 255),
        "tab_hovered": (148, 46, 70, 255),
        "tab_active": (128, 38, 58, 255),
        "text": (250, 232, 236, 255),
        "check_mark": (255, 140, 175, 255),
        "slider_grab": (220, 92, 128, 255),
        "header": (136, 42, 60, 255),
        "header_hovered": (168, 62, 86, 255),
        "header_active": (196, 88, 112, 255),
    },
    "Verdant Noir": {
        "window_bg": (16, 22, 18, 255),
        "title_bg": (10, 16, 12, 255),
        "title_bg_active": (60, 128, 96, 255),
        "frame_bg": (40, 84, 62, 255),
        "frame_bg_hovered": (56, 116, 88, 255),
        "frame_bg_active": (84, 152, 118, 255),
        "button": (62, 120, 92, 255),
        "button_hovered": (94, 156, 124, 255),
        "button_active": (44, 100, 74, 255),
        "tab": (34, 76, 56, 255),
        "tab_hovered": (70, 120, 92, 255),
        "tab_active": (54, 104, 76, 255),
        "text": (232, 250, 238, 255),
        "check_mark": (140, 255, 180, 255),
        "slider_grab": (110, 220, 150, 255),
        "header": (48, 92, 72, 255),
        "header_hovered": (76, 128, 104, 255),
        "header_active": (108, 162, 136, 255),
    },
    "Cobalt Flux": {
        "window_bg": (18, 20, 30, 255),
        "title_bg": (12, 14, 22, 255),
        "title_bg_active": (58, 86, 220, 255),
        "frame_bg": (34, 54, 142, 255),
        "frame_bg_hovered": (56, 80, 178, 255),
        "frame_bg_active": (86, 112, 214, 255),
        "button": (70, 90, 190, 255),
        "button_hovered": (104, 126, 228, 255),
        "button_active": (44, 62, 154, 255),
        "tab": (28, 46, 110, 255),
        "tab_hovered": (56, 84, 158, 255),
        "tab_active": (44, 68, 136, 255),
        "text": (236, 242, 255, 255),
        "check_mark": (120, 168, 255, 255),
        "slider_grab": (92, 142, 240, 255),
        "header": (48, 72, 148, 255),
        "header_hovered": (76, 104, 184, 255),
        "header_active": (104, 132, 218, 255),
    },
    "Peach Surge": {
        "window_bg": (36, 24, 22, 255),
        "title_bg": (28, 18, 16, 255),
        "title_bg_active": (232, 138, 112, 255),
        "frame_bg": (176, 96, 76, 255),
        "frame_bg_hovered": (208, 126, 102, 255),
        "frame_bg_active": (242, 160, 132, 255),
        "button": (218, 128, 106, 255),
        "button_hovered": (248, 162, 138, 255),
        "button_active": (186, 104, 84, 255),
        "tab": (154, 86, 66, 255),
        "tab_hovered": (198, 124, 100, 255),
        "tab_active": (178, 110, 86, 255),
        "text": (255, 244, 236, 255),
        "check_mark": (255, 208, 178, 255),
        "slider_grab": (246, 186, 154, 255),
        "header": (172, 110, 90, 255),
        "header_hovered": (208, 142, 116, 255),
        "header_active": (238, 170, 144, 255),
    },
    "Mauve Halo": {
        "window_bg": (28, 22, 28, 255),
        "title_bg": (20, 16, 20, 255),
        "title_bg_active": (164, 114, 178, 255),
        "frame_bg": (112, 78, 122, 255),
        "frame_bg_hovered": (144, 104, 154, 255),
        "frame_bg_active": (180, 134, 188, 255),
        "button": (158, 108, 164, 255),
        "button_hovered": (196, 142, 200, 255),
        "button_active": (132, 86, 138, 255),
        "tab": (96, 70, 108, 255),
        "tab_hovered": (138, 100, 146, 255),
        "tab_active": (120, 88, 130, 255),
        "text": (250, 236, 248, 255),
        "check_mark": (233, 186, 255, 255),
        "slider_grab": (206, 156, 224, 255),
        "header": (126, 90, 140, 255),
        "header_hovered": (158, 122, 172, 255),
        "header_active": (188, 146, 200, 255),
    },
    "Aqua Pulse": {
        "window_bg": (20, 26, 28, 255),
        "title_bg": (14, 18, 20, 255),
        "title_bg_active": (66, 182, 176, 255),
        "frame_bg": (42, 126, 128, 255),
        "frame_bg_hovered": (64, 160, 158, 255),
        "frame_bg_active": (92, 196, 188, 255),
        "button": (74, 176, 170, 255),
        "button_hovered": (108, 210, 204, 255),
        "button_active": (54, 142, 144, 255),
        "tab": (40, 108, 116, 255),
        "tab_hovered": (72, 146, 152, 255),
        "tab_active": (56, 128, 136, 255),
        "text": (232, 252, 250, 255),
        "check_mark": (146, 255, 245, 255),
        "slider_grab": (120, 232, 224, 255),
        "header": (54, 124, 132, 255),
        "header_hovered": (84, 158, 164, 255),
        "header_active": (110, 190, 194, 255),
    },
    "Obsidian Gold": {
        "window_bg": (24, 20, 16, 255),
        "title_bg": (16, 14, 10, 255),
        "title_bg_active": (168, 122, 42, 255),
        "frame_bg": (114, 82, 28, 255),
        "frame_bg_hovered": (146, 108, 44, 255),
        "frame_bg_active": (182, 138, 64, 255),
        "button": (160, 116, 42, 255),
        "button_hovered": (198, 148, 66, 255),
        "button_active": (132, 92, 28, 255),
        "tab": (100, 74, 22, 255),
        "tab_hovered": (140, 104, 38, 255),
        "tab_active": (124, 90, 32, 255),
        "text": (247, 240, 220, 255),
        "check_mark": (255, 212, 112, 255),
        "slider_grab": (226, 184, 92, 255),
        "header": (128, 96, 30, 255),
        "header_hovered": (160, 126, 48, 255),
        "header_active": (194, 154, 72, 255),
    },
    "Plasma Wave": {
        "window_bg": (18, 18, 28, 255),
        "title_bg": (12, 12, 20, 255),
        "title_bg_active": (94, 56, 220, 255),
        "frame_bg": (58, 34, 136, 255),
        "frame_bg_hovered": (90, 58, 176, 255),
        "frame_bg_active": (126, 84, 214, 255),
        "button": (102, 60, 190, 255),
        "button_hovered": (140, 90, 228, 255),
        "button_active": (78, 40, 158, 255),
        "tab": (54, 34, 108, 255),
        "tab_hovered": (90, 60, 158, 255),
        "tab_active": (74, 44, 134, 255),
        "text": (242, 240, 255, 255),
        "check_mark": (170, 220, 255, 255),
        "slider_grab": (140, 180, 255, 255),
        "header": (70, 46, 140, 255),
        "header_hovered": (106, 72, 182, 255),
        "header_active": (142, 102, 218, 255),
    },
    "Rose Quartz": {
        "window_bg": (30, 20, 24, 255),
        "title_bg": (22, 14, 18, 255),
        "title_bg_active": (216, 118, 152, 255),
        "frame_bg": (156, 78, 108, 255),
        "frame_bg_hovered": (190, 112, 140, 255),
        "frame_bg_active": (226, 144, 174, 255),
        "button": (202, 118, 146, 255),
        "button_hovered": (238, 152, 180, 255),
        "button_active": (172, 94, 122, 255),
        "tab": (136, 72, 96, 255),
        "tab_hovered": (176, 104, 136, 255),
        "tab_active": (156, 90, 118, 255),
        "text": (255, 242, 246, 255),
        "check_mark": (255, 186, 216, 255),
        "slider_grab": (240, 156, 196, 255),
        "header": (164, 92, 118, 255),
        "header_hovered": (198, 124, 150, 255),
        "header_active": (228, 154, 180, 255),
    },
    "Citrus Nova": {
        "window_bg": (30, 28, 16, 255),
        "title_bg": (22, 20, 10, 255),
        "title_bg_active": (214, 184, 56, 255),
        "frame_bg": (158, 136, 44, 255),
        "frame_bg_hovered": (196, 172, 70, 255),
        "frame_bg_active": (232, 208, 96, 255),
        "button": (206, 174, 56, 255),
        "button_hovered": (240, 208, 90, 255),
        "button_active": (176, 144, 42, 255),
        "tab": (136, 118, 34, 255),
        "tab_hovered": (182, 162, 68, 255),
        "tab_active": (160, 144, 52, 255),
        "text": (255, 252, 232, 255),
        "check_mark": (255, 240, 142, 255),
        "slider_grab": (242, 224, 112, 255),
        "header": (168, 148, 48, 255),
        "header_hovered": (202, 182, 76, 255),
        "header_active": (232, 214, 102, 255),
    },
    "Slate Mirage": {
        "window_bg": (28, 30, 34, 255),
        "title_bg": (20, 22, 26, 255),
        "title_bg_active": (110, 126, 142, 255),
        "frame_bg": (76, 88, 104, 255),
        "frame_bg_hovered": (102, 112, 132, 255),
        "frame_bg_active": (128, 140, 160, 255),
        "button": (106, 118, 136, 255),
        "button_hovered": (138, 150, 170, 255),
        "button_active": (84, 96, 116, 255),
        "tab": (70, 80, 96, 255),
        "tab_hovered": (100, 112, 132, 255),
        "tab_active": (88, 100, 120, 255),
        "text": (240, 244, 248, 255),
        "check_mark": (182, 192, 204, 255),
        "slider_grab": (154, 164, 180, 255),
        "header": (88, 100, 116, 255),
        "header_hovered": (116, 128, 148, 255),
        "header_active": (142, 154, 174, 255),
    },
    "Eclipse Teal": {
        "window_bg": (18, 24, 24, 255),
        "title_bg": (12, 16, 16, 255),
        "title_bg_active": (46, 152, 148, 255),
        "frame_bg": (28, 96, 100, 255),
        "frame_bg_hovered": (46, 126, 128, 255),
        "frame_bg_active": (74, 160, 158, 255),
        "button": (56, 138, 142, 255),
        "button_hovered": (88, 174, 176, 255),
        "button_active": (38, 110, 112, 255),
        "tab": (30, 82, 86, 255),
        "tab_hovered": (60, 120, 124, 255),
        "tab_active": (46, 104, 108, 255),
        "text": (232, 250, 248, 255),
        "check_mark": (128, 255, 248, 255),
        "slider_grab": (104, 228, 224, 255),
        "header": (44, 104, 108, 255),
        "header_hovered": (70, 136, 138, 255),
        "header_active": (96, 164, 166, 255),
    },
}
...
Keybinds_Config = {
    ...
    "esp_toggle_key": "none",  # Key to toggle ESP on/off
    ...
    "acs_key": "none",  # Key to activate auto crosshair placement (hold)
    "aimbot_toggle_key": "none",  # Key to toggle aimbot on/off
    "anti_afk_toggle_key": "none",  # Key to toggle anti-AFK on/off
    "bhop_key": "space",  # Key to activate bunnyhop (hold)
}

# Default keybinds configuration for reset functionality
Default_Keybinds_Config = copy.deepcopy(Keybinds_Config)


def sync_keybinds_to_active_config():
    """Mirror the current keybind state into the active settings payload."""
    global Active_Config

    if not isinstance(Active_Config, dict):
        Active_Config = {}

    Active_Config["keybinds"] = copy.deepcopy(Keybinds_Config)
    return Active_Config


def sync_keybinds_from_active_config(config_data=None):
    """Restore the runtime keybind state from the active settings payload."""
    global Keybinds_Config

    base_keybinds = copy.deepcopy(Default_Keybinds_Config)
    source = config_data if isinstance(config_data, dict) else Active_Config

    if isinstance(source, dict):
        embedded_keybinds = source.get("keybinds")
        if isinstance(embedded_keybinds, dict):
            base_keybinds.update(embedded_keybinds)
            Keybinds_Config = base_keybinds
            return True

    Keybinds_Config = base_keybinds
    return False

# Shared config-key collections keep save/load behavior consistent everywhere.
COLOR_TUPLE_KEYS_AUTOSAVE = (
    ...
    "aimbot_snapline_color",
    "aimbot_radius_color", "aimbot_deadzone_color",
    "aimbot_targeting_radius_color", "aimbot_targeting_deadzone_color",
)

COLOR_TUPLE_KEYS_CONFIG_FILES = COLOR_TUPLE_KEYS_AUTOSAVE + (
    "radar_bg_color", "radar_border_color", "radar_crosshair_color",
    "radar_player_color", "radar_enemy_color", "radar_team_color",
    "acs_line_color",
)

# Backward compatibility keys from the old flat triggerbot config format.
LEGACY_TRIGGERBOT_KEYS = (
    "triggerbot_enabled",
    "triggerbot_first_shot_delay",
    "triggerbot_between_shots_delay",
    "triggerbot_burst_mode",
    "triggerbot_burst_shots",
    "triggerbot_head_only",
)
...
drag_state = {
    ...
    "mouse_was_down": False,  # Whether left mouse button was already down on the previous frame
...
app_state = {
    ...
    "switch_to_loader": False,   # Flag to trigger unload / return to loader
    "requested_window": "loader",  # Next window target after a transition
    "resume_loader_wait_for_cs2": False,  # Auto-start loader launch flow to wait for CS2 restart
    "last_window_pos": None,     # (x, y) position of last window for persistence
    "active_debug_subtab": "tab_debug_output",
    "ui_initialization_complete": False,
    "cheat_window_visible": True  # Whether the cheat/config UI is currently visible
}
...
dwWeaponC4 = None
dwSensitivity = None
dwSensitivity_sensitivity = None  # Offset within sensitivity struct for the float value
dwForceJump = None
...
m_hObserverPawn = None
m_sSanitizedPlayerName = None
...
m_pObserverServices = None
m_hObserverTarget = None
m_vOldOrigin = None
...
m_flFlashDuration = None
m_flFlashBangTime = None

# Per-player flash countdown state. m_flFlashDuration is the original flash
# length and stays constant while the effect is active, so it cannot itself be
# used as a live remaining-time value.
flash_indicator_state = {}
...
esp_overlay = {
    ...
    "current_triggerbot_weapon": "Default",  # Current weapon for triggerbot status
}

radar_external_state = {
    "running": False,         # Whether external radar window thread is running
    "thread": None,           # Reference to external radar thread
}

spectator_list_state = {
    "running": False,         # Whether spectator list window thread is running
    "thread": None,           # Reference to spectator list thread
}

spectator_protection_state = {
    "anti_flash_manual_state": None,
    "aimbot_manual_state": None,
    "rcs_manual_state": None,
    "anti_flash_force_disabled": False,
    "aimbot_force_disabled": False,
    "rcs_force_disabled": False,
}

# Shared memory connection state
memory_state = {
    "running": False,         # Whether shared memory thread is running
    "thread": None,           # Reference to shared memory thread
    "pm": None,               # Shared pymem instance
    "client": None,           # Shared client.dll base
}

SHARED_MEMORY_RETRY_INTERVAL_SECONDS = 5.0


def get_shared_memory_connection():
    """Return the current shared (pm, client) tuple if available."""
    pm = memory_state.get("pm")
    client = memory_state.get("client")
    if pm and client:
        return pm, client

    # Fallback for compatibility while migrating older call-sites.
    pm = esp_overlay.get("pm")
    client = esp_overlay.get("client")
    return pm, client


def memory_connection_thread():
    """Maintain a persistent CS2 memory connection for all feature threads."""
    global memory_state

    debug_log("[MEMORY] Thread starting...", "INFO")
    while memory_state["running"]:
        try:
            pm = memory_state.get("pm")
            client = memory_state.get("client")

            # Keep existing connection alive while process is healthy.
            if pm and client and is_cs2_running_fast(pm):
                esp_overlay["pm"] = pm
            ...
            memory_state["pm"] = pm
            memory_state["client"] = client
            esp_overlay["pm"] = pm
            esp_overlay["client"] = client
            debug_log("[MEMORY] Connected to CS2", "SUCCESS")
            time.sleep(0.25)
        except Exception:
            if memory_state.get("pm") or memory_state.get("client"):
                debug_log("[MEMORY] Lost CS2 connection, retrying...", "WARNING")
            memory_state["pm"] = None
            memory_state["client"] = None
            esp_overlay["pm"] = None
    ...
    memory_state["pm"] = None
    memory_state["client"] = None
    if not esp_overlay.get("running"):
        esp_overlay["pm"] = None
        esp_overlay["client"] = None
    debug_log("[MEMORY] Thread stopped", "INFO")


def start_memory_thread():
    """Start the shared memory connection thread."""
    global memory_state

    if memory_state["running"]:
        return

    memory_state["running"] = True
    memory_state["thread"] = threading.Thread(target=memory_connection_thread, daemon=True)
    memory_state["thread"].start()
    debug_log("Shared memory thread started", "SUCCESS")


def stop_memory_thread():
    """Stop the shared memory connection thread."""
    global memory_state
    ...
    if memory_state["thread"]:
        memory_state["thread"].join(timeout=2.0)
        memory_state["thread"] = None
...
def wait_for_shared_memory_connection(max_wait_seconds=10):
    """Wait for shared memory connection and return (pm, client) or (None, None)."""
    if not memory_state.get("running"):
        start_memory_thread()

    max_wait = max(1, int(max_wait_seconds * 10))
    wait_count = 0
    while wait_count < max_wait:
        pm, client = get_shared_memory_connection()
        if pm and client:
            return pm, client
        if not memory_state.get("running"):
            start_memory_thread()
...
def wait_for_feature_memory_connection(feature_name, max_wait_seconds=10):
    """Keep retrying until shared memory becomes available for a feature thread."""
    while True:
        pm, client = wait_for_shared_memory_connection(max_wait_seconds=max_wait_seconds)
        if pm and client:
            return pm, client
        ...
        debug_log(
            f"[{feature_name}] Failed to get pm/client from shared memory, retrying in {SHARED_MEMORY_RETRY_INTERVAL_SECONDS} seconds",
            "WARNING",
        )
        time.sleep(SHARED_MEMORY_RETRY_INTERVAL_SECONDS)


def build_default_aimbot_humanization_state():
    """Create a fresh humanization state container for aimbot motion."""
    return {
        "velocity_x": 0.0,
        "velocity_y": 0.0,
        "carry_x": 0.0,
        ...
        "curve_p2": (0.0, 0.0),
        "curve_p3": (0.0, 0.0),
        "curve_last_point": (0.0, 0.0),
        "curve_target_dx": 0.0,
        "curve_target_dy": 0.0,
        "curve_bias_scale": 1.0,
        "last_output_x": 0.0,
        "last_output_y": 0.0,
        "last_distance": 0.0,
        "last_dx": 0.0,
...
aimbot_state = {
    ...
    "smoothed_move_x": 0.0,      # Exponentially smoothed mouse movement for aimbot
    "smoothed_move_y": 0.0,      # Exponentially smoothed mouse movement for aimbot
    "residual_x": 0.0,           # Fractional remainder after rounding aimbot motion
    "residual_y": 0.0,           # Fractional remainder after rounding aimbot motion
    "is_targeting": False,      # Whether currently targeting a player
    "last_scan_time": 0.0,       # Last time the aimbot target scan ran
    "last_target_pos": None,    # Last screen-space target position used for snapline persistence
    "last_target_time": 0.0,     # Timestamp of the last resolved target position
    "humanization": build_default_aimbot_humanization_state(),  # Stateful human-like movement profile for aimbot
}
...
rcs_state = {
    ...
    "settings": None,           # Current RCS settings snapshot
}
...
hitsound_state = {
    ...
    "previous_total_hits": None,   # Last observed total hits count (None until first sample)
}
...
def save_settings():
    """
    Save Active_Config (including keybinds) to autosave.json in the Settings folder.
    Called when settings change or on application exit.
    """
    global Active_Config

    try:
        sync_keybinds_to_active_config()
...
def save_keybinds():
    """Persist the current keybind state as part of the main settings payload."""
    save_settings()
    debug_log("Keybinds synced into settings autosave", "SUCCESS")


def load_keybinds():
    """
    Load keybinds from the active settings payload or legacy keybind autosave file.
    If neither exists, keep default keybinds.
    Called at application startup.
    """
    global Keybinds_Config

    if sync_keybinds_from_active_config(Active_Config):
        debug_log("Keybinds loaded from active settings payload", "SUCCESS")
        return True
    ...
    if os.path.exists(KEYBINDS_AUTOSAVE_PATH):
        try:
            ...
            Keybinds_Config = copy.deepcopy(Default_Keybinds_Config)
            Keybinds_Config.update(loaded_keybinds)
            debug_log("Legacy keybinds loaded from autosave.json", "SUCCESS")
            return True
        except Exception as e:
            debug_log(f"Failed to load legacy keybinds: {str(e)}", "ERROR")
            return False
...
def load_settings(file_path=None):
    ...
    if os.path.exists(file_path):
        try:
            ...
            for key in COLOR_TUPLE_KEYS_AUTOSAVE:
            ...
            sanitize_target_bones_in_config(Active_Config)
            sanitize_radar_modes_in_config(Active_Config)
            sanitize_radar_modes_in_config(Active_Config)

            # Backward compatibility: migrate old aimbot_spotted_check key.
            if "aimbot_visible_check" not in loaded_config and "aimbot_spotted_check" in loaded_config:
                Active_Config["aimbot_visible_check"] = loaded_config.get("aimbot_spotted_check", False)

            # Backward compatibility: migrate old single-axis rcs_smoothing key.
            if "rcs_smoothing" in loaded_config:
                legacy_rcs_smoothing = max(0.0, min(float(loaded_config.get("rcs_smoothing", 0.0)), 10.0))
                if "rcs_horizontal_smoothing" not in loaded_config:
                    Active_Config["rcs_horizontal_smoothing"] = legacy_rcs_smoothing
                if "rcs_vertical_smoothing" not in loaded_config:
                    Active_Config["rcs_vertical_smoothing"] = legacy_rcs_smoothing

            sync_keybinds_from_active_config(Active_Config)
...
def get_available_configs():
    ...
    Returns list of filenames (without extension) excluding autosave and radar backup.
    """
    configs = []
    if os.path.exists(SETTINGS_FOLDER):
        for filename in os.listdir(SETTINGS_FOLDER):
            if filename.endswith('.json') and filename not in ('autosave.json', 'radar_backup.json'):
...
def get_available_configs_for_preload():
    """
    Get list of all JSON config files in the Settings folder, excluding autosave and radar backup.
...
def load_config_from_file(config_name):
    """
    Load settings from the settings file and restore the keybinds from the same payload.
    ...
    legacy_keybinds_path = os.path.join(KEYBINDS_FOLDER, f"{config_name}.json")
    ...
    if os.path.exists(settings_path):
        try:
            ...
            for key in COLOR_TUPLE_KEYS_CONFIG_FILES:
                if key in loaded_config and isinstance(loaded_config[key], list):
                    loaded_config[key] = tuple(loaded_config[key])
            ...
            # Backward compatibility: if old flat triggerbot settings exist, copy to "Default" preset
            if "triggerbot_enabled" in loaded_config:
                if "Default" not in Active_Config["triggerbot_presets"]:
                    Active_Config["triggerbot_presets"]["Default"] = {}
                Active_Config["triggerbot_presets"]["Default"].update({
                ...
                # Move first_shot_delay to global setting
                if "triggerbot_first_shot_delay" not in Active_Config:
                    Active_Config["triggerbot_first_shot_delay"] = loaded_config.get("triggerbot_first_shot_delay", 0)
                # Remove old keys
                for key in LEGACY_TRIGGERBOT_KEYS:
            ...
            if not isinstance(Active_Config.get("keybinds"), dict) and os.path.exists(legacy_keybinds_path):
                try:
                    with open(legacy_keybinds_path, 'r') as f:
                        legacy_keybinds = json.load(f)
                    if isinstance(legacy_keybinds, dict):
                        Active_Config["keybinds"] = copy.deepcopy(legacy_keybinds)
                except Exception as e:
                    debug_log(f"Failed to load legacy bundled keybinds {config_name}: {str(e)}", "ERROR")
...
def get_current_triggerbot_settings():
    ...
    preset = Active_Config.get("current_triggerbot_preset", "Default")
...
def get_triggerbot_settings_for_weapon(weapon_name):
    """Get the triggerbot settings for a specific weapon, falling back to 'Default'."""
    if weapon_name in Active_Config["triggerbot_presets"]:
        return Active_Config["triggerbot_presets"][weapon_name]
    else:
        return Active_Config["triggerbot_presets"].get("Default", DEFAULT_TRIGGERBOT_SETTINGS.copy())


def is_any_triggerbot_preset_enabled():
    """Return True if any triggerbot preset has triggerbot_enabled set to True."""
    presets = Active_Config.get("triggerbot_presets", {})
    if not isinstance(presets, dict):
        return False

    for preset_settings in presets.values():
        if isinstance(preset_settings, dict) and preset_settings.get("triggerbot_enabled", False):
            return True
...
def update_triggerbot_ui_from_preset():
    ...
    try:
        ...
        dpg.set_value("slider_triggerbot_first_shot_cooldown", Active_Config.get("triggerbot_first_shot_cooldown", 0.0))
        ...
        # Keep burst shot count visible at all times
        show_tips = Active_Config.get("show_tooltips", True)
        dpg.configure_item("slider_triggerbot_burst_shots", show=True)
        dpg.configure_item("tooltip_triggerbot_burst_shots", show=show_tips)
    except:
        pass
...
def apply_config_to_ui():
    ...
    try:
        ...
        checkbox_mappings = {
            ...
            "chk_flashed_indicator": "flashed_indicator_enabled",
            "chk_health_bar": "health_bar",
            "chk_armor_bar": "armor_bar",
            "chk_spotted_esp": "spotted_esp",
            "chk_hide_spotted_players": "hide_spotted_players",
            "chk_hide_while_spectated": "hide_while_spectated",
            "chk_custom_crosshair": "custom_crosshair",
            "chk_custom_crosshair_snipers_only": "custom_crosshair_snipers_only",
            "chk_radar_enabled": "radar_enabled",
            "chk_radar_external_mode": "radar_external_mode",
            "chk_radar_external_additional_info": "radar_external_additional_info",
            "chk_radar_overlap_game": "radar_overlap_game",
            "chk_aimbot_enabled": "aimbot_enabled",
            "chk_aimbot_require_key": "aimbot_require_key",
            "chk_aimbot_visible_check": "aimbot_visible_check",
            ...
            "chk_rounded_corners_cheat": "rounded_window_corners",
            ...
            "chk_local_flashed_indicator": "local_flashed_indicator_enabled",
            "chk_fov_apply_while_scoped": "fov_apply_while_scoped",
            "chk_fps_cap_enabled": "fps_cap_enabled",
            "chk_hide_on_tabout": "hide_on_tabout",
            "chk_show_status_labels": "show_status_labels",
            "chk_show_spectator_list": "show_spectator_list_overlay",

            "chk_triggerbot_auto_weapon_preset": "triggerbot_auto_weapon_preset",
            "chk_rcs_enabled": "rcs_enabled",
            "chk_rcs_horizontal_compensation": "rcs_horizontal_compensation",
            "chk_rcs_vertical_compensation": "rcs_vertical_compensation",
        ...
        slider_mappings = {
            ...
            "slider_rcs_visualizer_transparency": "rcs_visualizer_transparency",
            ...
            "slider_distance_text_size": "distance_text_size",
            "slider_flashed_indicator_text_size": "flashed_indicator_text_size",
            ...
            "slider_rcs_horizontal_smoothing": "rcs_horizontal_smoothing",
            "slider_rcs_vertical_smoothing": "rcs_vertical_smoothing",
            "slider_rcs_horizontal_compensation_percentage": "rcs_horizontal_compensation_percentage",
            "slider_rcs_vertical_compensation_percentage": "rcs_vertical_compensation_percentage",
        ...
        # Apply runtime viewport and transparency settings immediately after slider values are refreshed
        try:
            viewport_width = Active_Config.get("window_width", WINDOW_WIDTH)
            viewport_height = Active_Config.get("window_height", WINDOW_HEIGHT)
            dpg.configure_viewport(0, width=viewport_width, height=viewport_height)
        except Exception:
            pass

        try:
            menu_alpha = Active_Config.get("menu_transparency", 255)
            apply_menu_transparency(menu_alpha)
        except Exception:
            pass
        ...
        # RCS smoothing technique combo
        if dpg.does_item_exist("combo_rcs_smoothing_technique"):
            technique = Active_Config.get("rcs_smoothing_technique", "Instant")
            dpg.set_value("combo_rcs_smoothing_technique", technique)
        ...
        # External radar additional info checkbox
        if dpg.does_item_exist("chk_radar_external_additional_info"):
            additional_info = Active_Config.get("radar_external_additional_info", False)
            dpg.set_value("chk_radar_external_additional_info", additional_info)
        ...
        if dpg.does_item_exist("combo_aimbot_target_bone"):
            bone = normalize_target_bone_value(Active_Config.get("aimbot_target_bone", "Head"))
            Active_Config["aimbot_target_bone"] = bone
        ...
        # Feature-specific targeting combos
        if dpg.does_item_exist("combo_esp_targeting_type"):
            dpg.set_value("combo_esp_targeting_type", "All Players" if Active_Config.get("esp_targeting_type", 0) == 1 else "Enemies Only")
        if dpg.does_item_exist("combo_aimbot_targeting_type"):
            dpg.set_value("combo_aimbot_targeting_type", "All Players" if Active_Config.get("aimbot_targeting_type", 0) == 1 else "Enemies Only")
        if dpg.does_item_exist("combo_triggerbot_targeting_type"):
            dpg.set_value("combo_triggerbot_targeting_type", "All Players" if Active_Config.get("triggerbot_targeting_type", 0) == 1 else "Enemies Only")
        if dpg.does_item_exist("combo_acs_targeting_type"):
            dpg.set_value("combo_acs_targeting_type", "All Players" if Active_Config.get("acs_targeting_type", 0) == 1 else "Enemies Only")
        ...
        if dpg.does_item_exist("combo_box_type"):
            box_type = Active_Config.get("box_type", "2D Full")
        ...
        color_mappings = {
            ...
            "color_crosshair_targeting": "crosshair_targeting_color",
        ...
        if rcs_state.get("settings") is not None:
        ...
        # Handle RCS thread based on config
        rcs_enabled = Active_Config.get("rcs_enabled", False)
        if rcs_enabled and not rcs_state["running"]:
            start_rcs_thread()
        elif not rcs_enabled and rcs_state["running"]:
            stop_rcs_thread()
        ...
        # Keep triggerbot burst shots slider visible regardless of burst mode
        if dpg.does_item_exist("slider_triggerbot_burst_shots"):
            dpg.configure_item("slider_triggerbot_burst_shots", show=True)

        # Keep the aimbot separator after smoothness aligned with the humanization state
        if dpg.does_item_exist("separator_aimbot_after_smoothness"):
            dpg.configure_item("separator_aimbot_after_smoothness", show=not Active_Config.get("aimbot_use_humanization", False))
        
        # Keep FOV slider always visible and always enabled
        if dpg.does_item_exist("slider_fov_value"):
            dpg.configure_item("slider_fov_value", show=True, enabled=True)
            if dpg.does_item_exist("tooltip_fov_value"):
                dpg.configure_item("tooltip_fov_value", show=True)
        if dpg.does_item_exist("chk_fov_apply_while_scoped"):
            dpg.configure_item("chk_fov_apply_while_scoped", show=True, enabled=True)
            if dpg.does_item_exist("tooltip_fov_apply_while_scoped"):
                dpg.configure_item("tooltip_fov_apply_while_scoped", show=True)
        ...
        # Custom crosshair option visibility stays visible even when the crosshair is disabled
        if dpg.does_item_exist("slider_custom_crosshair_scale"):
            show_tips = Active_Config.get("show_tooltips", True)
            crosshair_options = [
            ...
            for option in crosshair_options:
                if dpg.does_item_exist(option):
                    dpg.configure_item(option, show=True)
            ...
            for option in tooltip_options:
                if dpg.does_item_exist(option):
                    dpg.configure_item(option, show=show_tips)
        ...
        sync_esp_overlay_thread_state()
        sync_external_radar_window_state()
        sync_spectator_list_thread_state()
...
def save_config_to_file(config_name):
    """
    Save the current settings payload to the settings folder, including the keybinds.
    ...
    try:
        ...
        debug_log(f"Config saved: {clean_name}.json (settings with keybinds)", "SUCCESS")
        return True
    except Exception as e:
...
def reset_to_default_config():
    ...
    # Reset Active_Config to default values (deep copy to avoid shared nested state)
    Active_Config = copy.deepcopy(Default_Config)
...
def on_bhop_toggle(sender, value):
    """Handle bunnyhop enable/disable toggle."""
...
def cycle_triggerbot_preset(direction):
    ...
    current_preset = Active_Config.get("current_triggerbot_preset", "Default")
...
def bhop_loop():
    """Main bunnyhop loop that toggles jump based on spacebar and player movement state."""
    global bhop_state
    ...
    while bhop_state["running"]:
        try:
            if not is_cs2_active():
                if was_jumping:
                    pm, client = get_shared_memory_connection()
                    if pm and client and dwForceJump is not None:
                        pm.write_int(client + dwForceJump, FORCE_JUMP_INACTIVE)
                    was_jumping = False
                time.sleep(MAIN_LOOP_SLEEP)
                continue

            if dwForceJump is None or dwLocalPlayerPawn is None or m_fFlags is None:
                time.sleep(MAIN_LOOP_SLEEP)
                continue
            ...
            bhop_key_name = (Keybinds_Config.get("bhop_key", "space") or "space").lower()
            if bhop_key_name == "none":
                key_pressed = False
            else:
                bhop_vk_code = KEY_NAME_TO_VK.get(bhop_key_name, 0x20)
                key_pressed = (win32api.GetAsyncKeyState(bhop_vk_code) & 0x8000) != 0

            if key_pressed:
                local_player_pawn = pm.read_longlong(client + dwLocalPlayerPawn)
                if local_player_pawn:
                    flags = pm.read_int(local_player_pawn + m_fFlags)
                    on_ground = (flags & 1) != 0
                    if on_ground:
                        pm.write_int(client + dwForceJump, FORCE_JUMP_ACTIVE)
                    else:
                        pm.write_int(client + dwForceJump, FORCE_JUMP_INACTIVE)
                    was_jumping = True
                else:
...
def is_cs2_active():
    ...
    try:
        pm, _ = get_shared_memory_connection()
        if not pm:
            return False
...
def load_offsets_from_github():
    ...
    try:
        ...
        import traceback
        error_msg = f"GitHub fetch error: {str(e)}\n{traceback.format_exc()}"
        try:
            print(error_msg)
            debug_log(error_msg, "ERROR")
            with open(os.path.join(SCRIPT_DIR, "offset_download_error.log"), "w") as f:
                f.write(error_msg)
        except:
            pass
        return None, None
...
def cleanup_local_offsets_directory(offsets_dir):
    """Remove all files from the local offsets output directory except the core offset files."""
    ...
    protected_files_lower = {name.lower() for name in protected_files}

    try:
        for entry in os.listdir(offsets_dir):
            entry_path = os.path.join(offsets_dir, entry)
            if entry.lower() in protected_files_lower:
                continue
            ...
            try:
                ...
                debug_log(f"Failed to remove local offsets entry {entry_path}: {str(e)}", "WARNING")
    except Exception as e:
        debug_log(f"Failed to cleanup local offsets directory {offsets_dir}: {str(e)}", "WARNING")
...
def initialize_offset_globals():
    ...
    global dwWeaponC4, dwSensitivity, dwSensitivity_sensitivity, dwForceJump
    global m_iTeamNum, m_lifeState, m_pGameSceneNode, m_iHealth, m_fFlags, m_vecVelocity
    global m_hPlayerPawn, m_iszPlayerName, m_hObserverPawn, m_sSanitizedPlayerName
    global m_iIDEntIndex, m_ArmorValue, m_entitySpottedState, m_angEyeAngles
    global m_bIsScoped, m_pCameraServices, m_pObserverServices, m_hObserverTarget
    ...
    global m_iFOV, m_flFOVRate, m_iFOVStart, m_iDesiredFOV, m_flFlashMaxAlpha, m_flFlashDuration, m_flFlashBangTime
    ...
    try:
        ...
        def get_cls(cls_name, field_name):
            nonlocal missing_count
            try: return client_dll['client.dll']['classes'][cls_name]['fields'][field_name]
            except KeyError:
                missing_count += 1
                missing_offsets.append(f"{cls_name}::{field_name}")
                debug_log(f"Missing offset: {cls_name}::{field_name}", "WARNING")
                return 0

        def get_first_cls_field(candidates, fallback=0):
            for cls_name, field_name in candidates:
                try:
                    return client_dll['client.dll']['classes'][cls_name]['fields'][field_name]
                except KeyError:
                    continue
            return fallback

        # Extract commonly used offsets
        dwEntityList = get_core('dwEntityList')
        ...
        # Match reference project behavior: use dwSensitivity_sensitivity from offsets data.
        # Resolve this without counting it as missing until all fallbacks are exhausted.
        dwSensitivity_sensitivity = offsets.get('client.dll', {}).get('dwSensitivity_sensitivity', 0)

        # If current provider doesn't include this key, pull it from cs2-dumper output.
        if not dwSensitivity_sensitivity:
            try:
                ref_offsets_url = "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/offsets.json"
                ref_offsets = requests.get(ref_offsets_url, timeout=5).json()
                dwSensitivity_sensitivity = ref_offsets.get('client.dll', {}).get('dwSensitivity_sensitivity', 0)
            except Exception:
                dwSensitivity_sensitivity = 0

        if dwSensitivity_sensitivity:
            debug_log(f"Loaded dwSensitivity_sensitivity = {hex(dwSensitivity_sensitivity)}", "INFO")
        else:
            missing_count += 1
            missing_offsets.append("core::dwSensitivity_sensitivity")
            debug_log("dwSensitivity_sensitivity missing; RCS sensitivity scaling may be inaccurate", "WARNING")
        # Manually define dwForceJump for the current build.
        dwForceJump = 0x2095490
        debug_log(f"dwForceJump set manually to 0x{dwForceJump:X}", "SUCCESS")

        # Entity field offsets
        m_iTeamNum = get_cls('C_BaseEntity', 'm_iTeamNum')
        m_lifeState = get_cls('C_BaseEntity', 'm_lifeState')
        m_pGameSceneNode = get_cls('C_BaseEntity', 'm_pGameSceneNode')
        m_iHealth = get_cls('C_BaseEntity', 'm_iHealth')
        m_fFlags = get_cls('C_BaseEntity', 'm_fFlags')
        m_vecVelocity = get_cls('C_BaseEntity', 'm_vecVelocity')
        
        # Player controller offsets
        m_hPlayerPawn = get_cls('CCSPlayerController', 'm_hPlayerPawn')
        m_iszPlayerName = get_cls('CBasePlayerController', 'm_iszPlayerName')
        m_hObserverPawn = get_cls('CCSPlayerController', 'm_hObserverPawn')
        m_sSanitizedPlayerName = get_cls('CCSPlayerController', 'm_sSanitizedPlayerName')
        
        # Player pawn offsets
        m_iIDEntIndex = get_cls('C_CSPlayerPawn', 'm_iIDEntIndex')
        m_ArmorValue = get_cls('C_CSPlayerPawn', 'm_ArmorValue')
        m_entitySpottedState = get_cls('C_CSPlayerPawn', 'm_entitySpottedState')
        m_angEyeAngles = get_cls('C_CSPlayerPawn', 'm_angEyeAngles')
        m_bIsScoped = get_cls('C_CSPlayerPawn', 'm_bIsScoped')
        m_pCameraServices = get_cls('C_BasePlayerPawn', 'm_pCameraServices')
        m_pObserverServices = get_cls('C_BasePlayerPawn', 'm_pObserverServices')
        m_hObserverTarget = get_cls('CPlayer_ObserverServices', 'm_hObserverTarget')
        ...
        m_vecOrigin = get_cls('CGameSceneNode', 'm_vecOrigin')
        m_modelState = get_cls('CSkeletonInstance', 'm_modelState')
        
        # Weapon and item offsets
        m_AttributeManager = get_cls('C_EconEntity', 'm_AttributeManager')
        m_Item = get_cls('C_AttributeContainer', 'm_Item')
        m_iItemDefinitionIndex = get_cls('C_EconItemView', 'm_iItemDefinitionIndex')
        m_hActiveWeapon = get_cls('CPlayer_WeaponServices', 'm_hActiveWeapon')
        m_iClip1 = get_cls('C_BasePlayerWeapon', 'm_iClip1')
        # Hitsound offsets can move between updates/classes, so resolve from known candidates with safe fallbacks.
        pBulletServicesOffset = get_first_cls_field([
            ('C_CSPlayerPawn', 'm_pBulletServices'),
            ('C_CSPlayerPawnBase', 'm_pBulletServices'),
        ], fallback=0x1468)
        m_totalHits = get_first_cls_field([
            ('CCSPlayer_BulletServices', 'm_totalHitsOnServer'),
            ('C_CSPlayer_BulletServices', 'm_totalHitsOnServer'),
            ('CCSPlayer_BulletServices', 'm_totalHits'),
            ('C_CSPlayer_BulletServices', 'm_totalHits'),
        ], fallback=0x48)
        ...
        # Spotted state offsets
        m_bSpotted = get_cls('EntitySpottedState_t', 'm_bSpotted')
        m_bSpottedByMask = get_cls('EntitySpottedState_t', 'm_bSpottedByMask')
        
        # Camera and visual offsets
        m_iFOV = get_cls('CCSPlayerBase_CameraServices', 'm_iFOV')
        m_flFOVRate = get_cls('CCSPlayerBase_CameraServices', 'm_flFOVRate')
        m_iFOVStart = get_cls('CCSPlayerBase_CameraServices', 'm_iFOVStart')
        m_iDesiredFOV = get_cls('CBasePlayerController', 'm_iDesiredFOV')
        m_flFlashMaxAlpha = get_cls('C_CSPlayerPawnBase', 'm_flFlashMaxAlpha')
        m_flFlashDuration = get_cls('C_CSPlayerPawnBase', 'm_flFlashDuration')
        m_flFlashBangTime = get_cls('C_CSPlayerPawnBase', 'm_flFlashBangTime')
        ...
        import traceback
        error_msg = f"Failed to initialize offsets: {str(e)}\n{traceback.format_exc()}"
        try:
            debug_log(error_msg, "ERROR")
            print(error_msg)
            with open(os.path.join(SCRIPT_DIR, "offset_init_error.log"), "w") as f:
                f.write(error_msg)
        except:
            pass
        return False
...
def load_dwForceJump():
    """
    Return the manually defined dwForceJump offset for the current build.
    
    Returns:
        int: The dwForceJump offset
    """
    global dwForceJump
    try:
        dwForceJump = 0x2095490
        ...
        debug_log(f"Failed to set dwForceJump manually: {str(e)}", "ERROR")
        dwForceJump = 0
        return 0
    ...
    try:
        for proc in psutil.process_iter(["name"]):
            try:
                ...
                if proc_name and proc_name.lower() == "cs2.exe":
                    return True
...
def _is_foreground_window(hwnd):
    """Return True if the provided HWND is the current foreground window."""
    try:
...
def is_cs2_foreground():
    """Check whether the current foreground context should keep the hide-on-tabout feature inactive."""
    try:
        foreground_hwnd = win32gui.GetForegroundWindow()
        if not foreground_hwnd:
            return False

        window_title = (win32gui.GetWindowText(foreground_hwnd) or "").strip()
        if "counter-strike 2" in window_title.lower():
            return True

        tracked_hwnds = []
        cheat_hwnd = drag_state.get("hwnd")
        if cheat_hwnd:
            tracked_hwnds.append(cheat_hwnd)

        overlay_hwnd = esp_overlay.get("hwnd")
        if overlay_hwnd:
            tracked_hwnds.append(overlay_hwnd)

        radar_hwnd = radar_external_state.get("hwnd")
        if radar_hwnd:
            tracked_hwnds.append(radar_hwnd)

        return any(_is_foreground_window(hwnd) for hwnd in tracked_hwnds)
    except Exception:
        return False
...
def w2s(view_matrix, x, y, z, width, height):
    ...
    Optimized with early screen culling to skip off-screen coordinate calculations.
    ...
    Returns:
        tuple: (screen_x, screen_y) or (-999, -999) if behind camera or off-screen
    """
    try:
        if not view_matrix or width is None or height is None:
            return -999, -999
        ...
        # Early exit for off-screen coordinates (with 50px margin)
...
def project_point_to_screen(view_matrix, x, y, z, width, height, clamp_to_edge=False):
    """Project a world point to screen coordinates and optionally clamp to the viewport edge."""
    try:
        if not view_matrix or width is None or height is None:
            return -999, -999, -1
...
class ESPOverlay:
    ...
    def _load_fonts(self):
        """Load fonts for overlay text rendering with sharper, pixel-snapped settings."""
        io = imgui.get_io()
        ...
        def add_font_with_config(path, size):
            """Add font with config tuned for sharper text on the overlay."""
            try:
                config = None
                if hasattr(imgui, 'FontConfig'):
                    ...
                    try:
                        ...
                        pass
                return io.fonts.add_font_from_file_ttf(path, size, config)
            except TypeError:
                return io.fonts.add_font_from_file_ttf(path, size)
            except Exception:
                return None
        ...
        if os.path.exists(default_font_path):
            ...
            for size in font_sizes:
                try:
                    font = add_font_with_config(default_font_path, size)
                    if font:
        ...
        for font_name, font_path in MENU_FONTS.items():
            if font_path and os.path.exists(font_path):
                ...
                for size in font_sizes:
                    try:
                        font = add_font_with_config(font_path, size)
                        if font:
    ...
    def create_window(self, x, y, width, height):
        ...
        # Use highest quality MSAA by default
    ...
    def draw_rect(self, x, y, width, height, color='red', thickness=1.5, rounding=0.0):
    ...
    def draw_filled_rect(self, x, y, width, height, r, g, b, rounding=0.0):
    ...
    def draw_line_rgb(self, x1, y1, x2, y2, rgb_tuple, thickness=1.5, alpha=1.0):
        """Draw a line with RGB color tuple (0-255 values) and optional alpha."""
        if not self.draw_list:
            return
        
        col = imgui.get_color_u32_rgba(rgb_tuple[0]/255.0, rgb_tuple[1]/255.0, rgb_tuple[2]/255.0, alpha)
        self.draw_list.add_line(float(x1), float(y1), float(x2), float(y2), col, thickness)

    def draw_triangle_filled_rgb(self, p1x, p1y, p2x, p2y, p3x, p3y, rgb_tuple, alpha=1.0):
        """Draw a filled triangle with RGB color tuple (0-255 values) and optional alpha."""
        if not self.draw_list:
            return
    ...
    def draw_rect_rgb(self, x, y, width, height, rgb_tuple, thickness=1.5, rounding=0.0):
    ...
    def draw_text(self, x, y, text, r=255, g=255, b=255, size=13.0, stroke=False, font_name=None):
        ...
        # Snap to integer pixels for crisper text rendering
        x_pos = int(round(float(x)))
        y_pos = int(round(float(y)))
        ...
        # Draw a crisp black outline for readability without changing size
        if stroke:
            ...
            for dx in [-stroke_offset, 0, stroke_offset]:
                for dy in [-stroke_offset, 0, stroke_offset]:
                    if dx == 0 and dy == 0:
                        continue
                    self.draw_list.add_text(float(x_pos + dx), float(y_pos + dy), black_col, str(text))
        
        # Draw main text
        col = imgui.get_color_u32_rgba(r/255.0, g/255.0, b/255.0, 1.0)
        self.draw_list.add_text(float(x_pos), float(y_pos), col, str(text))
...
def read_spotted_state(pm, entity_pawn_addr, spotted_state_offset, settings=None, spotted_bit_offset=0, default=False):
    """Read the spotted state for an entity while respecting the hide-spotted toggle."""
    if not pm or entity_pawn_addr is None:
        return default
    ...
    try:
        spotted_flag = pm.read_int(entity_pawn_addr + spotted_state_offset + spotted_bit_offset)
        return spotted_flag != 0
    except Exception:
        try:
            return bool(pm.read_bool(entity_pawn_addr + spotted_state_offset + spotted_bit_offset))
        except Exception:
            return default


def should_hide_spotted_player(settings, is_spotted):
    """Return True when visible-player hiding is enabled and the entity is spotted."""
    return bool(settings.get("hide_spotted_players", False) and is_spotted)


def get_flash_indicator_text(pm, entity_pawn_addr):
    """Return a live flashed/not-flashed label for an entity.

    CS2 keeps ``m_flFlashDuration`` at the original flash duration while the
    effect is active, so track elapsed monotonic time for the countdown.
    """
    if not pm or not entity_pawn_addr or m_flFlashDuration is None:
        return "Not Flashed"

    try:
        flash_duration = max(0.0, float(pm.read_float(entity_pawn_addr + m_flFlashDuration)))
        if flash_duration <= 0.0:
            flash_indicator_state.pop(entity_pawn_addr, None)
            return "Not Flashed"

        flash_event_time = None
        if m_flFlashBangTime is not None:
            try:
                flash_event_time = float(pm.read_float(entity_pawn_addr + m_flFlashBangTime))
            except Exception:
                pass

        now = time.monotonic()
        state = flash_indicator_state.get(entity_pawn_addr)
        new_flash = (
            state is None
            or (flash_event_time is not None and flash_event_time != state.get("event_time"))
            or flash_duration > state.get("duration", 0.0) + 0.05
        )
        if new_flash:
            state = {
                "duration": flash_duration,
                "started_at": now,
                "event_time": flash_event_time,
            }
            flash_indicator_state[entity_pawn_addr] = state

        remaining = max(0.0, state["duration"] - (now - state["started_at"]))
        if remaining <= 0.0:
            return "Not Flashed"
        return f"Flashed: {max(1, math.ceil(remaining))}"
    except Exception:
        return "Not Flashed"


def render_local_flashed_indicator(overlay, pm, client, settings):
    """Render a centered countdown while the local player is flashed."""
    if not settings.get("local_flashed_indicator_enabled", False):
        return
    if not overlay or not pm or not client or dwLocalPlayerPawn is None:
        return
    ...
    try:
        ...
        indicator_text = get_flash_indicator_text(pm, local_player_pawn)
        if not indicator_text.startswith("Flashed:"):
            return

        indicator_text = indicator_text.replace("Flashed:", "FLASHED:", 1)
        text_size = 24.0
        text_width = overlay.calc_text_width(
            indicator_text,
            size=text_size,
            font_name=settings.get("menu_font", "Default"),
        )
        x = (overlay.width - text_width) / 2.0
...
def is_entity_visible_to_local_player(pm, entity_pawn_addr, local_player_index, default=True):
    """Return True when the local player has spotted the entity via the visibility mask."""
    if not pm or entity_pawn_addr is None or local_player_index is None or local_player_index <= 0:
        return default

    try:
        spotted_mask = pm.read_int(entity_pawn_addr + m_entitySpottedState + m_bSpottedByMask)
        return bool(spotted_mask & (1 << (local_player_index - 1)))
    except Exception:
        return default


def should_hide_esp_while_spectated(settings, pm, client):
    """Return True when ESP should stay hidden while spectators are present."""
    if not settings.get("hide_while_spectated", False):
        return False

    try:
        if not pm or not client:
            return False
        if dwLocalPlayerPawn is None:
            return False
        ...
        spectator_names = get_spectator_names(pm, client, local_player_pawn_addr)
        return bool(spectator_names)
    except Exception:
        return False
...
def render_esp_frame(overlay, pm, client, settings):
    ...
    if settings.get('hide_while_spectated', False) and should_hide_esp_while_spectated(settings, pm, client):
        return
    ...
    if list_entry:
        for i in range(1, 64):
            try:
                ...
                if entity_pawn_addr == local_player_pawn_addr:
                    local_player_index = i
                    break
            except Exception:
                continue
    
    targeting_type = get_targeting_mode_value(settings, "esp_targeting_type")

    # Cache settings to avoid repeated dict lookups in the hot ESP loop.
    line_esp_enabled = settings.get('line_esp', True)
    box_esp_enabled = settings.get('box_esp', True)
    box_type = settings.get('box_type', '2D Full')
    box_thickness_global = settings.get('box_thickness', 1.5)
    snapline_thickness_global = settings.get('snapline_thickness', 1.5)
    health_enabled = settings.get('health_bar', True)
    armor_enabled = settings.get('armor_bar', True)
    skeleton_enabled = settings.get('skeleton_esp', True)
    skeleton_thickness = settings.get('skeleton_thickness', 1.5)
    name_enabled = settings.get('name_esp', False)
    weapon_enabled = settings.get('weapon_esp', False)
    distance_enabled = settings.get('distance_esp', True)
    hide_spotted_players = settings.get('hide_spotted_players', False)
    default_font_name = settings.get('menu_font', 'Default')
    health_text_size = settings.get('health_text_size', 12.0)
    weapon_text_size = settings.get('weapon_text_size', 12.0)
    distance_text_size = settings.get('distance_text_size', 12.0)
    flashed_indicator_enabled = settings.get('flashed_indicator_enabled', False)
    flashed_indicator_text_size = settings.get('flashed_indicator_text_size', 14.0)
    name_text_size = settings.get('name_text_size', 14.0)
    health_position = str(settings.get('health_position', 'Vertical Left')).strip().lower()
    healthbar_type = str(settings.get('healthbar_type', 'Bars')).strip().lower()
    head_hitbox_enabled = settings.get('draw_head_hitbox', False)
    head_hitbox_color = settings.get('head_hitbox_color', (255, 0, 0))
    head_hitbox_thickness = settings.get('head_hitbox_thickness', 2.0)
    player_width = 32.0
    player_length = 32.0
    player_height = 72.0
    ...
    for i in range(1, 64):
        try:
            ...
            # Check visibility state only when visible-player hiding is active.
            if hide_spotted_players:
                is_visible_to_local = is_entity_visible_to_local_player(
                    pm,
                    entity_pawn_addr,
                    local_player_index,
                    default=False,
                )
                if is_visible_to_local:
                    continue
            ...
            # Use the unclamped projection for boxes/skeletons/labels so those elements stop rendering once the target is off-screen.
            head_pos = w2s(view_matrix, headX, headY, headZ, width, height)
            leg_pos = w2s(view_matrix, legX, legY, legZ, width, height)
            head_snap_pos = project_point_to_screen(view_matrix, headX, headY, headZ, width, height, clamp_to_edge=True)
            leg_snap_pos = project_point_to_screen(view_matrix, legX, legY, legZ, width, height, clamp_to_edge=True)

            # If the target is off-screen, only the snapline should render.
            if head_pos[0] == -999 or leg_pos[0] == -999:
                if line_esp_enabled and head_snap_pos[0] != -999:
                    if lines_position == 'Top':
                        line_end_x = head_snap_pos[0]
                        line_end_y = head_snap_pos[1]
                    else:
                        line_end_x = head_snap_pos[0]
                        line_end_y = leg_snap_pos[1] if leg_snap_pos[0] != -999 else head_snap_pos[1]
                    overlay.draw_line_rgb(center_x, center_y, line_end_x, line_end_y, snapline_color, snapline_thickness_global)
                continue
            ...
            deltaZ = abs(box_bottom_y - box_top_y)
            box_width = deltaZ // 2
            leftX = head_pos[0] - box_width // 2
            rightX = head_pos[0] + box_width // 2

            # Build a more accurate 2D rectangle from projected 3D player bounds.
            try:
                half_width = player_width / 2.0
                half_length = player_length / 2.0

                bound_corners = [
                    (entity_origin_x - half_width, entity_origin_y - half_length, entity_origin_z),
                    (entity_origin_x + half_width, entity_origin_y - half_length, entity_origin_z),
                    (entity_origin_x + half_width, entity_origin_y + half_length, entity_origin_z),
                    (entity_origin_x - half_width, entity_origin_y + half_length, entity_origin_z),
                    (entity_origin_x - half_width, entity_origin_y - half_length, entity_origin_z + player_height),
                    (entity_origin_x + half_width, entity_origin_y - half_length, entity_origin_z + player_height),
                    (entity_origin_x + half_width, entity_origin_y + half_length, entity_origin_z + player_height),
                    (entity_origin_x - half_width, entity_origin_y + half_length, entity_origin_z + player_height),
                ]
                ...
                if projected_points:
                    ...
                    proj_height = proj_bottom - proj_top

                    if proj_width > 2 and proj_height > 4:
                        leftX = proj_left
                        rightX = proj_right
                        box_top_y = proj_top
                        box_bottom_y = proj_bottom
                        box_width = proj_width
                        deltaZ = proj_height
            except Exception:
                pass
            
            # Draw snap lines using the clamped projection so lines remain visible even when the target is off-screen.
            if line_esp_enabled:
                if lines_position == 'Top':
                    line_end_x = head_snap_pos[0]
                    line_end_y = head_snap_pos[1]
            ...
            # Draw box ESP
            if box_esp_enabled:
                box_thickness = box_thickness_global
                ...
                if box_type == '3D Full':
                    ...
                    try:
                        # Use the already-read entity origin values instead of reading game scene again.
                        origin_x = entity_origin_x
                        origin_y = entity_origin_y
                        origin_z = entity_origin_z
                        ...
                        if all_corners_valid:
                            ...
                            overlay.draw_rect_rgb(leftX, box_top_y, box_width, deltaZ, box_color, box_thickness)
                            
                    except Exception:
                        # Fallback to 2D box on error
                        overlay.draw_rect_rgb(leftX, box_top_y, box_width, deltaZ, box_color, box_thickness)
                elif box_type == '3D Corner':
                    # 3D corner box - draw short corner markers on the projected 3D box corners.
                    corner_length = max(8, min(16, int(min(max(box_width, 1), max(deltaZ, 1)) * 0.2)))
                    try:
                        origin_x = entity_origin_x
                        origin_y = entity_origin_y
                        origin_z = entity_origin_z
                        ...
                        if all_corners_valid:
                            center_x = (leftX + rightX) / 2.0
                            center_y = (box_top_y + box_bottom_y) / 2.0
                            for screen_pos in bottom_screen + top_screen:
                                x, y = screen_pos
                                if x < center_x and y < center_y:
                                    overlay.draw_line_rgb(x, y, x + corner_length, y, box_color, box_thickness)
                                    overlay.draw_line_rgb(x, y, x, y + corner_length, box_color, box_thickness)
                        ...
                        overlay.draw_rect_rgb(leftX, box_top_y, box_width, deltaZ, box_color, box_thickness)
                elif box_type == '2D Corner':
                    # 2D corner box - draw only short corner angles at each box corner.
                    corner_length = max(8, min(16, int(min(max(box_width, 1), max(deltaZ, 1)) * 0.2)))
                    overlay.draw_line_rgb(leftX, box_top_y, leftX + corner_length, box_top_y, box_color, box_thickness)
                    overlay.draw_line_rgb(leftX, box_top_y, leftX, box_top_y + corner_length, box_color, box_thickness)
                    overlay.draw_line_rgb(rightX, box_top_y, rightX - corner_length, box_top_y, box_color, box_thickness)
                    overlay.draw_line_rgb(rightX, box_top_y, rightX, box_top_y + corner_length, box_color, box_thickness)
                    overlay.draw_line_rgb(leftX, box_bottom_y, leftX + corner_length, box_bottom_y, box_color, box_thickness)
                    overlay.draw_line_rgb(leftX, box_bottom_y, leftX, box_bottom_y - corner_length, box_color, box_thickness)
                    overlay.draw_line_rgb(rightX, box_bottom_y, rightX - corner_length, box_bottom_y, box_color, box_thickness)
                    overlay.draw_line_rgb(rightX, box_bottom_y, rightX, box_bottom_y - corner_length, box_color, box_thickness)
                else:
                    # 2D Full box (default)
                    overlay.draw_rect_rgb(leftX, box_top_y, box_width, deltaZ, box_color, box_thickness)
            ...
            if health_enabled:
                ...
                # Shared 2D box geometry used by both text and bars for consistent placement.
                box_left_i = int(min(leftX, rightX))
                box_right_i = int(max(leftX, rightX))
                box_top_i = int(min(box_top_y, box_bottom_y))
                box_bottom_i = int(max(box_top_y, box_bottom_y))
                box_w_i = max(12, box_right_i - box_left_i)
                box_h_i = max(1, box_bottom_i - box_top_i)
                box_center_x = (box_left_i + box_right_i) / 2.0
                
                if healthbar_type == 'text':
                    # Text mode - show percentage labels
                    text_size = health_text_size
                    ...
                    if health_position == 'horizontal above':
                        ...
                        text_x = box_center_x - health_width / 2
                        text_y = box_top_i - text_size - 2
                        ...
                        if armor_text:
                            ...
                            armor_x = box_center_x - armor_width / 2
                        ...
                        text_x = box_right_i + 5
                        text_y = box_top_i + box_h_i / 2 - text_size / 2
                        overlay.draw_text(text_x, text_y, health_text, health_color[0], health_color[1], health_color[2], size=text_size, stroke=True, font_name=selected_font)
                        if armor_text:
                        ...
                        health_width = overlay.calc_text_width(health_text, text_size, selected_font)
                        text_x = box_left_i - health_width - 5
                        text_y = box_top_i + box_h_i / 2 - text_size / 2
                        overlay.draw_text(text_x, text_y, health_text, health_color[0], health_color[1], health_color[2], size=text_size, stroke=True, font_name=selected_font)
                        if armor_text:
                            armor_width = overlay.calc_text_width(armor_text, text_size, selected_font)
                            armor_x = box_left_i - armor_width - 5
                            overlay.draw_text(armor_x, text_y + text_size + 1, armor_text, armor_color[0], armor_color[1], armor_color[2], size=text_size, stroke=True, font_name=selected_font)
                else:
                    # Bars mode - keep bar geometry distance-stable.
                    box_top = box_top_i
                    box_bottom = box_bottom_i
                    box_h = box_h_i
                    box_left = box_left_i
                    box_w = box_w_i

                    horizontal_len = box_w
                    horizontal_thickness = 4
                    vertical_thickness = 4
                    padding = 3

                    if health_position == 'horizontal below':
                        bar_x = box_left + max(0, (box_w - horizontal_len) // 2)
                        below_offset = 8
                        bar_y = box_bottom + padding + below_offset
                        hp_width = max(0, min(horizontal_len, int(round(horizontal_len * hp_percent))))
                        overlay.draw_filled_rect_brush(bar_x, bar_y, horizontal_len, horizontal_thickness, 'health_bg')
                        if hp_percent > 0.5:
                            overlay.draw_filled_rect_brush(bar_x, bar_y, hp_width, horizontal_thickness, 'health_green')
                        elif hp_percent > 0.25:
                            overlay.draw_filled_rect_brush(bar_x, bar_y, hp_width, horizontal_thickness, 'health_yellow')
                        else:
                            overlay.draw_filled_rect_brush(bar_x, bar_y, hp_width, horizontal_thickness, 'health_red')

                        if armor_percent > 0:
                            armor_y = bar_y + horizontal_thickness + padding
                            armor_width = max(0, min(horizontal_len, int(round(horizontal_len * armor_percent))))
                            overlay.draw_filled_rect_brush(bar_x, armor_y, horizontal_len, horizontal_thickness, 'health_bg')
                            overlay.draw_filled_rect_brush(bar_x, armor_y, armor_width, horizontal_thickness, 'armor_blue')
                    elif health_position == 'horizontal above':
                        bar_x = box_left + max(0, (box_w - horizontal_len) // 2)
                        bar_y = box_top - horizontal_thickness - padding
                        hp_width = max(0, min(horizontal_len, int(round(horizontal_len * hp_percent))))
                        overlay.draw_filled_rect_brush(bar_x, bar_y, horizontal_len, horizontal_thickness, 'health_bg')
                        if hp_percent > 0.5:
                            overlay.draw_filled_rect_brush(bar_x, bar_y, hp_width, horizontal_thickness, 'health_green')
                        ...
                        bar_x = box_left + box_w + padding
                        hp_height = max(0, min(box_h, int(round(box_h * hp_percent))))
                        overlay.draw_filled_rect_brush(bar_x, box_top, vertical_thickness, box_h, 'health_bg')
                        if hp_percent > 0.5:
                            overlay.draw_filled_rect_brush(bar_x, box_bottom - hp_height, vertical_thickness, hp_height, 'health_green')
                        elif hp_percent > 0.25:
                            overlay.draw_filled_rect_brush(bar_x, box_bottom - hp_height, vertical_thickness, hp_height, 'health_yellow')
                        else:
                            overlay.draw_filled_rect_brush(bar_x, box_bottom - hp_height, vertical_thickness, hp_height, 'health_red')

                        if armor_percent > 0:
                            armor_x = bar_x + vertical_thickness + padding
                            armor_height = max(0, min(box_h, int(round(box_h * armor_percent))))
                        ...
                        bar_x = box_left - vertical_thickness - padding
                        hp_height = max(0, min(box_h, int(round(box_h * hp_percent))))
                        overlay.draw_filled_rect_brush(bar_x, box_top, vertical_thickness, box_h, 'health_bg')
                        if hp_percent > 0.5:
                            overlay.draw_filled_rect_brush(bar_x, box_bottom - hp_height, vertical_thickness, hp_height, 'health_green')
            ...
            if head_hitbox_enabled:
                ...
                if world_hitbox:
                    ...
                    rings = generate_projected_sphere_rings(
                        ...
                        sphere_center,
                        sphere_radius_world,
                        segments=HEAD_HITBOX_RING_SEGMENTS
                    )
                    draw_projected_sphere(overlay, rings, head_hitbox_color, head_hitbox_thickness)
            ...
            if healthbar_type == 'text' and health_position == 'horizontal above':
                ...
                if health_enabled and entity_armor > 0:
                    text_y_offset = box_top_y - text_size * 2 - 6  # Start above both health and armor text
                else:
                    text_y_offset = box_top_y - text_size - 4  # Start above health text only
                ...
                if health_enabled and entity_armor > 0:
                    text_y_offset = box_top_y - 14  # Start above armor bar
                else:
                    text_y_offset = box_top_y - 10  # Start above health bar only
            else:
            ...
            if settings.get('name_esp', False):
                try:
                    ...
                    if player_name:
                        ...
                        # Draw with white color and no stroke for a cleaner look
                        overlay.draw_text(text_x, text_y_offset - font_size, player_name, 255, 255, 255, size=font_size, stroke=False, font_name=selected_font)
                        
                        # Move offset up for the next label with consistent spacing
                        text_y_offset -= (font_size + 2)
                except:
                    pass

            # Draw weapon label above head (stacked with nickname/spotted labels)
            if settings.get('weapon_esp', False):
                try:
                    if not player_weapon_name:
                        player_weapon_name = get_weapon_name_from_pawn(pm, client, entity_pawn_addr)

                    if player_weapon_name and player_weapon_name != "Unknown":
                        font_size = weapon_text_size
                        selected_font = default_font_name

                        text_width = overlay.calc_text_width(player_weapon_name, font_size, selected_font)
                        text_x = head_pos[0] - (text_width / 2)

                        overlay.draw_text(text_x, text_y_offset - font_size, player_weapon_name, 255, 255, 255, size=font_size, stroke=False, font_name=selected_font)
            ...
            # Draw distance above the player, stacked with other labels when enabled
            if distance_enabled:
                ...
                # Place the distance label using the same stacked offset as the other labels,
                # keeping the spacing even with the nickname and weapon text.
                distance_y = text_y_offset - text_size
                
                overlay.draw_text(text_x, distance_y, distance_text, 255, 255, 255, size=text_size, stroke=False, font_name=selected_font)
                text_y_offset -= (text_size + 2)

            # Draw the flashed state after the other labels so it remains stacked cleanly.
            if flashed_indicator_enabled:
                try:
                    flashed_text = get_flash_indicator_text(pm, entity_pawn_addr)
                    text_size = flashed_indicator_text_size
                    selected_font = default_font_name
                    text_width = overlay.calc_text_width(flashed_text, text_size, selected_font)
                    text_x = head_pos[0] - (text_width / 2)
                    flashed_y = text_y_offset - text_size
                    overlay.draw_text(text_x, flashed_y, flashed_text, 255, 255, 255, size=text_size, stroke=False, font_name=selected_font)
                    text_y_offset -= (text_size + 2)
                except Exception:
                    pass
...
def render_snaplines_frame(overlay, pm, client, settings):
    ...
    # Cache snapline-related settings for performance in the hot loop.
    line_esp_enabled = settings.get('line_esp', True)
    snapline_thickness_global = settings.get('snapline_thickness', 1.5)
    hide_spotted_players = settings.get('hide_spotted_players', False)
    ...
    for i in range(1, 64):
        try:
            ...
            # Convert to screen coordinates
            head_pos = project_point_to_screen(view_matrix, headX, headY, headZ, width, height, clamp_to_edge=True)
            if head_pos[0] == -999:
                continue
            
            leg_pos = project_point_to_screen(view_matrix, headX, headY, legZ, width, height, clamp_to_edge=True)
...
def draw_skeleton(overlay, pm, bone_matrix, view_matrix, width, height, skeleton_color=(255, 255, 255), thickness=1.5):
    ...
    Optimized to read a contiguous bone block once and then render only
    the required bone connections.
    """
    bone_positions = {}
    
    try:
        bone_ids = SKELETON_BONE_IDS
        if not bone_ids:
            return
        
        bone_block = pm.read_bytes(bone_matrix + SKELETON_BONE_MIN_ID * 0x20, SKELETON_BONE_BLOCK_SIZE)
        
        for bone_name, bone_id in bone_ids.items():
            offset = (bone_id - SKELETON_BONE_MIN_ID) * 0x20
            bone_bytes = bone_block[offset:offset + 12]
            if len(bone_bytes) < 12:
                continue
        ...
        for bone1, bone2 in BONE_CONNECTIONS:
            pos1 = bone_positions.get(bone1)
            pos2 = bone_positions.get(bone2)
...
def render_radar(overlay, pm, client, settings):
    ...
    try:
        ...
        if settings.get('radar_external_mode', False):
            return
...
def render_rcs_visualizer(overlay, pm, client, settings):
    """
    Render a simple recoil visualizer using the same aim-punch offsets as RCS.

    The indicator is based on the current aim punch cache entry and
    draws a screen-center vector showing recoil direction.
    """
    if not settings or not isinstance(settings, dict):
        return
    if not settings.get('rcs_visualizer_enabled', False):
        return
    if not overlay or not pm or not client:
        return
    ...
    try:
        ...
        aim_punch_services = pm.read_longlong(local_player_pawn + m_pAimPunchServices)
        if not aim_punch_services:
            return

        cache_offset = m_unpredictableBaseTick - 0x18
        cache_count = pm.read_ulonglong(aim_punch_services + cache_offset)
        cache_data = pm.read_ulonglong(aim_punch_services + cache_offset + 0x08)
        if cache_count <= 0 or cache_count >= 1000 or not cache_data:
            return

        last_elem_addr = cache_data + (cache_count - 1) * 12
        punch_x = pm.read_float(last_elem_addr)
        punch_y = pm.read_float(last_elem_addr + 4)

        # Only show while shooting. Track shot count so single pistol shots still display.
        current_time = time.time()
        previous_shots_fired = getattr(render_rcs_visualizer, "last_shots_fired", 0)
        shots_fired = 0
        try:
        ...
        if shots_fired > previous_shots_fired:
            render_rcs_visualizer.last_shot_time = current_time
        render_rcs_visualizer.last_shots_fired = shots_fired

        last_shot_time = getattr(render_rcs_visualizer, "last_shot_time", 0.0)
        if current_time - last_shot_time > 0.20:
            return

        # Limit the visible magnitude to avoid runaway indicators
        visual_scale = 16.0
        dx = punch_y * visual_scale
        dy = -punch_x * visual_scale

        center_x = overlay.width / 2.0
        center_y = overlay.height / 2.0
        target_x = center_x + dx
        target_y = center_y + dy

        color = settings.get('rcs_visualizer_color', (255, 200, 0))
        if not color or len(color) < 3:
            color = (255, 200, 0)
        transparency = max(0, min(int(settings.get('rcs_visualizer_transparency', 255)), 255))
        alpha = transparency / 255.0
        direction_length = math.hypot(dx, dy)
        if direction_length > 1e-3:
            dir_x = dx / direction_length
            dir_y = dy / direction_length
            perp_x = -dir_y
            perp_y = dir_x

            arrow_length = 10.0
            arrow_width = 14.0
            base_x = target_x - dir_x * arrow_length
            base_y = target_y - dir_y * arrow_length
            left_x = base_x + perp_x * (arrow_width * 0.5)
            left_y = base_y + perp_y * (arrow_width * 0.5)
            right_x = base_x - perp_x * (arrow_width * 0.5)
            right_y = base_y - perp_y * (arrow_width * 0.5)

            overlay.draw_line_rgb(center_x, center_y, base_x, base_y, color, thickness=2.5, alpha=alpha)
            overlay.draw_triangle_filled_rgb(target_x, target_y, left_x, left_y, right_x, right_y, color, alpha=alpha)
    except Exception:
        pass


def render_custom_crosshair(overlay, pm, client, settings):
    """
    ...
    Optionally changes color when aiming at an enemy (uses same detection as triggerbot).
    ...
    try:
        ...
        # Snipers-only mode: only show crosshair for AWP/SSG 08 when not scoped
        if settings.get('custom_crosshair_snipers_only', False):
            try:
                if not pm or not client:
                    return
                ...
                is_scoped = False
                if m_bIsScoped is not None:
                    try:
                        is_scoped = bool(pm.read_bool(local_player_pawn + m_bIsScoped))
                    except Exception:
                        is_scoped = False
                ...
                weapon_name = get_local_weapon_name(pm, client)
                if (weapon_name or '').upper() not in {"AWP", "SSG 08"}:
                    return
            except Exception:
                return
        ...
        # Check if we should use targeting color (same detection as triggerbot)
        if settings.get('crosshair_use_targeting_color', False) and pm and client:
            try:
                ...
                if local_player_pawn:
                    ...
                    # Read entity ID under crosshair (same as triggerbot)
                    ...
                    if entity_id > 0:
                        ...
                        if entity_list:
                            ...
                            if list_entry:
                                ...
                                if entity_pawn:
                                    # Check if it's a valid target based on targeting type
                                    entity_team = pm.read_int(entity_pawn + m_iTeamNum)
                                    targeting_type = get_targeting_mode_value(settings, "triggerbot_targeting_type")
                                    # Use targeting color if: enemy (always) OR all players mode (targeting_type == 1)
                                    if entity_team != local_team or targeting_type == 1:
                                        # Aiming at a valid target - use targeting color
                                        targeting_color = settings.get('crosshair_targeting_color', (255, 0, 0))
                                        if targeting_color and len(targeting_color) >= 3:
                                            color = targeting_color
            except:
                pass  # Fall back to normal color on any error
...
def render_aimbot_snaplines(overlay, pm, client, settings):
    ...
    try:
        ...
        # Draw every frame so the line stays visually smooth instead of snapping
        # at the coarse refresh interval that previously caused visible stutter.
        ...
        # Get aimbot settings
        radius = settings.get('aimbot_radius', 50)
        targeting_type = get_targeting_mode_value(settings, "aimbot_targeting_type")
        spotted_check = settings.get('aimbot_visible_check', settings.get('aimbot_spotted_check', False))
        ...
        target_bone_id = get_target_bone_offset(target_bone_name)
        
        persistence_window = 0.35

        cached_target_pos = aimbot_state.get('last_target_pos')
        cached_target_time = aimbot_state.get('last_target_time', 0.0)

        # Prefer the aimbot thread's already-resolved target point so the snapline
        # follows the same screen-space point as the actual aim movement instead of
        # doing a second independent target scan that can jitter.
        if (
            cached_target_pos
            and aimbot_state.get('is_targeting', False)
            and (now - cached_target_time) <= persistence_window
        ):
            target_pos = cached_target_pos
        else:
            # Build target list
            target_list = []
            ...
            try:
                ...
                # Resolve local player index so visibility filtering can require
                # that the local player specifically has spotted the target.
                local_player_index = -1
                if list_entry:
                    for i in range(1, 64):
                        try:
                            ...
                            if entity_pawn == local_player_pawn:
                                local_player_index = i
                                break
                        except:
                            continue
                ...
                for i in range(1, 64):
                    try:
                        ...
                        # Check visibility state if enabled (require local-player spotted bit)
                        if spotted_check:
                            try:
                                if local_player_index <= 0:
                                    continue

                                spotted_mask = pm.read_int(entity_pawn + m_entitySpottedState + m_bSpottedByMask)
                                is_visible_to_local = (spotted_mask & (1 << (local_player_index - 1))) != 0
                            except:
                                is_visible_to_local = False
                            
                            if not is_visible_to_local:
                                continue
                        ...
                        # Convert to screen coordinates using the same projection path
                        # as the aimbot target selection so the visual snapline stays
                        # stable and matches the actual aim point.
                        screen_pos = apply_head_target_bone_offset(
                            target_bone_name,
                            w2s_aimbot(view_matrix, head_x, head_y, head_z, width, height)
                        )
            ...
            if not target_list:
                if cached_target_pos and (now - cached_target_time) <= persistence_window:
                    target_pos = cached_target_pos
                else:
                    return
            else:
                ...
                if not closest_target:
                    ...
                    aimbot_state['last_target_pos'] = target_pos
                    aimbot_state['last_target_time'] = now

        # Smooth the snapline endpoint with a lightweight deadzone so tiny
        # projection noise does not cause visible twitching while still tracking
        # larger target movement almost immediately.
        snapline_state = aimbot_state.setdefault('_snapline_state', {'pos': None, 'last_update': now})
        previous_snapline_pos = snapline_state.get('pos')
        if previous_snapline_pos is None:
            smoothed_target_pos = target_pos
        else:
            prev_x, prev_y = previous_snapline_pos
            target_x, target_y = target_pos
            dx = target_x - prev_x
            dy = target_y - prev_y
            dist = math.hypot(dx, dy)

            # Ignore tiny jitter but remain responsive to real movement.
            if dist < 1.2:
                smoothed_target_pos = previous_snapline_pos
            else:
                alpha = 0.65 if dist < 24.0 else 0.9
                target_x = prev_x + (dx * alpha)
                target_y = prev_y + (dy * alpha)
                smoothed_target_pos = (target_x, target_y)

        snapline_state['pos'] = smoothed_target_pos
        snapline_state['last_update'] = now
        draw_target_pos = (
            max(0, min(width, smoothed_target_pos[0])),
            max(0, min(height, smoothed_target_pos[1]))
        )
        
        # Draw snapline from center to target
        snapline_color = Active_Config.get('aimbot_snapline_color', settings.get('aimbot_snapline_color', (255, 255, 255)))
        snapline_thickness = settings.get('aimbot_snapline_thickness', 2.0)
        
        overlay.draw_line_rgb(
            center_x, center_y,
            draw_target_pos[0], draw_target_pos[1],
            snapline_color,
            snapline_thickness
        )
...
def render_acs_deadzone_lines(overlay, pm, client, settings):
    """Render a simple crosshair placement indicator for the selected target bone."""
    try:
        if not settings or not isinstance(settings, dict):
            return
        ...
        targeting_type = get_targeting_mode_value(settings, "acs_targeting_type")
        ...
        try:
            ...
            for i in range(1, 64):
                try:
                    ...
                    target_bone_name = settings.get("acs_target_bone", settings.get("aimbot_target_bone", "Head"))
                    target_bone_id = get_target_bone_offset(target_bone_name)
                    ...
                    screen_pos = apply_head_target_bone_offset(
                        target_bone_name,
                        w2s_aimbot(view_matrix, bone_x, bone_y, bone_z, int(screen_width), int(screen_height))
                    )
        ...
        line_length = screen_width * (0.02 + line_width_multiplier * 0.33)
...
def _has_enabled_esp_primitives(cfg):
    """Return True when at least one base ESP primitive is enabled."""
    return any([
        cfg.get("box_esp", True),
        cfg.get("line_esp", True),
        cfg.get("skeleton_esp", True),
        cfg.get("name_esp", True),
        cfg.get("weapon_esp", False),
        cfg.get("distance_esp", False),
        cfg.get("flashed_indicator_enabled", False),
        cfg.get("local_flashed_indicator_enabled", False),
        cfg.get("health_bar", True),
        cfg.get("armor_bar", True),
        cfg.get("draw_head_hitbox", False),
    ])


def should_run_esp_overlay(config=None):
    """Return True when any overlay-rendered feature currently requires the ESP overlay thread."""
    cfg = config if isinstance(config, dict) else Active_Config

    aimbot_enabled = cfg.get("aimbot_enabled", False)
    acs_enabled = cfg.get("acs_enabled", False)

    return any([
        cfg.get("esp_enabled", True) and _has_enabled_esp_primitives(cfg),
        cfg.get("radar_enabled", False) and not cfg.get("radar_external_mode", False),
        cfg.get("custom_crosshair", False),
        cfg.get("show_overlay_fps", False),
        cfg.get("show_status_labels", False),
        cfg.get("rcs_visualizer_enabled", False),
        cfg.get("show_spectator_list_overlay", False),
        aimbot_enabled and cfg.get("aimbot_show_radius", True),
        aimbot_enabled and cfg.get("aimbot_show_deadzone", True) and cfg.get("aimbot_deadzone_enabled", False),
        aimbot_enabled and cfg.get("aimbot_snaplines", False),
        acs_enabled and cfg.get("acs_enabled", False),
    ])


def sync_esp_overlay_thread_state():
    """Start or stop the ESP overlay thread based on current overlay feature requirements."""
    if app_state.get("current_window") != "cheat":
        return

    should_run = should_run_esp_overlay(Active_Config)

    if should_run and not esp_overlay["running"]:
        start_esp_overlay()
    elif not should_run and esp_overlay["running"]:
        stop_esp_overlay()


def should_run_external_radar(config=None):
    """Return True when radar should render in the standalone external radar window."""
    cfg = config if isinstance(config, dict) else Active_Config
    return cfg.get("radar_enabled", False) and cfg.get("radar_external_mode", False)


def _rgb_to_tk(color, fallback="#ffffff"):
...
def external_radar_thread():
    """Render radar in a standalone OS window (separate from game overlay)."""
    global radar_external_state

    try:
        import tkinter as tk
    except Exception as e:
        debug_log(f"External radar unavailable (tkinter import failed): {e}", "ERROR")
        radar_external_state["running"] = False
        return
    ...
    def persist_external_radar_position(force_save=False):
        """Persist current external radar window position into config/autosave."""
        try:
            if root is None:
                return
            ...
            changed = (
                Active_Config.get("radar_external_x") != x
                or Active_Config.get("radar_external_y") != y
            )

            Active_Config["radar_external_x"] = x
            Active_Config["radar_external_y"] = y

            if esp_overlay["settings"]:
                esp_overlay["settings"]["radar_external_x"] = x
                esp_overlay["settings"]["radar_external_y"] = y
    ...
    def on_drag_start(event):
        ...
        drag_state["start_mouse_x"] = event.x_root
        drag_state["start_mouse_y"] = event.y_root
        drag_state["start_window_x"] = root.winfo_x()
        drag_state["start_window_y"] = root.winfo_y()

    def on_drag_motion(event):
        if not drag_state["dragging"]:
            return

        delta_x = event.x_root - drag_state["start_mouse_x"]
        delta_y = event.y_root - drag_state["start_mouse_y"]
    ...
    def on_drag_end(_event):
        if drag_state["dragging"]:
            ...
            persist_external_radar_position()

    try:
        root = tk.Tk()
        root.title("Juggware External Radar")
        root.overrideredirect(True)
        root.attributes("-topmost", True)
        root.configure(bg="#101010")
        root.minsize(180, 180)

        canvas = tk.Canvas(root, bg="#101010", highlightthickness=0, bd=0)
        ...
        # Hide from taskbar/Alt+Tab while keeping the radar interactive.
        try:
            hwnd = root.winfo_id()
            radar_external_state["hwnd"] = hwnd
            ex_style = win32gui.GetWindowLong(hwnd, GWL_EXSTYLE)
        ...
        persisted_x = Active_Config.get("radar_external_x")
        persisted_y = Active_Config.get("radar_external_y")
        if isinstance(persisted_x, int) and isinstance(persisted_y, int):
            root.geometry(f"+{persisted_x}+{persisted_y}")

        while radar_external_state["running"]:
            try:
                root.update_idletasks()
                root.update()
            except Exception:
            ...
            if not should_run_external_radar(settings):
                break
            ...
            if pm is None or client is None:
                if now - last_connect_try >= 1.0:
                    ...
                    try:
                        pm = pymem.Pymem("cs2.exe")
                        client = pymem.process.module_from_name(pm.process_handle, "client.dll").lpBaseOfDll
                        debug_log("[RADAR-EXTERNAL] Connected to CS2", "INFO")
                    except Exception:
                        pm = None
                        client = None
            ...
            radar_size = max(100, int(radar_size))
            show_additional_info = settings.get("radar_external_additional_info", False)
            info_font_size = max(6, min(14, int(radar_size / 20)))
            window_size = radar_size + 20
            geometry = f"{window_size}x{window_size}"
            ...
            right = center_x + radar_radius
            bottom = center_y + radar_radius
            ...
            canvas.create_oval(
                ...
                right,
                bottom,
                fill=_rgb_to_tk(radar_bg, "#000000"),
                outline=_rgb_to_tk(radar_border, "#808080"),
                width=2,
            )
            canvas.create_line(left + 5, center_y, right - 5, center_y, fill=_rgb_to_tk(radar_crosshair, "#4d4d4d"), width=1)
            canvas.create_line(center_x, top + 5, center_x, bottom - 5, fill=_rgb_to_tk(radar_crosshair, "#4d4d4d"), width=1)
            canvas.create_oval(center_x - 4, center_y - 4, center_x + 4, center_y + 4, fill=_rgb_to_tk(radar_player, "#ffffff"), outline="")
            ...
            try:
                local_player_pawn_addr = pm.read_longlong(client + dwLocalPlayerPawn)
                if not local_player_pawn_addr:
                    raise RuntimeError("Local player not available")
                ...
                for i in range(1, 64):
                    ...
                    entity_armor = 0
                    try:
                        entity_armor = max(0, pm.read_int(entity_pawn_addr + m_ArmorValue))
                    except Exception:
                        entity_armor = 0

                    entity_weapon_name = "Unknown"
                    try:
                        entity_weapon_name = get_weapon_name_from_pawn(pm, client, entity_pawn_addr)
                    except Exception:
                        entity_weapon_name = "Unknown"
                    ...
                    if entity_team == local_player_team:
                        entity_color = _rgb_to_tk(radar_team, "#00ff00")
                    else:
                        entity_color = _rgb_to_tk(radar_enemy, "#ff0000")

                    height_diff = entity_z - local_z
                    if abs(height_diff) > 50.0:
                        if height_diff > 0:
                            points = [
                                radar_entity_x, radar_entity_y - 4,
                                radar_entity_x - 3, radar_entity_y + 2,
                        ...
                        canvas.create_polygon(points, fill=entity_color, outline=entity_color)
                    else:
                        canvas.create_oval(
                            radar_entity_x - 3,
                            radar_entity_y - 3,
                    ...
                    if show_additional_info:
                        info_x = radar_entity_x + 8
                        weapon_y = radar_entity_y - info_font_size * 0.55
                        stats_y = radar_entity_y + info_font_size * 0.55
                        info_health_text = f"{entity_health}/{entity_armor}"

                        canvas.create_text(
                            info_x + 1,
                            weapon_y + 1,
                            text=entity_weapon_name,
                            fill="#000000",
                            anchor="w",
        ...
        persist_external_radar_position(force_save=True)
        if root is not None:
            try:
                root.destroy()
            except Exception:
                pass
        radar_external_state["running"] = False
        radar_external_state["hwnd"] = None
        debug_log("[RADAR-EXTERNAL] External radar window stopped", "INFO")


def start_external_radar_window():
    """Start the standalone external radar window thread."""
    global radar_external_state

    if radar_external_state["running"]:
        return

    radar_external_state["running"] = True
    radar_external_state["thread"] = threading.Thread(target=external_radar_thread, daemon=True)
    radar_external_state["thread"].start()
    debug_log("[RADAR-EXTERNAL] External radar window started", "SUCCESS")


def stop_external_radar_window():
    """Stop the standalone external radar window thread."""
    global radar_external_state

    if not radar_external_state["running"]:
        return

    radar_external_state["running"] = False
    if radar_external_state["thread"]:
        radar_external_state["thread"].join(timeout=2.0)
        radar_external_state["thread"] = None


def sync_external_radar_window_state():
    """Start or stop external radar window based on current settings and window context."""
    if app_state.get("current_window") != "cheat":
        if radar_external_state["running"]:
            stop_external_radar_window()
        return

    should_run = should_run_external_radar(Active_Config)
    if should_run and not radar_external_state["running"]:
        start_external_radar_window()
    elif not should_run and radar_external_state["running"]:
        stop_external_radar_window()


def should_run_spectator_list_overlay(config=None):
    """Return True when the in-overlay spectator list should be rendered."""
    cfg = config if isinstance(config, dict) else Active_Config
    return bool(cfg.get("show_spectator_list_overlay", False))


def get_spectator_names(pm, client, local_pawn_addr):
    """
    Return the list of player names currently spectating the local player.

    For every controller in the entity list, resolve its observer pawn
    (m_hObserverPawn), read that pawn's observer services, and compare the
    observed target handle against the local player's pawn address. This
    mirrors the reference C++ implementation which walks controllers and
    checks m_hObserverPawn -> m_pObserverServices -> m_hObserverTarget.
    """
    names = []
    if not pm or not client or not local_pawn_addr:
        return names
    if m_hObserverPawn is None or m_pObserverServices is None or m_hObserverTarget is None:
        return names
    ...
    for i in range(1, 64):
        try:
            ...
            player_pawn_handle = pm.read_int(current_controller + m_hPlayerPawn)
            if not player_pawn_handle or player_pawn_handle == 0xFFFFFFFF:
                continue

            player_pawn_addr = get_entity_from_handle(pm, client, player_pawn_handle)
            if not player_pawn_addr or player_pawn_addr == local_pawn_addr:
                continue

            if m_lifeState is not None:
                try:
                    player_life_state = pm.read_int(player_pawn_addr + m_lifeState)
                    if player_life_state == 256:
                        continue
                except Exception:
                    pass

            obs_pawn_handle = pm.read_int(current_controller + m_hObserverPawn)
            if not obs_pawn_handle or obs_pawn_handle == 0xFFFFFFFF:
                continue

            obs_pawn_addr = get_entity_from_handle(pm, client, obs_pawn_handle)
            if not obs_pawn_addr or obs_pawn_addr == local_pawn_addr:
                continue

            if obs_pawn_addr == player_pawn_addr:
                continue

            observer_services = pm.read_longlong(obs_pawn_addr + m_pObserverServices)
            if not observer_services:
                continue

            target_handle = pm.read_int(observer_services + m_hObserverTarget)
            if not target_handle or target_handle == 0xFFFFFFFF:
                continue

            target_pawn_addr = get_entity_from_handle(pm, client, target_handle)
            if target_pawn_addr != local_pawn_addr:
                continue

            player_name = ""
            try:
                if m_sSanitizedPlayerName is not None:
                    player_name = pm.read_string(current_controller + m_sSanitizedPlayerName, 32)
            except Exception:
                player_name = ""
            if not player_name and m_iszPlayerName is not None:
                try:
                    player_name = pm.read_string(current_controller + m_iszPlayerName, 32)
                except Exception:
                    player_name = ""
...
def get_local_player_pawn(pm, client):
    """Return the local player pawn address or 0 if unavailable."""
    if not pm or not client or dwLocalPlayerPawn is None:
        return 0
    try:
        return pm.read_longlong(client + dwLocalPlayerPawn) or 0
    except Exception:
        return 0


def get_current_spectator_names(pm=None, client=None):
    """Return the current spectator names using shared memory if needed."""
    if pm is None or client is None:
        pm, client = get_shared_memory_connection()
    if not pm or not client:
        return []

    local_pawn_addr = get_local_player_pawn(pm, client)
    if not local_pawn_addr:
        return []

    return get_spectator_names(pm, client, local_pawn_addr)


def are_spectators_present(pm=None, client=None):
    """Return True if at least one spectator is currently observing the local player."""
    return bool(get_current_spectator_names(pm, client))


def _update_spectator_protection_manual_state(feature, value):
    """Keep manual desired feature state while protections are active."""
    protection_key = f"spectator_protections_disable_{feature}"
    if Active_Config.get(protection_key, False) and are_spectators_present():
        spectator_protection_state[f"{feature}_manual_state"] = bool(value)


def _apply_spectator_protection(feature, protection_key, active_key, set_fn, has_spectators):
    """Enforce or restore a protected feature based on spectator presence."""
    protection_enabled = Active_Config.get(protection_key, False)
    current_enabled = Active_Config.get(active_key, False)
    manual_key = f"{feature}_manual_state"
    force_key = f"{feature}_force_disabled"
    manual_state = spectator_protection_state.get(manual_key)
    forced_disabled = spectator_protection_state.get(force_key, False)

    if protection_enabled and has_spectators:
        if not forced_disabled:
            spectator_protection_state[manual_key] = current_enabled
        if current_enabled:
            set_fn(False, save=False)
        spectator_protection_state[force_key] = True
        return

    if forced_disabled:
        if manual_state:
            set_fn(True, save=False)
        else:
            set_fn(False, save=False)
        spectator_protection_state[force_key] = False
        spectator_protection_state[manual_key] = None


def update_spectator_protections(pm=None, client=None):
    """Update spectator protection enforcement based on current spectator state."""
    if pm is None or client is None:
        pm, client = get_shared_memory_connection()
    if not pm or not client:
        return

    has_spectators = are_spectators_present(pm, client)
    _apply_spectator_protection(
        feature="anti_flash",
        protection_key="spectator_protections_disable_anti_flash",
        active_key="anti_flash_enabled",
        set_fn=_set_anti_flash_enabled,
        has_spectators=has_spectators,
    )
    _apply_spectator_protection(
        feature="aimbot",
        protection_key="spectator_protections_disable_aimbot",
        active_key="aimbot_enabled",
        set_fn=_set_aimbot_enabled,
        has_spectators=has_spectators,
    )
    _apply_spectator_protection(
        feature="rcs",
        protection_key="spectator_protections_disable_rcs",
        active_key="rcs_enabled",
        set_fn=_set_rcs_enabled,
        has_spectators=has_spectators,
    )


def _set_anti_flash_enabled(enabled, save=False):
    if anti_flash_state.get("settings"):
        anti_flash_state["settings"]["anti_flash_enabled"] = enabled
    Active_Config["anti_flash_enabled"] = enabled
    if enabled:
        anti_flash_state["last_enabled"] = False
        start_anti_flash_thread()
    else:
        restore_flash_alpha()
        stop_anti_flash_thread()
    if dpg.does_item_exist("chk_anti_flash_enabled"):
        try:
            dpg.set_value("chk_anti_flash_enabled", enabled)
        except Exception:
            pass
    if save:
        save_settings()
    debug_log(f"Anti-Flash {'enabled' if enabled else 'disabled'}", "INFO")


def _set_aimbot_enabled(enabled, save=False):
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_enabled"] = enabled
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_enabled"] = enabled
    Active_Config["aimbot_enabled"] = enabled
    if enabled:
        start_aimbot_thread()
    else:
        stop_aimbot_thread()
    if dpg.does_item_exist("chk_aimbot_enabled"):
        try:
            dpg.set_value("chk_aimbot_enabled", enabled)
        except Exception:
            pass
    if save:
        save_settings()
    debug_log(f"Aimbot {'enabled' if enabled else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()


def _set_rcs_enabled(enabled, save=False):
    Active_Config["rcs_enabled"] = enabled
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_enabled"] = enabled
    if enabled:
        start_rcs_thread()
    else:
        stop_rcs_thread()
    if dpg.does_item_exist("chk_rcs_enabled"):
        try:
            dpg.set_value("chk_rcs_enabled", enabled)
        except Exception:
            pass
    if save:
        save_settings()
    debug_log(f"RCS {'enabled' if enabled else 'disabled'}", "INFO")


def persist_spectator_list_position(force_save=False, x=None, y=None):
    """Persist the spectator list panel position into config/autosave."""
    try:
        if x is None:
            x = spectator_list_state.get("panel_x")
        if y is None:
            y = spectator_list_state.get("panel_y")
        ...
        changed = (
            Active_Config.get("spectator_list_x") != x
            or Active_Config.get("spectator_list_y") != y
        )

        Active_Config["spectator_list_x"] = x
        Active_Config["spectator_list_y"] = y

        if esp_overlay.get("settings"):
            esp_overlay["settings"]["spectator_list_x"] = x
            esp_overlay["settings"]["spectator_list_y"] = y
...
def _get_spectator_panel_theme(settings=None):
    """Return overlay-friendly colors and font settings for the spectator list panel."""
    cfg = dict(Active_Config)
    if isinstance(settings, dict):
        cfg.update(settings)

    colorway_name = cfg.get("menu_colorway", "Default") or "Default"
    use_rainbow = bool(rainbow_active) or str(colorway_name).strip().lower() == "rainbow"
    ...
    if use_rainbow:
        hue = rainbow_hue if isinstance(rainbow_hue, (int, float)) else 0.0
        accent = _hsv_to_rgb_tuple(hue, 0.8, 0.7)
        header = _hsv_to_rgb_tuple(hue, 0.9, 0.55)
        border = _hsv_to_rgb_tuple(hue, 0.95, 0.85)
        return {
            "bg": (10, 10, 10),
            "header": header,
            "text": (255, 255, 255),
            "border": border,
            "font_name": cfg.get("menu_font", "Default") or "Default",
            "accent": accent,
        }

    colorway = UI_COLORWAYS.get(colorway_name, UI_COLORWAYS.get("Default", {}))
    return {
        "bg": _coerce_rgb(colorway.get("window_bg", (12, 12, 12)), (12, 12, 12)),
        "header": _coerce_rgb(colorway.get("title_bg_active", (40, 40, 40)), (40, 40, 40)),
        "text": _coerce_rgb(colorway.get("text", (255, 255, 255)), (255, 255, 255)),
        "border": _coerce_rgb(colorway.get("button", (80, 80, 80)), (80, 80, 80)),
        "font_name": cfg.get("menu_font", "Default") or "Default",
        "accent": _coerce_rgb(colorway.get("button", (80, 80, 80)), (80, 80, 80)),
    }


def render_spectator_list_overlay(overlay, pm, client, settings=None):
    """Render the spectator list panel inside the CS2 overlay."""
    cfg = settings if isinstance(settings, dict) else Active_Config
    if not spectator_list_state.get("running", False):
        return
    if not should_run_spectator_list_overlay(cfg):
        return

    state = spectator_list_state
    panel_x = state.get("panel_x")
    panel_y = state.get("panel_y")
    if panel_x is None or panel_y is None:
        panel_x = cfg.get("spectator_list_x")
        panel_y = cfg.get("spectator_list_y")
    if panel_x is None:
        panel_x = 40
    if panel_y is None:
        panel_y = 220
    ...
    panel_x = int(panel_x)
    panel_y = int(panel_y)
    theme = _get_spectator_panel_theme(cfg)

    now = time.time()
    if (pm is None or client is None) and now - state.get("last_connect_try", 0.0) >= 1.0:
        state["last_connect_try"] = now
        try:
            pm = pymem.Pymem("cs2.exe")
            client = pymem.process.module_from_name(pm.process_handle, "client.dll").lpBaseOfDll
            state["pm"] = pm
            state["client"] = client
            debug_log("[SPECTATOR-LIST] Connected to CS2", "INFO")
        except Exception:
            state["pm"] = None
            state["client"] = None
            pm = None
    ...
    if now - state.get("last_refresh_time", 0.0) >= 0.5:
        state["last_refresh_time"] = now
        new_names = []
        if pm is not None and client is not None and dwLocalPlayerPawn is not None:
            try:
                local_pawn_addr = pm.read_longlong(client + dwLocalPlayerPawn)
                if local_pawn_addr:
                    new_names = get_spectator_names(pm, client, local_pawn_addr)
            except Exception:
                state["pm"] = None
                state["client"] = None
                pm = None
                client = None
                new_names = []
        state["current_names"] = new_names

    names = state.get("current_names", []) or []

    header_height = 28
    row_height = 18
    max_rows = 14
    visible_names = names[:max_rows]
    title = f"SPECTATORS: {len(names)}" if names else "SPECTATORS: 0"
    title_size = 20.0
    min_width = 180
    padding = 16
    # Compute panel width dynamically to avoid unnecessarily wide spectator list.
    longest_name_width = 0.0
    for name in visible_names:
        text_width = overlay.calc_text_width(f"- {name}", size=17.0, font_name=theme["font_name"])
        longest_name_width = max(longest_name_width, text_width)
    title_width = overlay.calc_text_width(title, size=title_size, font_name=theme["font_name"])
    panel_width = int(max(min_width, longest_name_width + padding, title_width + padding))
    panel_height = max(70, header_height + 12 + len(visible_names) * row_height + 12)

    screen_width = max(1, int(getattr(overlay, "width", 0) or 1920))
    screen_height = max(1, int(getattr(overlay, "height", 0) or 1080))
    panel_x = max(5, min(panel_x, screen_width - panel_width - 5))
    panel_y = max(5, min(panel_y, screen_height - panel_height - 5))
    state["panel_x"] = panel_x
    state["panel_y"] = panel_y

    # Dragging support using Win32 cursor position while the mouse is held over the title bar.
    cursor_x, cursor_y = win32api.GetCursorPos()
    is_mouse_down = bool(win32api.GetAsyncKeyState(win32con.VK_LBUTTON) & 0x8000)
    mouse_was_down = bool(state.get("mouse_was_down", False))

    if state.get("dragging", False):
        if not is_mouse_down:
            state["dragging"] = False
            persist_spectator_list_position(force_save=False, x=panel_x, y=panel_y)
        else:
            panel_x = max(5, min(state.get("drag_start_panel_x", panel_x) + (cursor_x - state.get("drag_start_mouse_x", cursor_x)), screen_width - panel_width - 5))
            panel_y = max(5, min(state.get("drag_start_panel_y", panel_y) + (cursor_y - state.get("drag_start_mouse_y", cursor_y)), screen_height - panel_height - 5))
            state["panel_x"] = panel_x
            state["panel_y"] = panel_y
            Active_Config["spectator_list_x"] = panel_x
            Active_Config["spectator_list_y"] = panel_y
    elif is_mouse_down and not mouse_was_down and panel_x <= cursor_x <= panel_x + panel_width and panel_y <= cursor_y <= panel_y + header_height:
        state["dragging"] = True
        state["drag_start_mouse_x"] = cursor_x
        state["drag_start_mouse_y"] = cursor_y
        state["drag_start_panel_x"] = panel_x
        state["drag_start_panel_y"] = panel_y

    state["mouse_was_down"] = is_mouse_down

    def _blend_rgb(base, accent, amount):
        amount = max(0.0, min(1.0, amount))
        return tuple(int(base[i] + (accent[i] - base[i]) * amount) for i in range(3))

    def draw_panel_text(x, y, text, size, color=None, shadow=True):
        if shadow:
            overlay.draw_text(x + 1, y + 1, text, 0, 0, 0, size=size, stroke=False, font_name=theme["font_name"])
        overlay.draw_text(x, y, text, *(color or theme["text"]), size=size, stroke=False, font_name=theme["font_name"])

    shadow_color = tuple(max(0, min(255, channel - 28)) for channel in theme["bg"])
    overlay.draw_filled_rect(panel_x + 4, panel_y + 4, panel_width, panel_height, shadow_color[0], shadow_color[1], shadow_color[2])
    overlay.draw_filled_rect(panel_x, panel_y, panel_width, panel_height, theme["bg"][0], theme["bg"][1], theme["bg"][2])
    overlay.draw_filled_rect(panel_x + 2, panel_y + 2, panel_width - 4, panel_height - 4, *_blend_rgb(theme["bg"], theme["header"], 0.05))
    overlay.draw_filled_rect(panel_x, panel_y, panel_width, header_height, theme["header"][0], theme["header"][1], theme["header"][2])
    overlay.draw_filled_rect(panel_x + 1, panel_y + header_height - 2, panel_width - 2, 2, theme["accent"][0], theme["accent"][1], theme["accent"][2])
    overlay.draw_rect_rgb(panel_x, panel_y, panel_width, panel_height, theme["border"], thickness=1.0)
    overlay.draw_rect_rgb(panel_x + 1, panel_y + 1, panel_width - 2, panel_height - 2, _blend_rgb(theme["border"], theme["bg"], 0.35), thickness=1.0)

    title = f"SPECTATORS: {len(names)}" if names else "SPECTATORS: 0"
    title_size = 20.0
    title_width = overlay.calc_text_width(title, size=title_size, font_name=theme["font_name"])
    title_x = panel_x + max(8, (panel_width - title_width) // 2)
    title_y = panel_y + (header_height - title_size) // 2 - 1
    draw_panel_text(title_x, title_y, title, title_size, color=_blend_rgb(theme["text"], theme["accent"], 0.18), shadow=True)

    if not visible_names:
        empty_text = "No Spectators"
        empty_text_size = 17.0
        empty_text_width = overlay.calc_text_width(empty_text, size=empty_text_size, font_name=theme["font_name"])
        empty_text_x = panel_x + max(12, (panel_width - empty_text_width) // 2)
        empty_text_y = panel_y + header_height + 6 + (row_height - empty_text_size) // 2 - 1
        draw_panel_text(empty_text_x, empty_text_y, empty_text, empty_text_size, color=theme["text"], shadow=False)
        return

    row_text_size = 17.0
    for idx, name in enumerate(visible_names):
        row_y = panel_y + header_height + 6 + idx * row_height
        row_center_y = row_y + (row_height - row_text_size) // 2 - 1
        text_width = overlay.calc_text_width(name, size=row_text_size, font_name=theme["font_name"])
        text_x = panel_x + max(12, (panel_width - text_width) // 2)
        draw_panel_text(text_x, row_center_y, f"{name}", row_text_size, color=theme["text"], shadow=False)


def start_spectator_list_thread():
    """Enable spectator list rendering inside the overlay."""
    global spectator_list_state

    if spectator_list_state.get("running", False):
        return

    spectator_list_state["running"] = True
    spectator_list_state["thread"] = None
    spectator_list_state["current_names"] = []
    spectator_list_state["last_refresh_time"] = 0.0
    spectator_list_state["last_connect_try"] = 0.0
    spectator_list_state["pm"] = None
    spectator_list_state["client"] = None
    spectator_list_state["dragging"] = False
    spectator_list_state["mouse_was_down"] = False
    spectator_list_state["drag_start_mouse_x"] = 0
    spectator_list_state["drag_start_mouse_y"] = 0
    spectator_list_state["drag_start_panel_x"] = 0
    spectator_list_state["drag_start_panel_y"] = 0
    spectator_list_state["panel_x"] = Active_Config.get("spectator_list_x", 40)
    spectator_list_state["panel_y"] = Active_Config.get("spectator_list_y", 220)
    debug_log("[SPECTATOR-LIST] Spectator list overlay enabled", "SUCCESS")


def stop_spectator_list_thread():
    """Disable spectator list rendering inside the overlay."""
    global spectator_list_state

    if not spectator_list_state.get("running", False):
        return

    spectator_list_state["running"] = False
    spectator_list_state["thread"] = None
    spectator_list_state["dragging"] = False


def sync_spectator_list_thread_state():
    """Start or stop the spectator list overlay based on current settings and window context."""
    if app_state.get("current_window") != "cheat":
        if spectator_list_state.get("running", False):
            stop_spectator_list_thread()
        return

    should_run = should_run_spectator_list_overlay(Active_Config)
    if should_run and not spectator_list_state.get("running", False):
        start_spectator_list_thread()
    elif not should_run and spectator_list_state.get("running", False):
        stop_spectator_list_thread()
...
def esp_overlay_thread():
    ...
    pm, client = wait_for_shared_memory_connection(max_wait_seconds=10)

    if not pm or not client:
        debug_log("[ESP] Failed to get shared memory connection", "ERROR")
        esp_overlay["running"] = False
        return

    debug_log("[ESP] Using shared memory connection", "INFO")
    ...
    while esp_overlay["running"] and overlay.running:
        try:
            ...
            # Always fetch latest shared connection in case of reconnect.
            pm, client = get_shared_memory_connection()
            if not pm or not client:
                time.sleep(0.05)
                continue

            # Stop thread when no overlay-rendered feature is active.
            if check_counter % 10 == 0 and not should_run_esp_overlay(Active_Config):
                debug_log("[ESP] No active overlay features, stopping overlay", "INFO")
                break
            ...
            if check_counter % 10 == 0:
                ...
                if cs2_in_foreground:
                    # Treat the cheat window as active focus so tabbing into it does not count as tabbing out.
            ...
            # Render local-player flash state independently of player ESP.
            render_local_flashed_indicator(overlay, pm, client, settings)
            ...
            render_custom_crosshair(overlay, pm, client, settings)
            
            # Render Recoil Visualizer (independent of main ESP toggle)
            render_rcs_visualizer(overlay, pm, client, settings)
            ...
            # Render spectator list inside the overlay
            render_spectator_list_overlay(overlay, pm, client, settings)
            ...
            if Active_Config.get("show_status_labels", True):
                ...
                esp_keybind = (Keybinds_Config.get("esp_toggle_key", "") or "").upper()
                esp_display_key = esp_keybind if esp_keybind and esp_keybind != "NONE" else "NONE"
                esp_label = f"({esp_display_key}) ESP: "
                ...
                # Aimbot status
                aimbot_keybind = (Keybinds_Config.get("aimbot_toggle_key", "") or "").upper()
                aimbot_display_key = aimbot_keybind if aimbot_keybind and aimbot_keybind != "NONE" else "NONE"
                aimbot_label = f"({aimbot_display_key}) Aimbot: "
                ...
                rcs_label = "RCS: "
                rcs_status_x = 5 + len(rcs_label) * 7 + 8
                ...
                # Bhop status
                bhop_status = "ON" if Active_Config.get("bhop_enabled", False) else "OFF"
                bhop_label = "Bhop: "
                bhop_status_x = 5 + len(bhop_label) * 7 + 8
                overlay.draw_text(5, 70, bhop_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if bhop_status == "ON" else (255, 0, 0)
                overlay.draw_text(bhop_status_x, 70, bhop_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
                
                # Anti-AFK status
                anti_afk_keybind = Keybinds_Config.get("anti_afk_toggle_key", "").upper() or "NONE"
                anti_afk_label = f"({anti_afk_keybind}) Anti-AFK: "
                anti_afk_status_x = 5 + len(anti_afk_label) * 7 + 8  # Estimate 7px per char + 8px padding
                overlay.draw_text(5, 85, anti_afk_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if anti_afk_status == "ON" else (255, 0, 0)
                overlay.draw_text(anti_afk_status_x, 85, anti_afk_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
                ...
                overlay.draw_text(5, 100, triggerbot_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if triggerbot_status == "ON" else (255, 0, 0)
                overlay.draw_text(triggerbot_status_x, 100, triggerbot_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)

                # Anti-Flash status
                anti_flash_status = "ON" if Active_Config.get("anti_flash_enabled", False) else "OFF"
                anti_flash_label = "Anti-Flash: "
                anti_flash_status_x = 5 + len(anti_flash_label) * 7 + 8
                overlay.draw_text(5, 115, anti_flash_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if anti_flash_status == "ON" else (255, 0, 0)
                overlay.draw_text(anti_flash_status_x, 115, anti_flash_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
    ...
    # Cleanup
    debug_log("[OVERLAY] Cleaning up overlay...", "INFO")
    overlay.destroy()
    esp_overlay["hwnd"] = None
    debug_log("[OVERLAY] Overlay thread stopped", "INFO")
...
def start_esp_overlay():
    ...
    if not GLFW_AVAILABLE:
        error_msg = (
            "Overlay failed to initialize: GLFW runtime is missing. "
            f"Import error: {GLFW_IMPORT_ERROR}"
        )
        debug_log(error_msg, "ERROR")
        try:
            ctypes.windll.user32.MessageBoxW(
                0,
                "GLFW runtime is missing in this build.\n"
                "Rebuild with glfw3.dll included in auto-py-to-exe.\n\n"
                f"Details: {GLFW_IMPORT_ERROR}",
                "Overlay Initialization Error",
                0x10,
            )
        except Exception:
            pass
    ...
    if esp_overlay["running"]:
        debug_log("Overlay already running", "WARNING")
        return
    
    debug_log("Starting overlay thread...", "INFO")
    ...
    # Start thread
    esp_overlay["thread"] = threading.Thread(target=esp_overlay_thread, name="OverlayThread", daemon=True)
    esp_overlay["thread"].start()
    
    debug_log("Overlay thread started", "SUCCESS")
...
def stop_esp_overlay():
    ...
    debug_log("Stopping overlay...", "INFO")
    esp_overlay["running"] = False
    ...
    debug_log("Overlay stopped", "SUCCESS")
...
def w2s_aimbot(view_matrix, x, y, z, width, height):
    ...
    # Clamp projected points to the viewport so snaplines stay visible
    # instead of disappearing when a target briefly exits the frame.
    if screen_x < 0:
        screen_x = 0
    elif screen_x > width:
        screen_x = width
...
def get_local_weapon_name(pm, client):
    ...
    try:
        ...
        return get_weapon_name_from_pawn(pm, client, local_player_pawn)
    except Exception:
        return "Unknown"


def get_weapon_name_from_pawn(pm, client, player_pawn):
    """Read a player's active weapon name from game memory via pawn pointer."""
    try:
        required_offsets = [
            m_pWeaponServices,
            m_hActiveWeapon,
        ...
        weapon_services = pm.read_longlong(player_pawn + m_pWeaponServices)
...
HEAD_HITBOX_RING_SEGMENTS = 18

_sphere_ring_angles_cache = {}
...
def _get_sphere_ring_angles(segments):
    if segments in _sphere_ring_angles_cache:
        return _sphere_ring_angles_cache[segments]

    two_pi = 2.0 * math.pi
    angles = [
        (math.cos(two_pi * i / segments), math.sin(two_pi * i / segments))
        for i in range(segments)
    ]
    _sphere_ring_angles_cache[segments] = angles
    return angles
...
def generate_projected_sphere_rings(view_matrix, width, height, center_xyz, world_radius, segments=24):
    ...
    rings = []
    angles = _get_sphere_ring_angles(segments)

    for plane in ['xy', 'xz', 'yz']:
        ring = []
        for cos_theta, sin_theta in angles:
            if plane == 'xy':
                x = center_xyz[0] + world_radius * cos_theta
                y = center_xyz[1] + world_radius * sin_theta
                z = center_xyz[2]
            elif plane == 'xz':
                x = center_xyz[0] + world_radius * cos_theta
                y = center_xyz[1]
                z = center_xyz[2] + world_radius * sin_theta
            else:  # yz
                x = center_xyz[0]
                y = center_xyz[1] + world_radius * cos_theta
                z = center_xyz[2] + world_radius * sin_theta
        ...
        if ring:
            # Close the ring explicitly rather than generating one extra point
            ring.append(ring[0])
            rings.append(ring)
...
def _cubic_bezier_point(p0, p1, p2, p3, t):
    """Sample a cubic bezier curve at t."""
    inv_t = 1.0 - t
...
def _plan_humanized_aimbot_curve(dx, dy, human_state, human_strength):
    """Build a short bezier segment toward the current target with controlled bias and curvature."""
    ...
    distance_ratio = min(1.0, dist / 175.0)
    lock_ratio = min(1.0, human_state.get("target_locked_frames", 0) / 18.0)
    randomness = 0.55 + (0.45 * human_strength)

    step_ratio = 0.18 + (0.24 * distance_ratio) + (0.10 * (1.0 - human_strength))
    step_ratio *= 1.0 - (0.18 * lock_ratio)
    step_distance = max(0.65, min(dist * step_ratio, 15.0 - (2.5 * human_strength)))

    if dist < 8.0:
        flick_low = 0.92 - (0.05 * human_strength)
        flick_high = 1.02 + (0.03 * human_strength)
    else:
        flick_low = 0.86 - (0.08 * human_strength)
        flick_high = 1.09 + (0.10 * human_strength)

    flick_scale = random.uniform(flick_low, flick_high)
    end_distance = min(dist * (1.10 + (0.08 * human_strength)), max(0.45, step_distance * flick_scale))

    lateral_strength = step_distance * (0.10 + (0.22 * randomness)) * (0.30 + (0.70 * distance_ratio))
    lateral_strength *= random.uniform(-1.0, 1.0)

    carry_x = human_state.get("carry_x", 0.0)
    carry_y = human_state.get("carry_y", 0.0)
    carry_scale = 0.18 + (0.16 * (1.0 - human_strength))
    carry_x *= carry_scale
    carry_y *= carry_scale

    p0 = (0.0, 0.0)
    p1 = (
        dir_x * step_distance * random.uniform(0.18, 0.38) + perp_x * lateral_strength * random.uniform(0.85, 1.15) + carry_x,
        dir_y * step_distance * random.uniform(0.18, 0.38) + perp_y * lateral_strength * random.uniform(0.85, 1.15) + carry_y,
    )
    p2 = (
        dir_x * step_distance * random.uniform(0.58, 0.88) - perp_x * lateral_strength * random.uniform(0.15, 0.60),
        dir_y * step_distance * random.uniform(0.58, 0.88) - perp_y * lateral_strength * random.uniform(0.15, 0.60),
    )
    p3 = (
        dir_x * end_distance + perp_x * lateral_strength * random.uniform(-0.18, 0.18),
        dir_y * end_distance + perp_y * lateral_strength * random.uniform(-0.18, 0.18),
    )
    ...
    if dist > 45.0:
        steps_total += 1
    if human_strength > 0.60:
        steps_total += 1
    steps_total = min(6, steps_total)

    human_state["curve_step"] = 0
    human_state["curve_steps_total"] = steps_total
    human_state["curve_p0"] = p0
    human_state["curve_p1"] = p1
    human_state["curve_p2"] = p2
    human_state["curve_p3"] = p3
    human_state["curve_last_point"] = p0
    human_state["curve_target_dx"] = dx
    human_state["curve_target_dy"] = dy
    human_state["curve_bias_scale"] = flick_scale


def compute_humanized_aimbot_movement(dx, dy, aimbot_state):
    """Drive humanized aim movement with short bezier segments, bias, jitter, and dynamic pacing."""
    human_state = aimbot_state.setdefault("humanization", build_default_aimbot_humanization_state())
    for key, default_value in build_default_aimbot_humanization_state().items():
        human_state.setdefault(key, default_value)

    settings = aimbot_state.get("settings") or {}
    strength = max(0.0, min(100.0, settings.get("aimbot_humanization_strength", 75.0)))
    human_strength = strength / 100.0
    dist = math.hypot(dx, dy)

    if dist <= 0.001:
        human_state["velocity_x"] *= 0.35
        human_state["velocity_y"] *= 0.35
        human_state["carry_x"] *= 0.45
        human_state["carry_y"] *= 0.45
        human_state["curve_steps_total"] = 0
        human_state["curve_step"] = 0
        human_state["curve_last_point"] = (0.0, 0.0)
        human_state["target_locked_frames"] = 0
        human_state["last_output_x"] = 0.0
        human_state["last_output_y"] = 0.0
        return 0.0, 0.0

    human_state["frame_counter"] += 1
    if dist < 10.0:
        human_state["target_locked_frames"] += 1
    else:
        human_state["target_locked_frames"] = max(0, human_state["target_locked_frames"] - 2)

    prev_dx = human_state.get("last_dx", dx)
    prev_dy = human_state.get("last_dy", dy)
    prev_dist = math.hypot(prev_dx, prev_dy)
    direction_changed = False
    if prev_dist > 0.001:
        direction_dot = max(-1.0, min(1.0, ((dx * prev_dx) + (dy * prev_dy)) / (dist * prev_dist)))
        direction_changed = direction_dot < 0.78

    curve_steps_total = human_state.get("curve_steps_total", 0)
    curve_step = human_state.get("curve_step", 0)
    curve_target_dx = human_state.get("curve_target_dx", 0.0)
    curve_target_dy = human_state.get("curve_target_dy", 0.0)
    curve_target_dist = math.hypot(curve_target_dx, curve_target_dy)

    replan_needed = curve_steps_total <= 0 or curve_step >= curve_steps_total
    if not replan_needed and curve_target_dist > 0.001:
        distance_ratio = dist / curve_target_dist
        replan_needed = distance_ratio < 0.48 or distance_ratio > 1.55 or direction_changed

    if replan_needed:
        _plan_humanized_aimbot_curve(dx, dy, human_state, human_strength)
        curve_steps_total = human_state["curve_steps_total"]
        curve_step = human_state["curve_step"]

    curve_step = min(curve_steps_total, curve_step + 1)
    human_state["curve_step"] = curve_step
    curve_t = curve_step / max(1, curve_steps_total)
    curve_point = _cubic_bezier_point(
        human_state["curve_p0"],
        human_state["curve_p1"],
        human_state["curve_p2"],
        human_state["curve_p3"],
        curve_t,
    )
    last_point = human_state.get("curve_last_point", (0.0, 0.0))
    curve_move_x = curve_point[0] - last_point[0]
    curve_move_y = curve_point[1] - last_point[1]
    human_state["curve_last_point"] = curve_point
    ...
    human_state["micro_phase"] += 0.55 + random.uniform(0.05, 0.30)
    human_state["coarse_phase"] += 0.18 + random.uniform(0.03, 0.12)
    jitter_wave = (math.sin(human_state["micro_phase"]) * 0.72) + (math.cos(human_state["coarse_phase"]) * 0.28)
    jitter_amount = (0.025 + (0.32 * human_strength)) * (0.30 + (0.70 * min(1.0, dist / 36.0)))
    jitter_amount *= 0.55 + (0.45 * random.random())
    micro_jitter_x = (perp_x * jitter_wave * jitter_amount) + random.uniform(-0.20, 0.20) * jitter_amount
    micro_jitter_y = (perp_y * jitter_wave * jitter_amount) + random.uniform(-0.20, 0.20) * jitter_amount

    if dist < 6.0:
        micro_jitter_x *= 0.55
        micro_jitter_y *= 0.55

    move_x = curve_move_x + micro_jitter_x
    move_y = curve_move_y + micro_jitter_y

    damping = 0.22 + (0.16 * (1.0 - human_strength))
    human_state["velocity_x"] += (move_x - human_state.get("velocity_x", 0.0)) * damping
    human_state["velocity_y"] += (move_y - human_state.get("velocity_y", 0.0)) * damping

    carry_blend = 0.42 + (0.20 * human_strength)
    move_x = (move_x * (1.0 - carry_blend)) + (human_state["velocity_x"] * carry_blend)
    move_y = (move_y * (1.0 - carry_blend)) + (human_state["velocity_y"] * carry_blend)

    human_state["carry_x"] = (human_state.get("carry_x", 0.0) * 0.45) + (move_x * 0.55)
    human_state["carry_y"] = (human_state.get("carry_y", 0.0) * 0.45) + (move_y * 0.55)

    dynamic_speed = 0.85 + (dist * (0.10 + (0.06 * (1.0 - human_strength))))
    dynamic_speed = min(13.5 - (2.5 * human_strength), dynamic_speed)
    move_mag = math.hypot(move_x, move_y)
    if move_mag > dynamic_speed and move_mag > 0.001:
        speed_scale = dynamic_speed / move_mag
        move_x *= speed_scale
        move_y *= speed_scale

    if dist > 22.0:
        min_step = 0.40 + (0.75 * (1.0 - human_strength))
        if abs(move_x) < min_step and abs(dx) > min_step:
            move_x = math.copysign(min_step, dx)
        if abs(move_y) < min_step and abs(dy) > min_step:
            move_y = math.copysign(min_step, dy)
    elif dist < 4.5:
        settle_scale = 0.42 + (0.22 * (1.0 - human_strength))
        move_x *= settle_scale
        move_y *= settle_scale

    overshoot_margin = 1.10 + (0.12 * human_strength)
    if abs(move_x) > (abs(dx) * overshoot_margin) and abs(dx) > 0.001:
        move_x = math.copysign(abs(dx) * overshoot_margin, move_x)
    if abs(move_y) > (abs(dy) * overshoot_margin) and abs(dy) > 0.001:
        move_y = math.copysign(abs(dy) * overshoot_margin, move_y)

    human_state["velocity_x"] = move_x
    human_state["velocity_y"] = move_y
    human_state["last_distance"] = dist
    human_state["last_dx"] = dx
    human_state["last_dy"] = dy
    human_state["last_output_x"] = move_x
    human_state["last_output_y"] = move_y
...
def aimbot_thread():
    ...
    pm, client = wait_for_feature_memory_connection("AIMBOT", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[AIMBOT] Connected to CS2 via shared memory", "INFO")
    
    while aimbot_state["running"]:
        try:
            now = time.perf_counter()
            last_scan_time = aimbot_state.get("last_scan_time", 0.0)
            scan_interval = 0.004 if aimbot_state.get("is_targeting", False) else 0.008
            if now - last_scan_time < scan_interval:
                time.sleep(0.001)
                continue
            aimbot_state["last_scan_time"] = now
            ...
            if require_key:
                ...
                if not key_held:
                    ...
                    aimbot_state["smoothed_move_x"] = 0.0
                    aimbot_state["smoothed_move_y"] = 0.0
                    aimbot_state["residual_x"] = 0.0
                    aimbot_state["residual_y"] = 0.0
                    aimbot_state["is_targeting"] = False
                    aimbot_state["humanization"] = build_default_aimbot_humanization_state()
                    time.sleep(0.002)
                    continue
            else:
                # If require-key is off, do not target while the config UI is visible
                if app_state.get("cheat_window_visible", False):
                    aimbot_state["locked_entity"] = None
                    aimbot_state["accum_x"] = 0.0
                    aimbot_state["accum_y"] = 0.0
                    aimbot_state["smoothed_move_x"] = 0.0
            ...
            # Get pm and client from shared memory
            pm, client = get_shared_memory_connection()
            ...
            targeting_type = get_targeting_mode_value(settings, "aimbot_targeting_type")  # 0 = enemies only
            ...
            try:
                ...
                for i in range(1, 64):
                    try:
                        ...
                        # Check visibility state if enabled (require local-player spotted bit)
                        if settings.get("aimbot_visible_check", settings.get("aimbot_spotted_check", False)):
                            try:
                                if local_player_index <= 0:
                                    continue
                        ...
                        # Keep this mapping in sync with render_aimbot_snaplines so
                        # visual snaplines and actual aimbot movement use identical points.
                        target_bone_name = settings.get("aimbot_target_bone", "Head")
                        target_bone_id = get_target_bone_offset(target_bone_name)
                        ...
                        screen_pos = apply_head_target_bone_offset(
                            target_bone_name,
                            w2s_aimbot(view_matrix, head_x, head_y, head_z, screen_width, screen_height)
                        )
            ...
            aimbot_state["last_target_pos"] = (target_x, target_y)
            aimbot_state["last_target_time"] = time.perf_counter()
            dx = target_x - center_x
            dy = target_y - center_y

            adjust_x = settings.get("aimbot_adjust_x_axis", True)
            adjust_y = settings.get("aimbot_adjust_y_axis", True)
            if not adjust_x:
                dx = 0.0
            if not adjust_y:
                dy = 0.0
            ...
            use_humanization = settings.get("aimbot_use_humanization", False)
            
            # Apply smoothing based on movement type, unless humanization is enabled.
            if use_humanization:
                frac_x, frac_y = compute_humanized_aimbot_movement(dx, dy, aimbot_state)
                ...
                if smoothness == 0:
                    ...
                    dist = math.sqrt(dx * dx + dy * dy)
                    if dist > 0:
                        speed = 10.0 / max(smoothness, 1.0)
                        target_move_x = (dx / dist) * speed
                        target_move_y = (dy / dist) * speed
                    else:
                        target_move_x = 0.0
                        target_move_y = 0.0
                else:
                    target_move_x = dx / max(smoothness, 1.0)
                    target_move_y = dy / max(smoothness, 1.0)

                if smoothness == 0:
                    aimbot_state["smoothed_move_x"] = target_move_x
                    aimbot_state["smoothed_move_y"] = target_move_y
                else:
                    strength = max(0.0, min(1.0, float(smoothness) / 100.0))
                    alpha = max(0.05, 1.0 - (strength * 0.95))
                    aimbot_state["smoothed_move_x"] += (target_move_x - aimbot_state["smoothed_move_x"]) * alpha
                    aimbot_state["smoothed_move_y"] += (target_move_y - aimbot_state["smoothed_move_y"]) * alpha

                if dx != 0 and abs(aimbot_state["smoothed_move_x"]) > abs(dx):
                    aimbot_state["smoothed_move_x"] = dx
                if dy != 0 and abs(aimbot_state["smoothed_move_y"]) > abs(dy):
                    aimbot_state["smoothed_move_y"] = dy

                move_x = aimbot_state["smoothed_move_x"] + aimbot_state["residual_x"]
                move_y = aimbot_state["smoothed_move_y"] + aimbot_state["residual_y"]
                int_x = int(round(move_x))
                int_y = int(round(move_y))
                aimbot_state["residual_x"] = move_x - int_x
                aimbot_state["residual_y"] = move_y - int_y
                move_x = int_x
                move_y = int_y
...
def start_aimbot_thread():
    ...
    aimbot_state["residual_y"] = 0.0
    aimbot_state["is_targeting"] = False
    aimbot_state["last_scan_time"] = 0.0
    aimbot_state["humanization"] = build_default_aimbot_humanization_state()
...
def get_current_recoil_magnitude(pm, local_player_pawn):
    """Read the latest recoil magnitude from the same aim-punch cache used by RCS."""
    try:
        aim_punch_services = pm.read_longlong(local_player_pawn + m_pAimPunchServices)
        if not aim_punch_services:
            return None
        ...
        punch_x = pm.read_float(last_elem_addr)
        punch_y = pm.read_float(last_elem_addr + 4)
        return math.sqrt(punch_x * punch_x + punch_y * punch_y)
    except Exception:
        return None


def can_fire_for_recoil(current_magnitude, peak_magnitude, avoid_range):
    """Return True when triggerbot may fire based on recoil magnitude.

    avoid_range is a normalized value from 0.0 to 1.0. A value of 0.0
    means never avoid fire, while 1.0 means wait until recoil magnitude
    is effectively zero compared to the previous peak.
    """
    if current_magnitude is None or peak_magnitude <= 0.0:
        return True

    safe_threshold = peak_magnitude * (1.0 - max(0.0, min(1.0, avoid_range)))
    return current_magnitude <= safe_threshold
...
def triggerbot_thread():
    ...
    pm, client = wait_for_feature_memory_connection("TRIGGERBOT", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[TRIGGERBOT] Connected to CS2 via shared memory", "INFO")
    ...
    # Track previous target and key state to detect changes
    previous_entity_id = -1
    previous_key_held = False
    # Track last shot time for between-shots delay (time-based, not sleep-based)
    last_shot_time = 0
    # Track how long the current target has been lost so first-shot delay can be re-applied after a cooldown window
    target_lost_since = None
    # Track the recoil magnitude from the most recent triggerbot shot for avoid-recoil gating.
    recoil_peak = 0.0
        ...
        try:
            ...
            try:
                ...
                if auto_weapon_preset:
                    target_preset = current_weapon_name if current_weapon_name in TRIGGERBOT_WEAPON_PRESETS else "Default"
                    if Active_Config.get("current_triggerbot_preset", "Default") != target_preset:
            ...
            # Detect key release (transition from held to not held)
            if previous_key_held and not key_held:
                # Key was released - reset first shot pending for next trigger press
                first_shot_pending = True
                target_lost_since = None
                recoil_peak = 0.0
                previous_key_held = False
            ...
            # If no entity under crosshair, track the target-loss window and continue.
            # If the target is reacquired before the cooldown expires, the first-shot delay is not re-applied.
            if entity_id <= 0:
                if previous_entity_id != -1:
                    if target_lost_since is None:
                        target_lost_since = time.monotonic()
                else:
                    target_lost_since = None
                previous_entity_id = -1
                recoil_peak = 0.0
            ...
            # Detect target change (entity ID changed from last shot target)
            if previous_entity_id != entity_id and previous_entity_id != -1:
                # Treat a new target as a fresh first shot opportunity, while clearing any prior loss window.
                target_lost_since = None
                first_shot_pending = True
            
            # Update tracked target for next iteration's change detection
            previous_entity_id = entity_id
            
            # Get timing settings (first_shot_delay is now global, not per-preset)
            first_shot_delay = Active_Config.get("triggerbot_first_shot_delay", 0)
            first_shot_cooldown = Active_Config.get("triggerbot_first_shot_cooldown", 0.0)
            ...
            should_apply_first_shot_delay = False
            if first_shot_pending:
                should_apply_first_shot_delay = first_shot_delay > 0 and (target_lost_since is None or first_shot_cooldown <= 0.0)
            elif target_lost_since is not None:
                target_lost_elapsed = time.monotonic() - target_lost_since
                should_apply_first_shot_delay = first_shot_delay > 0 and (first_shot_cooldown <= 0.0 or target_lost_elapsed >= first_shot_cooldown)
            
            # Apply first shot delay if needed
            if should_apply_first_shot_delay:
            ...
            # Check if enough time has passed since last shot for between_shots_delay
            current_time = time.time()
            time_since_last_shot = (current_time - last_shot_time) * 1000  # Convert to milliseconds
            if time_since_last_shot < between_shots_delay:
                # Not enough time has passed, skip firing this frame
                time.sleep(0.001)
                continue

            if burst_mode:
                # Burst fire mode - fire burst_shots times rapidly
                # first_shot_delay applies before burst, between_shots_delay applies between bursts
                
                # Check if player is moving before burst
                if settings.get("triggerbot_dont_shoot_while_moving", False):
                    player_speed = get_local_player_speed(pm, local_player_pawn)
                    if player_speed is not None and player_speed > 0.0:
                        first_shot_pending = True
                ...
                # Fire the complete burst
                for i in range(burst_shots):
                    mouse.click(MouseButton.left)
                    time.sleep(0.15)  # 50ms delay between burst shots to ensure game registers them
                
                # Update last shot time for between-burst delay
...
def restore_flash_alpha():
    """Restore the flash alpha value when anti-flash is disabled."""
    ...
    try:
        pm, client = get_shared_memory_connection()
        if not pm or not client:
            pm, client = wait_for_feature_memory_connection("ANTI FLASH", max_wait_seconds=3)
...
def anti_flash_thread():
    ...
    pm, client = wait_for_feature_memory_connection("ANTI FLASH", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[ANTI FLASH] Connected to CS2 via shared memory", "INFO")
    
    while anti_flash_state["running"]:
        try:
            # Get settings from anti_flash_state, falling back to the active config.
            settings = anti_flash_state.get("settings")
            if not isinstance(settings, dict):
                settings = {}

            current_enabled = Active_Config.get("anti_flash_enabled", settings.get("anti_flash_enabled", False))
            if isinstance(settings, dict) and "anti_flash_enabled" in settings:
                current_enabled = settings.get("anti_flash_enabled", current_enabled)
...
def stop_anti_flash_thread():
    """Stop the anti-flash background thread and restore the normal flash alpha."""
    global anti_flash_state
    ...
    # Restore the flash alpha immediately so the game is not left blinded.
    restore_flash_alpha()
...
def hitsound_thread():
    ...
    pm, client = wait_for_feature_memory_connection("HITSOUND", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[HITSOUND] Connected to CS2 via shared memory", "INFO")
    ...
    while hitsound_state["running"]:
        try:
            ...
            try:
                ...
                if local_player_pawn:
                    # Get bullet services
                    bullet_services_offset = pBulletServicesOffset if pBulletServicesOffset else 0x1468
                    total_hits_offset = m_totalHits if m_totalHits else 0x48
                    pBulletServices = pm.read_longlong(local_player_pawn + bullet_services_offset)
                    if pBulletServices:
                        total_hits = pm.read_int(pBulletServices + total_hits_offset)
                        
                        previous_total_hits = hitsound_state.get("previous_total_hits")

                        # Prime baseline on first read to avoid false sound when overlay starts.
                        if previous_total_hits is None:
                            hitsound_state["previous_total_hits"] = total_hits
                        elif total_hits != previous_total_hits:
                            # Only play when the counter increases (actual new hit events).
                            if total_hits > previous_total_hits:
                                try:
...
def start_hitsound_thread():
    ...
    hitsound_state["previous_total_hits"] = None  # Reset; first sample primes baseline
...
def _apply_fov_value(pm, client, fov_value, apply_while_scoped=False):
    """Write the requested FOV value to the active camera services object if possible."""
    if not pm or not client:
        return False

    if not dwLocalPlayerPawn or not m_pCameraServices or not m_iFOV:
        return False
    ...
    try:
        ...
        if not apply_while_scoped:
            if not m_bIsScoped:
                return False
            try:
                if bool(pm.read_bool(local_player_pawn + m_bIsScoped)):
                    return False
            except Exception:
                return False

        target_fov = int(fov_value)
        #slight flickering when independant on reload/equip
        pm.write_int(camera_services + m_iFOV, target_fov)

        #nothing while independant
        #pm.write_float(camera_services + m_flFOVRate, target_fov)
        
        #nothiing while independant
        #pm.write_int(camera_services + m_iFOVStart, target_fov)
        
        
        pm.write_int(local_controller + m_iDesiredFOV, target_fov)
...
def fov_changer_thread():
    ...
    Functionality:
    1. Resolve local player pawn and camera services pointer
    2. Skip writes while scoped (preserves scoped zoom behavior)
    3. Write desired FOV to CCSPlayerBase_CameraServices::m_iFOV
    """
    global fov_changer_state
    
    debug_log("[FOV CHANGER] Thread starting...", "INFO")
    
    pm, client = wait_for_feature_memory_connection("FOV CHANGER", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[FOV CHANGER] Connected to CS2 via shared memory", "INFO")
    ...
    while fov_changer_state["running"]:
        try:
            ...
            # Always maintain a shared memory connection and write the proper FOV
            pm, client = get_shared_memory_connection()
            ...
            fov_value = int(settings.get("fov_value", 90))
            apply_while_scoped = bool(settings.get("fov_apply_while_scoped", False))
            _apply_fov_value(pm, client, fov_value, apply_while_scoped)
            time.sleep(0.001)
...
def acs_thread():
    """Compatibility thread that no longer moves the camera; it simply keeps the feature idle."""
    global acs_state
...
# =============================================================================
# RCS (RECOIL CONTROL SYSTEM) THREAD
# =============================================================================

def rcs_thread():
    """Recoil control system thread - compensates weapon recoil using aim punch deltas."""
    global rcs_state

    debug_log("[RCS] Thread starting...", "INFO")

    pm, client = wait_for_feature_memory_connection("RCS", max_wait_seconds=10)
    if not pm or not client:
        return
    ...
    while rcs_state["running"]:
        try:
            settings = rcs_state.get("settings") or {}
            ...
            if not is_cs2_foreground() or is_cheat_foreground():
                time.sleep(0.01)
                continue
            ...
            # Only compensate while actually firing
            if shots_fired <= 1 or shots_fired >= 999999:
                old_punch_x = 0.0
                old_punch_y = 0.0
                residual_x = 0.0
                residual_y = 0.0
            ...
            # Read aim punch cache pointer
            aim_punch_services = pm.read_longlong(local_player_pawn + m_pAimPunchServices)
            if not aim_punch_services:
                time.sleep(0.005)
                continue

            # Cache count and data pointer live at m_unpredictableBaseTick - 0x18
            cache_offset = m_unpredictableBaseTick - 0x18
            cache_count = pm.read_ulonglong(aim_punch_services + cache_offset)
            cache_data  = pm.read_ulonglong(aim_punch_services + cache_offset + 0x08)
            ...
            # Each element is a Vector3 (3 floats = 12 bytes) — grab the last entry
            last_elem_addr = cache_data + (cache_count - 1) * 12
            punch_x = pm.read_float(last_elem_addr)
            punch_y = pm.read_float(last_elem_addr + 4)

            # Match reference project behavior: read only from dwSensitivity pointer path.
            sensitivity = 1.0
            try:
                if dwSensitivity and dwSensitivity_sensitivity:
                    sens_ptr = pm.read_longlong(client + dwSensitivity)
                    if sens_ptr:
                        raw_sens = pm.read_float(sens_ptr + dwSensitivity_sensitivity)
                        if 0.01 < raw_sens < 20.0:
                            sensitivity = raw_sens
            except Exception:
                pass

            # Compute per-frame aim punch delta and convert to pixel movement
            delta_x = (punch_x - old_punch_x) * -1.0
            delta_y = (punch_y - old_punch_y) * -1.0

            move_x = (delta_y * 2.0 / sensitivity) / -0.022
            move_y = (delta_x * 2.0 / sensitivity) /  0.022

            # Apply human-like smoothing on top of the selected compensation percentage.
            horizontal_smoothing = max(0.0, min(float(settings.get("rcs_horizontal_smoothing", 0.0)), 10.0))
            vertical_smoothing = max(0.0, min(float(settings.get("rcs_vertical_smoothing", 0.0)), 10.0))
            horizontal_compensation_percentage = max(1, min(int(settings.get("rcs_horizontal_compensation_percentage", 100)), 100)) / 100.0
            vertical_compensation_percentage = max(1, min(int(settings.get("rcs_vertical_compensation_percentage", 100)), 100)) / 100.0

            horizontal_enabled = settings.get("rcs_horizontal_compensation", True)
            vertical_enabled = settings.get("rcs_vertical_compensation", True)

            smoothing_technique = str(settings.get("rcs_smoothing_technique", "Instant")).strip().capitalize()
            ...
            if not horizontal_enabled:
                ...
                strength = max(0.0, min(horizontal_smoothing / 10.0, 1.0))
                if smoothing_technique == "Softened":
                    alpha = max(0.05, 1.0 - (strength * 0.95))
                    target_move_x = (move_x * horizontal_compensation_percentage) + residual_x
                    smoothed_move_x = smoothed_move_x + (target_move_x - smoothed_move_x) * alpha
                else:
                    alpha = max(0.08, 1.0 - (strength * 0.92))
                    catch_up_strength = min(0.35, 0.12 + (strength * 0.23))
                    effective_alpha = min(0.95, alpha + catch_up_strength)
                    target_move_x = (move_x * horizontal_compensation_percentage) + residual_x
                    smoothed_move_x = smoothed_move_x + (target_move_x - smoothed_move_x) * effective_alpha
                move_x = smoothed_move_x
            ...
            if not vertical_enabled:
                ...
                strength = max(0.0, min(vertical_smoothing / 10.0, 1.0))
                if smoothing_technique == "Softened":
                    alpha = max(0.05, 1.0 - (strength * 0.95))
                    target_move_y = (move_y * vertical_compensation_percentage) + residual_y
                    smoothed_move_y = smoothed_move_y + (target_move_y - smoothed_move_y) * alpha
                else:
                    alpha = max(0.08, 1.0 - (strength * 0.92))
                    catch_up_strength = min(0.35, 0.12 + (strength * 0.23))
                    effective_alpha = min(0.95, alpha + catch_up_strength)
                    target_move_y = (move_y * vertical_compensation_percentage) + residual_y
                    smoothed_move_y = smoothed_move_y + (target_move_y - smoothed_move_y) * effective_alpha
                move_y = smoothed_move_y

            # Preserve sub-pixel movement to avoid under-compensation at higher sensitivities.
            int_x = int(round(move_x))
            int_y = int(round(move_y))
            residual_x = move_x - int_x
            residual_y = move_y - int_y

            if int_x != 0 or int_y != 0:
                win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, int_x, int_y, 0, 0)
    ...
    debug_log("[RCS] Thread stopped", "INFO")
...
def on_test_clicked():
    ...
    # Wait indefinitely for CS2 while keeping the loader responsive/draggable.
    # There is intentionally no timeout or elapsed-time limit here.
    while True:
        try:
            if is_cs2_running():
                break
        except Exception as e:
            debug_log(f"CS2 detection check failed; continuing to wait: {e}", "WARNING")

        # Keep custom drag behavior active during the waiting stage.
        update_window_drag()

        # Keep UI responsive while this callback is waiting.
        try:
            dpg.render_dearpygui_frame()
        except Exception:
            pass

        # Stop waiting if the app is closing or a window transition was requested.
        if (not dpg.is_dearpygui_running()) or app_state.get("pending_window_transition", False):
            restore_loader_ui()
            return
    ...
    dpg.set_value("progress_status_text", "Downloading hitsounds...")
    dpg.render_dearpygui_frame()  # Force UI update
    ...
    sound_files = [
        ...
        (SPECIALHIT_SOUND_URL, "nigger.wav")
    ]
    ...
    # Stage 4: Check local offsets if enabled
    if loader_settings["UseLocalOffsets"]:
        dpg.set_value("progress_launch", 0.75)
        dpg.set_value("progress_status_text", "Checking local offsets...")
        dpg.render_dearpygui_frame()  # Force UI update
        ...
        if not os.path.exists(output_folder):
            ...
            ctypes.windll.user32.MessageBoxW(
                hwnd,
                "Local offsets not found. Please create offsets first.",
                title,
        ...
        if not load_and_initialize_offsets(use_local=True):
            ...
            ctypes.windll.user32.MessageBoxW(
                hwnd,
                "Failed to load local offsets. Files may be corrupted.",
                title,
                0x00000010 | 0x00040000  # MB_ICONERROR | MB_TOPMOST
            )
        ...
        if not load_and_initialize_offsets(use_local=False):
            ...
            error_hint = "Check your internet connection."
            try:
                log_path = os.path.join(SCRIPT_DIR, "offset_download_error.log")
                if os.path.exists(log_path):
                    with open(log_path, "r") as f:
                        err_content = f.read().split('\n')[0]
                        error_hint = f"Details in offset_download_error.log: {err_content[:100]}"
            except:
                pass
...
def on_unload_clicked():
    """Unload the cheat runtime and return to the loader window."""
    app_state["requested_window"] = "loader"
    app_state["pending_window_transition"] = True
    app_state["switch_to_cheat"] = False
    app_state["switch_to_loader"] = True
    try:
        dpg.stop_dearpygui()
    except Exception:
        pass
...
def on_rounded_corners_changed(sender, value):
    ...
    """
    Active_Config["rounded_window_corners"] = value
    save_settings()
...
def on_anti_flash_toggle(sender, value):
    ...
    _update_spectator_protection_manual_state("anti_flash", value)
    if anti_flash_state.get("settings"):
    ...
    update_spectator_protections()
    debug_log(f"Anti-Flash {'enabled' if value else 'disabled'}", "INFO")


def on_local_flashed_indicator_toggle(sender, value):
    """Handle the local player's flashed indicator toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["local_flashed_indicator_enabled"] = value
    Active_Config["local_flashed_indicator_enabled"] = value
    save_settings()
    debug_log(f"Flashed indicator {'enabled' if value else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()
...
def upload_custom_sound():
    ...
    try:
        ...
        if file_path:
            ...
            # If the target name already exists, create a unique filename instead of overwriting it.
...
def refresh_hitsound_combo():
    ...
    # Build items list
    items = ["hitmarker", "criticalhit", "metalhit", "nigger"]
...
def reset_fov_to_default():
    ...
    try:
        pm, client = wait_for_feature_memory_connection("FOV CHANGER RESET", max_wait_seconds=5)
        if not pm or not client:
            return

        for attempt in range(3):
            if _apply_fov_value(pm, client, 90):
...
def on_fov_apply_while_scoped_toggle(sender, value):
    """Handle Apply While Scoped toggle."""
    if fov_changer_state["settings"]:
        fov_changer_state["settings"]["fov_apply_while_scoped"] = value
    Active_Config["fov_apply_while_scoped"] = value
    save_settings()
    debug_log(f"FOV Apply While Scoped {'enabled' if value else 'disabled'}", "INFO")
...
def on_show_spectator_list_toggle(sender, value):
    """Handle show spectator list overlay toggle."""
    Active_Config["show_spectator_list_overlay"] = value
    save_settings()
    debug_log(f"Spectator list overlay {'enabled' if value else 'disabled'}", "INFO")
    sync_spectator_list_thread_state()


def on_spectator_disable_anti_flash_toggle(sender, value):
    """Handle spectator protection toggle for Anti-Flash."""
    Active_Config["spectator_protections_disable_anti_flash"] = value
    save_settings()
    debug_log(f"Spectator protection Disable Anti-Flash {'enabled' if value else 'disabled'}", "INFO")
    update_spectator_protections()


def on_spectator_disable_aimbot_toggle(sender, value):
    """Handle spectator protection toggle for Aimbot."""
    Active_Config["spectator_protections_disable_aimbot"] = value
    save_settings()
    debug_log(f"Spectator protection Disable Aimbot {'enabled' if value else 'disabled'}", "INFO")
    update_spectator_protections()


def on_spectator_disable_rcs_toggle(sender, value):
    """Handle spectator protection toggle for RCS."""
    Active_Config["spectator_protections_disable_rcs"] = value
    save_settings()
    debug_log(f"Spectator protection Disable Recoil Control {'enabled' if value else 'disabled'}", "INFO")
    update_spectator_protections()
...
def on_show_overlay_fps_toggle(sender, value):
    ...
    if esp_overlay["settings"]:
        esp_overlay["settings"]["show_overlay_fps"] = value
...
def on_create_offsets_clicked():
    ...
    def create_offsets_thread():
        ...
        try:
            ...
            # Step 5: Keep only the core manual offset files in the output folder
            try:
                cleanup_local_offsets_directory(output_folder)
            except Exception:
                pass
            
            # Step 6: Update button to show success (UI updates are safe via configure_item)
            dpg.configure_item("btn_create_offsets", label="Offsets Created!")
            time.sleep(2)
            
            # Step 7: Reset button to original state
...
def update_window_drag():
    ...
    if is_mouse_down:
        ...
        if not drag_state["is_dragging"]:
            # Only start a drag on the initial press, and only if the press began on the titlebar
            if not drag_state.get("mouse_was_down", False):
                _try_start_drag(cursor_x, cursor_y)
        else:
            # Currently dragging - update window position
            _continue_drag(cursor_x, cursor_y)

        drag_state["mouse_was_down"] = True
    else:
        # Mouse released - stop dragging
        drag_state["is_dragging"] = False
        drag_state["mouse_was_down"] = False
...
def create_settings_tab():
    ...
    with dpg.tab(label="Settings"):
        ...
        dpg.add_checkbox(label="Show Debug Tab", tag="chk_show_debug_tab", default_value=False)
        with dpg.tooltip("chk_show_debug_tab", tag="tooltip_show_debug_tab", show=show_tips):
            dpg.add_text("Show the Debug tab in the menu for troubleshooting.")
        ...
        with dpg.tooltip("chk_show_tooltips", tag="tooltip_show_tooltips", show=show_tips):
            dpg.add_text("Turn these help pop-ups on or off.")
        ...
        with dpg.tooltip("chk_use_local_offsets", tag="tooltip_use_local_offsets", show=show_tips):
            dpg.add_text("Create local offset files for offline use.")
            dpg.add_text("Useful when live offsets are unavailable.")
        ...
        with dpg.tooltip("chk_preload_config", tag="tooltip_preload_config", show=show_tips):
            dpg.add_text("Load a saved config automatically at launch.")
            dpg.add_text("This skips the default setup and uses your chosen preset.")
        ...
        with dpg.tooltip("combo_preload_config", tag="tooltip_combo_preload_config", show=show_tips):
            dpg.add_text("Choose which config to load automatically when the menu opens.")
...
def create_changelog_tab():
    ...
    with dpg.tab(label="Changelog"):
        dpg.add_separator()
        dpg.add_text("V1.0.5: 7/14/26 ")
        dpg.add_text(" - Added external radar")
        dpg.add_text(" - Radar max size increased to 600 ")
        dpg.add_text(" - Improved threading")
        dpg.add_text(" - Reorganized UI")
        dpg.add_text(" - Updated RCS")
        dpg.add_text(" - Overlay and ESP optimizations")
        dpg.add_text(" - Added bhop")
        dpg.add_text(" - Added aimbot humanization")
        dpg.add_text(" - Added aimbot axis toggles")
        dpg.add_text(" - Added spectator list")
        dpg.add_separator()
        dpg.add_text("V1.0.4: 7/13/26 ")
        dpg.add_text("- Updated aimbot spotted check to visible check")
        dpg.add_text("- Revamped recoil control system")
        dpg.add_text("- Fixed fov changer resetting on reload")
        dpg.add_text("- Fixed hitsound playing on cheat start")
        dpg.add_text("- Improved aimbot bone targeting ")
        dpg.add_text("- Removed pelvis bone target")
        dpg.add_text("- Fixed slow triggerbot thread activation")
        dpg.add_separator()
        dpg.add_text("V1.0.3: 7/12/26")
        dpg.add_text("- Reorganized UI")
        ...
        dpg.add_separator()
        dpg.add_text("V1.0.0: 7/9/26")
        dpg.add_text("- Temporarily removed RCS")
        dpg.add_text("- Updated head detection")
        dpg.add_text("- Fixed local offset creation")
        dpg.add_text("- Changed default UI theme")
        dpg.add_text("- Removed bullet tracers")
        dpg.add_text("- Fixed hitsounds not playing")


def on_info_reveal_image_clicked(sender, app_data, user_data):
    """Reveal the Info tab image and hide the reveal button."""
    dpg.configure_item("btn_info_reveal_image", show=False)
    dpg.configure_item("group_info_image_container", show=True)

    if dpg.does_item_exist("img_loader_info") or dpg.does_item_exist("txt_loader_info_error"):
        return

    image_size = ensure_loader_info_image_texture()
    if image_size:
        image_width, image_height = image_size
        display_width = min(LOADER_INFO_IMAGE_MAX_WIDTH, image_width)
        display_height = max(1, int(image_height * (display_width / image_width)))
        dpg.add_image(
            LOADER_INFO_TEXTURE_TAG,
            width=display_width,
            height=display_height,
            parent="group_info_image_container",
            tag="img_loader_info",
        )
    else:
        dpg.add_text(
            "Info image unavailable (download failed).",
            color=(255, 140, 140),
            parent="group_info_image_container",
            tag="txt_loader_info_error",
        )


def ensure_loader_info_image_texture():
    """
    Download and register the loader Info tab image texture if needed.

    Returns:
        (width, height) tuple on success, None on failure.
    """
    def _download_loader_image_file():
        image_urls = [LOADER_INFO_IMAGE_URL]
        clean_discord_url = LOADER_INFO_IMAGE_URL.split("?", 1)[0]
        if clean_discord_url != LOADER_INFO_IMAGE_URL:
            image_urls.append(clean_discord_url)
        ...
        for image_url in image_urls:
            try:
                ...
                with open(LOADER_INFO_IMAGE_PATH, "wb") as image_file:
                    image_file.write(response.content)

                if os.path.getsize(LOADER_INFO_IMAGE_PATH) >= 128:
        ...
        if not download_success:
            raise RuntimeError(f"Could not download image from provided URL(s): {last_error}")

    try:
        if not os.path.exists(LOADER_INFO_IMAGE_PATH) or os.path.getsize(LOADER_INFO_IMAGE_PATH) < 128:
            _download_loader_image_file()

        try:
            width, height, _channels, data = dpg.load_image(LOADER_INFO_IMAGE_PATH)
        except Exception:
            # Cached file may be an HTML error page or otherwise invalid image.
            try:
                if os.path.exists(LOADER_INFO_IMAGE_PATH):
                    os.remove(LOADER_INFO_IMAGE_PATH)
            except Exception:
                pass
            _download_loader_image_file()
            width, height, _channels, data = dpg.load_image(LOADER_INFO_IMAGE_PATH)

        if not dpg.does_item_exist(LOADER_TEXTURE_REGISTRY_TAG):
            with dpg.texture_registry(show=False, tag=LOADER_TEXTURE_REGISTRY_TAG):
                dpg.add_static_texture(width, height, data, tag=LOADER_INFO_TEXTURE_TAG)
        elif not dpg.does_item_exist(LOADER_INFO_TEXTURE_TAG):
            dpg.add_static_texture(
                width,
                height,
                data,
                parent=LOADER_TEXTURE_REGISTRY_TAG,
                tag=LOADER_INFO_TEXTURE_TAG,
            )

        return (width, height)
    except Exception as e:
        debug_log(f"Failed to load loader Info tab image: {e}", "WARNING")
        return None
...
def on_head_hitbox_thickness_change(sender, value):
    """Handle head hitbox thickness change."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["head_hitbox_thickness"] = value
    Active_Config["head_hitbox_thickness"] = value
    save_settings()
    debug_log(f"Head hitbox thickness changed to: {value}", "INFO")


def on_hide_spotted_players_toggle(sender, value):
    """Handle Hide Visible Players toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["hide_spotted_players"] = value
        Active_Config["hide_spotted_players"] = value
        save_settings()
        debug_log(f"Hide Visible Players {'enabled' if value else 'disabled'}", "INFO")


def on_hide_while_spectated_toggle(sender, value):
    """Handle Hide While Spectated toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["hide_while_spectated"] = value
    Active_Config["hide_while_spectated"] = value
    save_settings()
    debug_log(f"Hide While Spectated {'enabled' if value else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()
...
def on_custom_crosshair_toggle(sender, value):
    ...
    # Keep the settings controls visible even when the crosshair itself is disabled.
    show_tips = Active_Config.get("show_tooltips", True)
    tooltip_options = [
        "tooltip_custom_crosshair_scale",
        "tooltip_custom_crosshair_thickness",
        "tooltip_custom_crosshair_shape",
        "tooltip_custom_crosshair_snipers_only",
        "tooltip_crosshair_use_targeting_color",
        "tooltip_custom_crosshair_color",
        "tooltip_crosshair_targeting_color"
    ]
    for tooltip in tooltip_options:
        if dpg.does_item_exist(tooltip):
            dpg.configure_item(tooltip, show=show_tips)
...
def on_esp_targeting_type_change(sender, value):
    """Handle ESP/radar targeting type change."""
    targeting_type = normalize_targeting_type_value(value)
    Active_Config["esp_targeting_type"] = targeting_type
    if esp_overlay["settings"]:
        esp_overlay["settings"]["esp_targeting_type"] = targeting_type
    save_settings()
    debug_log(f"ESP targeting type changed to: {value}", "INFO")


def on_aimbot_targeting_type_change(sender, value):
    """Handle aimbot targeting type change."""
    targeting_type = normalize_targeting_type_value(value)
    Active_Config["aimbot_targeting_type"] = targeting_type
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_targeting_type"] = targeting_type
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_targeting_type"] = targeting_type
    save_settings()
    debug_log(f"Aimbot targeting type changed to: {value}", "INFO")


def on_triggerbot_targeting_type_change(sender, value):
    """Handle triggerbot targeting type change."""
    targeting_type = normalize_targeting_type_value(value)
    Active_Config["triggerbot_targeting_type"] = targeting_type
    if esp_overlay["settings"]:
        esp_overlay["settings"]["triggerbot_targeting_type"] = targeting_type
    if triggerbot_state["settings"]:
        triggerbot_state["settings"]["triggerbot_targeting_type"] = targeting_type
    save_settings()
    debug_log(f"Triggerbot targeting type changed to: {value}", "INFO")


def on_acs_targeting_type_change(sender, value):
    """Handle ACP targeting type change."""
    targeting_type = normalize_targeting_type_value(value)
    Active_Config["acs_targeting_type"] = targeting_type
    if esp_overlay["settings"]:
        esp_overlay["settings"]["acs_targeting_type"] = targeting_type
    if acs_state["settings"]:
        acs_state["settings"]["acs_targeting_type"] = targeting_type
    save_settings()
    debug_log(f"ACP targeting type changed to: {value}", "INFO")
...
def on_custom_crosshair_snipers_only_toggle(sender, value):
    """Handle custom crosshair snipers-only toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["custom_crosshair_snipers_only"] = value
    Active_Config["custom_crosshair_snipers_only"] = value
    save_settings()
    debug_log(f"Custom crosshair snipers-only {'enabled' if value else 'disabled'}", "INFO")


def on_crosshair_use_targeting_color_toggle(sender, value):
    """Handle crosshair use targeting color toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["crosshair_use_targeting_color"] = value
    Active_Config["crosshair_use_targeting_color"] = value
    save_settings()
    debug_log(f"Crosshair targeting color {'enabled' if value else 'disabled'}", "INFO")


def on_crosshair_targeting_color_change(sender, value):
    """Handle crosshair targeting color change."""
    rgb_color = (int(value[0] * 255), int(value[1] * 255), int(value[2] * 255))
    if esp_overlay["settings"]:
        esp_overlay["settings"]["crosshair_targeting_color"] = rgb_color
    Active_Config["crosshair_targeting_color"] = rgb_color
    save_settings()
    debug_log(f"Crosshair targeting color changed to: {rgb_color}", "INFO")
...
def on_radar_external_mode_toggle(sender, value):
    """Handle external radar mode toggle."""
    # External mode and overlap-game mode are mutually exclusive.
    if value and Active_Config.get("radar_overlap_game", False):
        on_radar_overlap_game_toggle(None, False)

    if esp_overlay["settings"]:
        esp_overlay["settings"]["radar_external_mode"] = value
    Active_Config["radar_external_mode"] = value

    if dpg.does_item_exist("chk_radar_external_mode"):
        dpg.set_value("chk_radar_external_mode", value)

    save_settings()

    mode_text = "enabled" if value else "disabled"
    debug_log(f"External radar mode {mode_text}", "INFO")
    sync_esp_overlay_thread_state()
    sync_external_radar_window_state()


def on_radar_external_additional_info_toggle(sender, value):
    """Handle external radar additional info toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["radar_external_additional_info"] = value
    Active_Config["radar_external_additional_info"] = value
    save_settings()
    debug_log(f"External radar additional info {'enabled' if value else 'disabled'}", "INFO")
...
def on_radar_overlap_game_toggle(sender, value):
    ...
    # Overlap-game mode and external mode are mutually exclusive.
    if value and Active_Config.get("radar_external_mode", False):
        if esp_overlay["settings"]:
            esp_overlay["settings"]["radar_external_mode"] = False
        Active_Config["radar_external_mode"] = False
        if dpg.does_item_exist("chk_radar_external_mode"):
            dpg.set_value("chk_radar_external_mode", False)
    ...
    if dpg.does_item_exist("chk_radar_overlap_game"):
        dpg.set_value("chk_radar_overlap_game", value)
...
def on_weapon_text_size_change(sender, value):
    """Handle weapon label text size slider change."""
    if esp_overlay["settings"]:
...
def on_distance_text_size_change(sender, value):
    """Handle distance text size slider change."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["distance_text_size"] = value
    Active_Config["distance_text_size"] = value
    save_settings()
    debug_log(f"Distance text size changed to: {value}", "INFO")


def on_flashed_indicator_toggle(sender, value):
    """Handle flashed indicator visibility toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["flashed_indicator_enabled"] = value
    Active_Config["flashed_indicator_enabled"] = value
    save_settings()
    debug_log(f"Flashed indicator {'enabled' if value else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()


def on_flashed_indicator_text_size_change(sender, value):
    """Handle flashed indicator label text size changes."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["flashed_indicator_text_size"] = value
    Active_Config["flashed_indicator_text_size"] = value
    save_settings()
    debug_log(f"Flashed indicator text size changed to: {value}", "INFO")
...
def on_aimbot_toggle(sender, value, manual=True):
    """Handle aimbot enable/disable toggle."""
    if manual:
        _update_spectator_protection_manual_state("aimbot", value)
    if esp_overlay["settings"]:
...
def on_aimbot_visible_check_toggle(sender, value):
    """Handle aimbot visible check toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_visible_check"] = value
        esp_overlay["settings"]["aimbot_spotted_check"] = value
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_visible_check"] = value
        aimbot_state["settings"]["aimbot_spotted_check"] = value
    Active_Config["aimbot_visible_check"] = value
    Active_Config["aimbot_spotted_check"] = value
    save_settings()
    debug_log(f"Aimbot visible check {'enabled' if value else 'disabled'}", "INFO")


def on_aimbot_adjust_x_axis_toggle(sender, value):
    """Handle aimbot X-axis adjustment toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_adjust_x_axis"] = value
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_adjust_x_axis"] = value
    Active_Config["aimbot_adjust_x_axis"] = value
    save_settings()
    debug_log(f"Aimbot adjust X axis {'enabled' if value else 'disabled'}", "INFO")


def on_aimbot_adjust_y_axis_toggle(sender, value):
    """Handle aimbot Y-axis adjustment toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_adjust_y_axis"] = value
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_adjust_y_axis"] = value
    Active_Config["aimbot_adjust_y_axis"] = value
    save_settings()
    debug_log(f"Aimbot adjust Y axis {'enabled' if value else 'disabled'}", "INFO")


def on_aimbot_spotted_check_toggle(sender, value):
    """Backward-compatible alias for old callback name."""
    on_aimbot_visible_check_toggle(sender, value)


def on_aimbot_target_bone_change(sender, value):
    """Handle aimbot target bone change."""
    value = normalize_target_bone_value(value)
    if esp_overlay["settings"]:
...
def on_aimbot_use_humanization_toggle(sender, value):
    """Handle aimbot humanization toggle."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_use_humanization"] = value
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_use_humanization"] = value
    aimbot_state["humanization"] = build_default_aimbot_humanization_state()
    Active_Config["aimbot_use_humanization"] = value
    save_settings()

    show_tips = Active_Config.get("show_tooltips", True)
    if dpg.does_item_exist("combo_aimbot_movement_type"):
        dpg.configure_item("combo_aimbot_movement_type", show=not value)
    if dpg.does_item_exist("tooltip_aimbot_movement_type"):
        dpg.configure_item("tooltip_aimbot_movement_type", show=show_tips and not value)
    if dpg.does_item_exist("slider_aimbot_smoothness"):
        dpg.configure_item("slider_aimbot_smoothness", show=not value)
    if dpg.does_item_exist("tooltip_aimbot_smoothness"):
        dpg.configure_item("tooltip_aimbot_smoothness", show=show_tips and not value)
    if dpg.does_item_exist("separator_aimbot_after_smoothness"):
        dpg.configure_item("separator_aimbot_after_smoothness", show=not value)

    debug_log(f"Aimbot humanization {'enabled' if value else 'disabled'}", "INFO")


def on_aimbot_humanization_strength_change(sender, value):
    """Handle humanization strength slider change."""
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_humanization_strength"] = value
    if aimbot_state["settings"]:
        aimbot_state["settings"]["aimbot_humanization_strength"] = value
    Active_Config["aimbot_humanization_strength"] = value
    save_settings()
    debug_log(f"Aimbot humanization strength changed to: {value}", "INFO")
...
def on_aimbot_snapline_color_change(sender, value):
    ...
    color = (int(value[0] * 255), int(value[1] * 255), int(value[2] * 255))
    if esp_overlay["settings"]:
        esp_overlay["settings"]["aimbot_snapline_color"] = color
    Active_Config["aimbot_snapline_color"] = color
    save_settings()
    debug_log(f"Aimbot snapline color changed to: {color}", "INFO")
...
def on_triggerbot_toggle(sender, value):
    ...
    if is_any_triggerbot_preset_enabled():
...
def on_triggerbot_first_shot_delay_change(sender, value):
    """Handle triggerbot first shot delay change (global setting)."""
...
def on_triggerbot_first_shot_cooldown_change(sender, value):
    """Handle triggerbot first shot cooldown change (global setting)."""
    Active_Config["triggerbot_first_shot_cooldown"] = value
    save_settings()
    if triggerbot_state["settings"]:
        triggerbot_state["settings"]["triggerbot_first_shot_cooldown"] = value
    debug_log(f"Triggerbot first shot cooldown changed to: {value}s", "INFO")
...
def on_acs_toggle(sender, value):
    """Handle the crosshair placement indicator toggle."""
    if esp_overlay["settings"]:
    ...
    debug_log(f"Crosshair indicator {'enabled' if value else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()
...
# =============================================================================
# RCS (RECOIL CONTROL SYSTEM) CALLBACKS
# =============================================================================

def on_rcs_toggle(sender, value, manual=True):
    """Handle RCS enable/disable toggle."""
    if manual:
        _update_spectator_protection_manual_state("rcs", value)
...
def on_rcs_visualizer_toggle(sender, value):
    """Handle Recoil Visualizer toggle."""
    Active_Config["rcs_visualizer_enabled"] = value
    if esp_overlay["settings"] is not None:
        esp_overlay["settings"]["rcs_visualizer_enabled"] = value
    save_settings()
    debug_log(f"Recoil Visualizer {'enabled' if value else 'disabled'}", "INFO")
    sync_esp_overlay_thread_state()


def on_rcs_visualizer_color_change(sender, value):
    """Handle Recoil Visualizer color picker change."""
    if not isinstance(value, (list, tuple)) or len(value) < 3:
        return
    rgb = (int(value[0] * 255), int(value[1] * 255), int(value[2] * 255))
    if esp_overlay["settings"] is not None:
        esp_overlay["settings"]["rcs_visualizer_color"] = rgb
    Active_Config["rcs_visualizer_color"] = rgb
    save_settings()
    debug_log(f"Recoil Visualizer color changed to: {rgb}", "INFO")


def on_rcs_visualizer_transparency_change(sender, value):
    """Handle Recoil Visualizer transparency slider change."""
    value = max(0, min(int(value), 255))
    if esp_overlay["settings"] is not None:
        esp_overlay["settings"]["rcs_visualizer_transparency"] = value
    Active_Config["rcs_visualizer_transparency"] = value
    save_settings()
    debug_log(f"Recoil Visualizer transparency changed to: {value}", "INFO")


def on_rcs_horizontal_smoothing_change(sender, value):
    """Handle RCS horizontal smoothing slider change."""
    value = max(0.0, min(float(value), 10.0))
    Active_Config["rcs_horizontal_smoothing"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_horizontal_smoothing"] = value
    save_settings()
    debug_log(f"RCS horizontal smoothing changed to: {value}", "INFO")


def on_rcs_vertical_smoothing_change(sender, value):
    """Handle RCS vertical smoothing slider change."""
    value = max(0.0, min(float(value), 10.0))
    Active_Config["rcs_vertical_smoothing"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_vertical_smoothing"] = value
    save_settings()
    debug_log(f"RCS vertical smoothing changed to: {value}", "INFO")


def on_rcs_horizontal_compensation_toggle(sender, value):
    """Handle horizontal recoil compensation toggle."""
    Active_Config["rcs_horizontal_compensation"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_horizontal_compensation"] = value
    save_settings()
    debug_log(f"RCS horizontal compensation {'enabled' if value else 'disabled'}", "INFO")


def on_rcs_vertical_compensation_toggle(sender, value):
    """Handle vertical recoil compensation toggle."""
    Active_Config["rcs_vertical_compensation"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_vertical_compensation"] = value
    save_settings()
    debug_log(f"RCS vertical compensation {'enabled' if value else 'disabled'}", "INFO")


def on_rcs_horizontal_compensation_percentage_change(sender, value):
    """Handle horizontal recoil compensation percentage slider change."""
    value = max(1, min(int(value), 100))
    Active_Config["rcs_horizontal_compensation_percentage"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_horizontal_compensation_percentage"] = value
    save_settings()
    debug_log(f"RCS horizontal compensation percentage changed to: {value}%", "INFO")


def on_rcs_vertical_compensation_percentage_change(sender, value):
    """Handle vertical recoil compensation percentage slider change."""
    value = max(1, min(int(value), 100))
    Active_Config["rcs_vertical_compensation_percentage"] = value
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_vertical_compensation_percentage"] = value
    save_settings()
    debug_log(f"RCS vertical compensation percentage changed to: {value}%", "INFO")


def on_rcs_smoothing_technique_change(sender, value):
    """Handle RCS smoothing technique dropdown change."""
    technique = "Instant" if str(value).lower() == "instant" else "Softened"
    Active_Config["rcs_smoothing_technique"] = technique
    if rcs_state.get("settings") is not None:
        rcs_state["settings"]["rcs_smoothing_technique"] = technique
    save_settings()
    debug_log(f"RCS smoothing technique changed to: {technique}", "INFO")
...
def apply_colorway(colorway_name):
    ...
    rounded_enabled = bool(Active_Config.get("rounded_window_corners", True))
    rounded_value = 10.0 if rounded_enabled else 0.0
    frame_rounding = 7.0 if rounded_enabled else 0.0
    tab_rounding = 6.0 if rounded_enabled else 0.0
    grab_rounding = 6.0 if rounded_enabled else 0.0
    ...
    with dpg.theme(tag="global_theme"):
        with dpg.theme_component(dpg.mvAll):
            ...
            # Rounded control styling tied to the toggle
            dpg.add_theme_style(dpg.mvStyleVar_WindowRounding, rounded_value)
            dpg.add_theme_style(dpg.mvStyleVar_ChildRounding, rounded_value)
            dpg.add_theme_style(dpg.mvStyleVar_FrameRounding, frame_rounding)
            dpg.add_theme_style(dpg.mvStyleVar_PopupRounding, rounded_value)
            dpg.add_theme_style(dpg.mvStyleVar_TabRounding, tab_rounding)
            dpg.add_theme_style(dpg.mvStyleVar_GrabRounding, grab_rounding)
            dpg.add_theme_style(dpg.mvStyleVar_ScrollbarRounding, rounded_value * 0.7)
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="ESP"):
                ...
                with dpg.tooltip("chk_esp_enabled", tag="tooltip_esp_enabled", show=show_tips):
                    dpg.add_text("Show or hide all ESP elements")
                ...
                with dpg.tooltip("btn_bind_esp_toggle_cheat", tag="tooltip_esp_toggle_key", show=show_tips):
                    dpg.add_text("Bind to toggle ESP")
                    dpg.add_text("ESC to remove keybind")
                ALL_TOOLTIP_TAGS.append("tooltip_esp_toggle_key")

                dpg.add_combo(
                    items=["Enemies Only", "All Players"],
                    default_value="All Players" if Active_Config.get("esp_targeting_type", 0) == 1 else "Enemies Only",
                    label="Target Team",
                    tag="combo_esp_targeting_type",
                    callback=on_esp_targeting_type_change
                )
                with dpg.tooltip("combo_esp_targeting_type", tag="tooltip_esp_targeting_type", show=show_tips):
                    dpg.add_text("Show ESP elements for enemies or all players")
                ALL_TOOLTIP_TAGS.append("tooltip_esp_targeting_type")

                dpg.add_checkbox(
                    label="Hide Visible Players", 
                ...
                with dpg.tooltip("chk_hide_spotted_players", tag="tooltip_hide_spotted_players", show=show_tips):
                    dpg.add_text("Hide ESP details for visible players")
                ALL_TOOLTIP_TAGS.append("tooltip_hide_spotted_players")

                dpg.add_spacer(height=UI_SPACING_SMALL)
                dpg.add_text("Box Settings")
                dpg.add_separator()
                ...
                dpg.add_combo(
                    items=["2D Full", "2D Corner", "3D Full", "3D Corner"],
                    default_value=Active_Config.get("box_type", "2D Full"),
                    label="Box Type",
                    tag="combo_box_type",
                    callback=on_box_type_change
                )
                with dpg.tooltip("combo_box_type", tag="tooltip_box_type", show=show_tips):
                    dpg.add_text("Box dimensions and style")
                ...
                with dpg.tooltip("slider_box_thickness", tag="tooltip_box_thickness", show=show_tips):
                    dpg.add_text("Line thickness for boxes")
                ALL_TOOLTIP_TAGS.append("tooltip_box_thickness")

                dpg.add_color_edit(
                    label="Enemy Box Color",
                ...
                dpg.add_text("Snapline Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_line_esp", tag="tooltip_line_esp", show=show_tips):
                    dpg.add_text("Draw snaplines from the screen edge to players")
                ...
                with dpg.tooltip("slider_snapline_thickness", tag="tooltip_snapline_thickness", show=show_tips):
                    dpg.add_text("Snapline line thickness")
                ALL_TOOLTIP_TAGS.append("tooltip_snapline_thickness")

                dpg.add_color_edit(
                    label="Enemy Snapline Color",
                ...
                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Skeleton Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_skeleton_esp", tag="tooltip_skeleton_esp", show=show_tips):
                    dpg.add_text("Show player bones")
                ...
                with dpg.tooltip("slider_skeleton_thickness", tag="tooltip_skeleton_thickness", show=show_tips):
                    dpg.add_text("Skeleton line thickness")
                ALL_TOOLTIP_TAGS.append("tooltip_skeleton_thickness")

                dpg.add_color_edit(
                    label="Enemy Skeleton Color",
                ...
                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Username Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_name_esp", tag="tooltip_name_esp", show=show_tips):
                    dpg.add_text("Show player steam nicknames")
                ...
                with dpg.tooltip("slider_name_text_size", tag="tooltip_name_text_size", show=show_tips):
                    dpg.add_text("Font size of username labels")
                ALL_TOOLTIP_TAGS.append("tooltip_name_text_size")

                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Weapon Settings")
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Show Weapon", 
                    default_value=Active_Config.get("weapon_esp", False),
                    tag="chk_weapon_esp",
                    callback=on_weapon_esp_toggle
                )
                with dpg.tooltip("chk_weapon_esp", tag="tooltip_weapon_esp", show=show_tips):
                    dpg.add_text("Show a players actively held weapon")
                ALL_TOOLTIP_TAGS.append("tooltip_weapon_esp")

                dpg.add_slider_float(
                    label="Weapon Text Size",
                    default_value=Active_Config.get("weapon_text_size", 12.0),
                    min_value=8.0,
                    max_value=24.0,
                ...
                with dpg.tooltip("slider_weapon_text_size", tag="tooltip_weapon_text_size", show=show_tips):
                    dpg.add_text("Font size of weapon labels")
                ALL_TOOLTIP_TAGS.append("tooltip_weapon_text_size")

                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Distance Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_distance_esp", tag="tooltip_distance_esp", show=show_tips):
                    dpg.add_text("Show the distance from you to a player")
                ALL_TOOLTIP_TAGS.append("tooltip_distance_esp")

                dpg.add_slider_float(
                    label="Distance Text Size",
                    default_value=Active_Config.get("distance_text_size", 12.0),
                    min_value=8.0,
                    max_value=24.0,
                    tag="slider_distance_text_size",
                    callback=on_distance_text_size_change,
                    format="%.1f"
                )
                with dpg.tooltip("slider_distance_text_size", tag="tooltip_distance_text_size", show=show_tips):
                    dpg.add_text("Font size of distance labels")
                ALL_TOOLTIP_TAGS.append("tooltip_distance_text_size")

                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Health Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_health_bar", tag="tooltip_health_bar", show=show_tips):
                    dpg.add_text("Show health and armor values")
                ...
                with dpg.tooltip("combo_healthbar_type", tag="tooltip_healthbar_type", show=show_tips):
                    dpg.add_text("Appearance of health indicator")
                ...
                with dpg.tooltip("combo_health_position", tag="tooltip_health_position", show=show_tips):
                    dpg.add_text("Where the health indicator is located")
                ...
                with dpg.tooltip("slider_health_text_size", tag="tooltip_health_text_size", show=show_tips):
                    dpg.add_text("Font size of health labels")
                ALL_TOOLTIP_TAGS.append("tooltip_health_text_size")

                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Head Hitbox Settings")
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Show Head Hitbox", 
                ...
                with dpg.tooltip("chk_head_hitbox", tag="tooltip_head_hitbox", show=show_tips):
                    dpg.add_text("Render a sphere at head hitbox")
                ALL_TOOLTIP_TAGS.append("tooltip_head_hitbox")

                dpg.add_slider_float(
                    label="Head Hitbox Thickness",
                    default_value=Active_Config.get("head_hitbox_thickness", 2.0),
                    min_value=1.0,
                    max_value=5.0,
                    tag="slider_head_hitbox_thickness",
                    callback=on_head_hitbox_thickness_change,
                    format="%.1f"
                )
                with dpg.tooltip("slider_head_hitbox_thickness", tag="tooltip_head_hitbox_thickness", show=show_tips):
                    dpg.add_text("Head hitbox line thickness")
                ALL_TOOLTIP_TAGS.append("tooltip_head_hitbox_thickness")

                dpg.add_color_edit(
                    label="Head Hitbox Color",
                ...
                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Flashed Indicator Settings")
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Show Flashed Indicator",
                    default_value=Active_Config.get("flashed_indicator_enabled", False),
                    tag="chk_flashed_indicator",
                    callback=on_flashed_indicator_toggle
                )
                with dpg.tooltip("chk_flashed_indicator", tag="tooltip_flashed_indicator", show=show_tips):
                    dpg.add_text("Show player flashed status & duration")
                ALL_TOOLTIP_TAGS.append("tooltip_flashed_indicator")

                dpg.add_slider_float(
                    label="Flashed Indicator Text Size",
                    default_value=Active_Config.get("flashed_indicator_text_size", 14.0),
                    min_value=8.0,
                    max_value=24.0,
                    tag="slider_flashed_indicator_text_size",
                    callback=on_flashed_indicator_text_size_change,
                    format="%.1f"
                )
                with dpg.tooltip("slider_flashed_indicator_text_size", tag="tooltip_flashed_indicator_text_size", show=show_tips):
                    dpg.add_text("Font size of flashed indicator label")
                ALL_TOOLTIP_TAGS.append("tooltip_flashed_indicator_text_size")
            ...
            with dpg.tab(label="Radar"):
                ...
                with dpg.tooltip("chk_radar_enabled", tag="tooltip_radar_enabled", show=show_tips):
                    dpg.add_text("Show radar in overlay or external window")
                ALL_TOOLTIP_TAGS.append("tooltip_radar_enabled")

                dpg.add_checkbox(
                    label="External Mode",
                    default_value=Active_Config.get("radar_external_mode", False),
                    tag="chk_radar_external_mode",
                    callback=on_radar_external_mode_toggle
                )
                with dpg.tooltip("chk_radar_external_mode", tag="tooltip_radar_external_mode", show=show_tips):
                    dpg.add_text("Render radar in a separate standalone window instead of the game overlay")
                ALL_TOOLTIP_TAGS.append("tooltip_radar_external_mode")

                dpg.add_checkbox(
                    label="Show External Radar Additional Info",
                    default_value=Active_Config.get("radar_external_additional_info", False),
                    tag="chk_radar_external_additional_info",
                    callback=on_radar_external_additional_info_toggle
                )
                with dpg.tooltip("chk_radar_external_additional_info", tag="tooltip_radar_external_additional_info", show=show_tips):
                    dpg.add_text("Show weapon and health/armor text beside external radar blips")
                ALL_TOOLTIP_TAGS.append("tooltip_radar_external_additional_info")
                ...
                with dpg.tooltip("slider_radar_scale", tag="tooltip_radar_scale", show=show_tips):
                    dpg.add_text("How far radar can see")
                ...
                with dpg.tooltip("slider_radar_opacity", tag="tooltip_radar_opacity", show=show_tips):
                    dpg.add_text("Background transparency of radar")
                ...
                with dpg.tooltip("slider_radar_x", tag="tooltip_radar_x", show=show_tips):
                    dpg.add_text("X offset from screen center")
                ...
                with dpg.tooltip("slider_radar_y", tag="tooltip_radar_y", show=show_tips):
                    dpg.add_text("Y offset from screen center")
            ...
            # =========== OVERLAY SETTINGS SUBTAB ===========
            with dpg.tab(label="Settings"):
                show_tips = Active_Config.get("show_tooltips", True)

                dpg.add_text("Overlay Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_fps_cap_enabled", tag="tooltip_fps_cap_enabled", show=show_tips):
                    dpg.add_text("Limit overlay framerate")
                ...
                with dpg.tooltip("slider_fps_cap", tag="tooltip_fps_cap_slider", show=show_tips and fps_cap_enabled):
                ...
                dpg.add_checkbox(
                    ...
                    default_value=Active_Config.get("hide_on_tabout", False),
                ...
                with dpg.tooltip("chk_hide_on_tabout", tag="tooltip_hide_on_tabout", show=show_tips):
                    dpg.add_text("Automatically hide overlay when CS2 is unfocused")
                ...
                with dpg.tooltip("chk_show_status_labels", tag="tooltip_show_status_labels", show=show_tips):
                    dpg.add_text("Show feature status labels and toggle keys in the overlay")
    ...
    with dpg.tab(label="Aim"):
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Aimbot"):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_text("Aimbot Targeting Key:")
                    dpg.add_button(
                ...
                with dpg.tooltip("btn_bind_aimbot_cheat", tag="tooltip_aimbot_key", show=show_tips):
                    dpg.add_text("Bind to activate aimbot")
                    dpg.add_text("ESC to remove bind")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_key")

                # Require aimbot key
                dpg.add_checkbox(
                    label="Require Aimbot Targeting Key",
                ...
                with dpg.tooltip("btn_bind_aimbot_toggle_cheat", tag="tooltip_aimbot_toggle_key", show=show_tips):
                    dpg.add_text("Bind to toggle aimbot on/off")
                ...
                with dpg.tooltip("slider_aimbot_radius", tag="tooltip_aimbot_radius", show=show_tips):
                    dpg.add_text("Aimbot radius in pixels")
                ...
                with dpg.tooltip("slider_aimbot_deadzone_radius", tag="tooltip_aimbot_deadzone_radius", show=show_tips):
                    dpg.add_text("Deadzone radius in pixels")
                ...
                dpg.add_combo(
                    items=["Enemies Only", "All Players"],
                    default_value="All Players" if Active_Config.get("aimbot_targeting_type", 0) == 1 else "Enemies Only",
                    label="Target Team",
                    tag="combo_aimbot_targeting_type",
                    callback=on_aimbot_targeting_type_change
                )
                with dpg.tooltip("combo_aimbot_targeting_type", tag="tooltip_aimbot_targeting_type", show=show_tips):
                    dpg.add_text("Team for aimbot to target")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_targeting_type")
                ...
                dpg.add_combo(
                    ...
                    show=not Active_Config.get("aimbot_use_humanization", False)
                )
                with dpg.tooltip("combo_aimbot_movement_type", tag="tooltip_aimbot_movement_type", show=show_tips and not Active_Config.get("aimbot_use_humanization", False)):
                    dpg.add_text("How aimbot moves crosshair")
                ...
                with dpg.tooltip("slider_aimbot_smoothness", tag="tooltip_aimbot_smoothness", show=show_tips and not Active_Config.get("aimbot_use_humanization", False)):
                    dpg.add_text("Adjusts aimbot movement speed")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_smoothness")

                dpg.add_separator(tag="separator_aimbot_after_smoothness", show=not Active_Config.get("aimbot_use_humanization", False))

                # Visible check toggle
                dpg.add_checkbox(
                    label="Visible Check",
                    default_value=Active_Config.get("aimbot_visible_check", Active_Config.get("aimbot_spotted_check", False)),
                    callback=on_aimbot_visible_check_toggle,
                    tag="chk_aimbot_visible_check"
                )
                with dpg.tooltip("chk_aimbot_visible_check", tag="tooltip_aimbot_visible_check", show=show_tips):
                    dpg.add_text("Only aim at enemies visible to you")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_visible_check")

                dpg.add_checkbox(
                    label="Adjust X Axis",
                    default_value=Active_Config.get("aimbot_adjust_x_axis", True),
                    callback=on_aimbot_adjust_x_axis_toggle,
                    tag="chk_aimbot_adjust_x_axis"
                )
                with dpg.tooltip("chk_aimbot_adjust_x_axis", tag="tooltip_aimbot_adjust_x_axis", show=show_tips):
                    dpg.add_text("Allow the aimbot to move horizontally")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_adjust_x_axis")

                dpg.add_checkbox(
                    label="Adjust Y Axis",
                    default_value=Active_Config.get("aimbot_adjust_y_axis", True),
                    callback=on_aimbot_adjust_y_axis_toggle,
                    tag="chk_aimbot_adjust_y_axis"
                )
                with dpg.tooltip("chk_aimbot_adjust_y_axis", tag="tooltip_aimbot_adjust_y_axis", show=show_tips):
                    dpg.add_text("Allow the aimbot to move vertically")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_adjust_y_axis")
                ...
                with dpg.tooltip("chk_aimbot_lock_target", tag="tooltip_aimbot_lock_target", show=lock_target_show):
                    dpg.add_text("Target the same enemy until aimkey is released. (Prevents erratic target switching)")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_lock_target")
                
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Use Humanization",
                    default_value=Active_Config.get("aimbot_use_humanization", False),
                    callback=on_aimbot_use_humanization_toggle,
                    tag="chk_aimbot_use_humanization"
                )
                with dpg.tooltip("chk_aimbot_use_humanization", tag="tooltip_aimbot_use_humanization", show=show_tips):
                    dpg.add_text("Mimic human aiming behavior")
                    dpg.add_text("Movement Type and Smoothness are ignored while enabled")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_use_humanization")

                dpg.add_slider_float(
                    label="Humanization Strength",
                    default_value=Active_Config.get("aimbot_humanization_strength", 75.0),
                    min_value=0.0,
                    max_value=100.0,
                    tag="slider_aimbot_humanization_strength",
                    callback=on_aimbot_humanization_strength_change,
                    format="%.0f"
                )
                with dpg.tooltip("slider_aimbot_humanization_strength", tag="tooltip_aimbot_humanization_strength", show=show_tips):
                    dpg.add_text("Controls how strong the humanization features are")
                    dpg.add_text("0 = minimal effect, 100 = full humanization behavior")
                ALL_TOOLTIP_TAGS.append("tooltip_aimbot_humanization_strength")
                ...
                with dpg.tooltip("chk_aimbot_show_deadzone", tag="tooltip_aimbot_show_deadzone", show=show_tips):
                    dpg.add_text("Display deadzone radius")
                ...
                dpg.add_checkbox(
                    label="Crosshair Placement Indicator",
                ...
                with dpg.tooltip("chk_acs_enabled", tag="tooltip_acs_enabled", show=show_tips):
                    dpg.add_text("Show a visual horizontal indicator at the selected target bone")
                ...
                with dpg.tooltip("color_acs_line", tag="tooltip_acs_line_color", show=show_tips):
                    dpg.add_text("Color of the crosshair placement indicator")
                ALL_TOOLTIP_TAGS.append("tooltip_acs_line_color")
                ...
                with dpg.tooltip("slider_acs_line_transparency", tag="tooltip_acs_line_transparency", show=show_tips):
                    dpg.add_text("Transparency of the indicator")
                ...
                with dpg.tooltip("slider_acs_line_width", tag="tooltip_acs_line_width", show=show_tips):
                    dpg.add_text("Width of the crosshair placement indicator")
                ALL_TOOLTIP_TAGS.append("tooltip_acs_line_width")
            
            # Triggerbot sub-tab
            with dpg.tab(label="Triggerbot"):
                dpg.add_text("Universal Triggerbot Settings")
                dpg.add_separator()
                ...
                with dpg.tooltip("btn_bind_triggerbot_cheat", tag="tooltip_triggerbot_key", show=show_tips):
                    dpg.add_text("Bind to activate triggerbot")
                    dpg.add_text("ESC to remove bind")
                ALL_TOOLTIP_TAGS.append("tooltip_triggerbot_key")

                dpg.add_combo(
                    items=["Enemies Only", "All Players"],
                    default_value="All Players" if Active_Config.get("triggerbot_targeting_type", 0) == 1 else "Enemies Only",
                    label="Target Team",
                    tag="combo_triggerbot_targeting_type",
                    callback=on_triggerbot_targeting_type_change
                )
                with dpg.tooltip("combo_triggerbot_targeting_type", tag="tooltip_triggerbot_targeting_type", show=show_tips):
                    dpg.add_text("Triggerbot target filtering")
                ALL_TOOLTIP_TAGS.append("tooltip_triggerbot_targeting_type")
                ...
                with dpg.tooltip("slider_triggerbot_first_shot_delay", tag="tooltip_triggerbot_first_shot_delay", show=show_tips):
                    dpg.add_text("Delay before the first shot fires after a trigger press")
                ALL_TOOLTIP_TAGS.append("tooltip_triggerbot_first_shot_delay")

                dpg.add_slider_float(
                    label="First Shot Cooldown (s)",
                    default_value=Active_Config.get("triggerbot_first_shot_cooldown", 0.0),
                    min_value=0.0,
                    max_value=5.0,
                    tag="slider_triggerbot_first_shot_cooldown",
                    callback=on_triggerbot_first_shot_cooldown_change
                )
                with dpg.tooltip("slider_triggerbot_first_shot_cooldown", tag="tooltip_triggerbot_first_shot_cooldown", show=show_tips):
                    dpg.add_text("How long a target can be lost before first-shot delay is re-applied")
                ALL_TOOLTIP_TAGS.append("tooltip_triggerbot_first_shot_cooldown")
                
                dpg.add_separator()
                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Preset-Specific Triggerbot Settings")

                # Weapon preset dropdown
                dpg.add_combo(
                    label="Weapon Preset",
                    items=get_triggerbot_dropdown_items(),
                    default_value=Active_Config.get("current_triggerbot_preset", "Default"),
                ...
                with dpg.tooltip("combo_triggerbot_preset", tag="tooltip_triggerbot_preset", show=show_tips):
                    dpg.add_text("Settings below will save to selected preset")
                ALL_TOOLTIP_TAGS.append("tooltip_triggerbot_preset")

                dpg.add_checkbox(
                    label="Auto Change Weapon Preset",
                ...
                with dpg.tooltip("slider_triggerbot_burst_shots", tag="tooltip_triggerbot_burst_shots", show=show_tips):
            ...
            with dpg.tab(label="Recoil Control"):
                show_tips = Active_Config.get("show_tooltips", True)
                dpg.add_text("Recoil Control System")
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Enable Recoil Control",
                    default_value=Active_Config.get("rcs_enabled", False),
                    callback=on_rcs_toggle,
                    tag="chk_rcs_enabled"
                )
                with dpg.tooltip("chk_rcs_enabled", tag="tooltip_rcs_enabled", show=show_tips):
                    dpg.add_text("Compensate weapon recoil while shooting")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_enabled")
                ...
                dpg.add_checkbox(
                    ...
                    default_value=Active_Config.get("rcs_horizontal_compensation", True),
                    callback=on_rcs_horizontal_compensation_toggle,
                    tag="chk_rcs_horizontal_compensation"
                )
                with dpg.tooltip("chk_rcs_horizontal_compensation", tag="tooltip_rcs_horizontal_compensation", show=show_tips):
                    dpg.add_text("Compensate recoil on the horizontal axis")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_horizontal_compensation")

                dpg.add_checkbox(
                    label="Vertical Compensation",
                    default_value=Active_Config.get("rcs_vertical_compensation", True),
                    callback=on_rcs_vertical_compensation_toggle,
                    tag="chk_rcs_vertical_compensation"
                )
                with dpg.tooltip("chk_rcs_vertical_compensation", tag="tooltip_rcs_vertical_compensation", show=show_tips):
                    dpg.add_text("Compensate recoil on the vertical axis")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_vertical_compensation")

                dpg.add_slider_int(
                    label="Horizontal Compensation %",
                    default_value=Active_Config.get("rcs_horizontal_compensation_percentage", 100),
                    min_value=1,
                    max_value=100,
                    tag="slider_rcs_horizontal_compensation_percentage",
                    callback=on_rcs_horizontal_compensation_percentage_change
                )
                with dpg.tooltip("slider_rcs_horizontal_compensation_percentage", tag="tooltip_rcs_horizontal_compensation_percentage", show=show_tips):
                    dpg.add_text("How much of horizontal recoil is compensated")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_horizontal_compensation_percentage")

                dpg.add_slider_int(
                    label="Vertical Compensation %",
                    default_value=Active_Config.get("rcs_vertical_compensation_percentage", 100),
                    min_value=1,
                    max_value=100,
                    tag="slider_rcs_vertical_compensation_percentage",
                    callback=on_rcs_vertical_compensation_percentage_change
                )
                with dpg.tooltip("slider_rcs_vertical_compensation_percentage", tag="tooltip_rcs_vertical_compensation_percentage", show=show_tips):
                    dpg.add_text("How much of vertical recoil is compensated")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_vertical_compensation_percentage")

                dpg.add_separator()

                dpg.add_combo(
                    label="Smoothing Technique",
                    items=["Softened", "Instant"],
                    default_value=Active_Config.get("rcs_smoothing_technique", "Instant"),
                    callback=on_rcs_smoothing_technique_change,
                    tag="combo_rcs_smoothing_technique"
                )
                with dpg.tooltip("combo_rcs_smoothing_technique", tag="tooltip_rcs_smoothing_technique", show=show_tips):
                    dpg.add_text("Softened = slightly delayed, Instant = no delay")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_smoothing_technique")

                dpg.add_slider_float(
                    label="Horizontal Smoothing",
                    default_value=Active_Config.get("rcs_horizontal_smoothing", 0.0),
                    min_value=0.0,
                    max_value=10.0,
                    tag="slider_rcs_horizontal_smoothing",
                    callback=on_rcs_horizontal_smoothing_change,
                    format="%.1f"
                )
                with dpg.tooltip("slider_rcs_horizontal_smoothing", tag="tooltip_rcs_horizontal_smoothing", show=show_tips):
                    dpg.add_text("How smooth horizontal compensation is")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_horizontal_smoothing")

                dpg.add_slider_float(
                    label="Vertical Smoothing",
                    default_value=Active_Config.get("rcs_vertical_smoothing", 0.0),
                    min_value=0.0,
                    max_value=10.0,
                    tag="slider_rcs_vertical_smoothing",
                    callback=on_rcs_vertical_smoothing_change,
                    format="%.1f"
                )
                with dpg.tooltip("slider_rcs_vertical_smoothing", tag="tooltip_rcs_vertical_smoothing", show=show_tips):
                    dpg.add_text("How smooth vertical compensation is")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_vertical_smoothing")
                ...
                dpg.add_checkbox(
                    label="Recoil Control Visualizer",
                    default_value=Active_Config.get("rcs_visualizer_enabled", False),
                    callback=on_rcs_visualizer_toggle,
                    tag="chk_rcs_visualizer_enabled"
                )
                with dpg.tooltip("chk_rcs_visualizer_enabled", tag="tooltip_rcs_visualizer", show=show_tips):
                    dpg.add_text("Shows direction to move mouse to control recoil")
                    dpg.add_text("Displays only while firing")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_visualizer")

                dpg.add_color_edit(
                    label="Recoil Visualizer Color",
                    default_value=get_color_for_picker("rcs_visualizer_color", (255, 200, 0)),
                    no_alpha=True,
                    callback=on_rcs_visualizer_color_change,
                    tag="color_rcs_visualizer_color"
                )
                with dpg.tooltip("color_rcs_visualizer_color", tag="tooltip_rcs_visualizer_color", show=show_tips):
                    dpg.add_text("Set the line and endpoint color used by the recoil visualizer")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_visualizer_color")

                dpg.add_slider_int(
                    label="Recoil Visualizer Transparency",
                    default_value=Active_Config.get("rcs_visualizer_transparency", 255),
                    min_value=0,
                    max_value=255,
                    tag="slider_rcs_visualizer_transparency",
                    callback=on_rcs_visualizer_transparency_change
                )
                with dpg.tooltip("slider_rcs_visualizer_transparency", tag="tooltip_rcs_visualizer_transparency", show=show_tips):
                    dpg.add_text("Opacity of the recoil visualizer (0 = invisible, 255 = solid)")
                ALL_TOOLTIP_TAGS.append("tooltip_rcs_visualizer_transparency")
...
def create_colors_tab():
    ...
    with dpg.tab(label="Menu"):
        ...
        dpg.add_checkbox(
            ...
            tag="chk_rounded_corners_cheat",
            default_value=Active_Config.get("rounded_window_corners", True),
            callback=on_rounded_corners_changed
        )
        ...
        dpg.add_button(
            label="Unload Cheat",
            callback=on_unload_clicked,
            width=150,
            tag="btn_unload_cheat"
        )
        with dpg.tooltip("btn_unload_cheat", tag="tooltip_unload_cheat", show=show_tips):
            dpg.add_text("Stop the cheat runtime and return to the loader")
        ALL_TOOLTIP_TAGS.append("tooltip_unload_cheat")
...
def initialize_color_pickers():
    ...
    Missing or removed widgets should be skipped silently.
    """
...
def create_debug_tab():
    ...
    def _is_performance_subtab_value(value):
        """Return True when a debug subtab value points to the Performance tab."""
        if value == "tab_performance":
            return True
        ...
        try:
            return dpg.get_item_label(value) == "Performance"
        except Exception:
            return False

    def on_debug_subtab_changed(sender, app_data):
        """Track currently selected debug subtab for reliable update gating."""
        try:
            app_state["active_debug_subtab"] = "tab_performance" if _is_performance_subtab_value(app_data) else app_data
        except Exception:
            pass

    with dpg.tab(label="Debug"):
        # Nested tab bar for sub-tabs
        with dpg.tab_bar(tag="debug_subtabs", callback=on_debug_subtab_changed):
            # Output sub-tab with mini-terminal
            with dpg.tab(label="Output", tag="tab_debug_output"):
            ...
            # Offsets sub-tab
            with dpg.tab(label="Offsets", tag="tab_debug_offsets"):
...
def _read_memory_value_from_address(pm, address, preferred_readers=None):
    """Try a few common read styles and return the first successful value."""
    if not pm or address is None:
        return None

    readers = preferred_readers or [
        pm.read_int,
        pm.read_uint,
        pm.read_short,
        pm.read_ushort,
...
def _get_local_player_pawn_address(pm, client):
    """Return the current local player pawn address from shared memory."""
    try:
        if not pm or not client or dwLocalPlayerPawn is None:
            return None
        return pm.read_longlong(client + dwLocalPlayerPawn)
    except Exception:
        return None


def _get_local_player_active_weapon_address(pm, client, player_pawn):
    """Return the active weapon entity address for the local player."""
    try:
        if not pm or not client or not player_pawn:
            return None
...
def _resolve_clicked_offset_value(label, offset_value, class_name=None, field_name=None):
    """Try to resolve a clicked offset entry to its live memory value."""
    pm, client = get_shared_memory_connection()
    if not pm or not client:
        return None

    # Handle client-dll class fields (for example C_BasePlayerWeapon.m_iClip1)
    if class_name and field_name:
        field_name_lower = field_name.lower()
        base_address = None

        if field_name_lower in {"m_iclip1", "m_iclip1", "m_ihealth", "m_iteamnum", "m_lifestate", "m_fflags", "m_iarmorvalue", "m_iidentindex", "m_hactiveweapon"}:
            base_address = _get_local_player_pawn_address(pm, client)

        if field_name_lower == "m_iclip1":
            weapon_address = _get_local_player_active_weapon_address(pm, client, base_address)
            if weapon_address is not None:
                return _read_memory_value_from_address(
                    pm,
                    weapon_address + offset_value,
                    preferred_readers=[pm.read_int, pm.read_uint, pm.read_short, pm.read_ushort],
                )
        ...
        if base_address is not None:
            return _read_memory_value_from_address(
                ...
                preferred_readers=[pm.read_int, pm.read_uint, pm.read_short, pm.read_ushort, pm.read_float, pm.read_bool],
            )

    # Handle core offsets like dwLocalPlayerPawn / dwEntityList / dwViewMatrix
    if isinstance(offset_value, int):
        try:
            return _read_memory_value_from_address(
                pm,
                client + offset_value,
                preferred_readers=[pm.read_ulonglong, pm.read_longlong, pm.read_uint, pm.read_int, pm.read_float],
            )
        except Exception:
            return None
...
def on_offset_entry_clicked(sender, app_data, user_data):
    """Log a clicked offset entry to the built-in debug console."""
    try:
        if user_data is None:
            return

        if isinstance(user_data, tuple) and len(user_data) >= 3 and user_data[0] == "main_offset":
            _, label, value = user_data
            resolved_value = _resolve_clicked_offset_value(label, value)
            if resolved_value is None:
                debug_log(f"Offset clicked: {label} = {value} (unavailable)", "INFO")
            elif isinstance(resolved_value, float):
                debug_log(f"Offset clicked: {label} = {resolved_value}", "INFO")
            else:
                debug_log(f"Offset clicked: {label} = {resolved_value}", "INFO")
            return

        if isinstance(user_data, tuple) and len(user_data) >= 4 and user_data[0] == "class_field":
            _, class_name, field_name, value = user_data
            resolved_value = _resolve_clicked_offset_value(f"{class_name}.{field_name}", value, class_name=class_name, field_name=field_name)
            if resolved_value is None:
                debug_log(f"Offset clicked: {class_name}.{field_name} = {value} (unavailable)", "INFO")
            else:
                debug_log(f"Offset clicked: {class_name}.{field_name} = {resolved_value}", "INFO")
            return

        label, value = user_data
        if isinstance(value, int):
            debug_log(f"Offset clicked: {label} = {value}", "INFO")
        else:
            debug_log(f"Offset clicked: {label} = {value}", "INFO")
    except Exception as e:
        debug_log(f"Failed to log offset click: {str(e)}", "ERROR")
...
def create_offsets_content():
    ...
    if offsets and client_dll:
        ...
        for key, value in offsets.get('client.dll', {}).items():
            display_value = f"{key}: 0x{value:X}" if isinstance(value, int) else f"{key}: {value}"
            dpg.add_selectable(
                label=display_value,
                callback=on_offset_entry_clicked,
                user_data=("main_offset", key, value),
                indent=8,
            )
        ...
        for class_name, class_data in classes.items():
            ...
            if fields:
                for field_name, field_value in fields.items():
                    display_value = f"  {field_name}: 0x{field_value:X}" if isinstance(field_value, int) else f"  {field_name}: {field_value}"
                    dpg.add_selectable(
                        label=display_value,
                        callback=on_offset_entry_clicked,
                        user_data=("class_field", class_name, field_name, field_value),
                        indent=16,
                    )
    else:
...
def get_process_cpu_percent_windows():
    """Return a stable CPU percentage for the current Python process on Windows."""
    try:
        if not hasattr(get_process_cpu_percent_windows, "_last_total_cpu_ns"):
            get_process_cpu_percent_windows._last_total_cpu_ns = None
            get_process_cpu_percent_windows._last_sample_time_ns = None
            get_process_cpu_percent_windows._last_percent = 0.0

        process_handle = ctypes.windll.kernel32.OpenProcess(0x0400, False, os.getpid())
        if not process_handle:
            return get_process_cpu_percent_windows._last_percent
        ...
        try:
            ...
            total_cpu_ns = (kernel_time.value + user_time.value) * 100
            now_ns = time.perf_counter_ns()

            if get_process_cpu_percent_windows._last_total_cpu_ns is None:
                get_process_cpu_percent_windows._last_total_cpu_ns = total_cpu_ns
                get_process_cpu_percent_windows._last_sample_time_ns = now_ns
                return 0.0

            elapsed_ns = now_ns - get_process_cpu_percent_windows._last_sample_time_ns
            if elapsed_ns <= 0:
                return get_process_cpu_percent_windows._last_percent

            delta_cpu_ns = max(0, total_cpu_ns - get_process_cpu_percent_windows._last_total_cpu_ns)
            logical_cores = max(1, os.cpu_count() or 1)
            cpu_percent = (delta_cpu_ns / elapsed_ns) * 100.0 / logical_cores
            cpu_percent = min(100.0, max(0.0, cpu_percent))

            get_process_cpu_percent_windows._last_total_cpu_ns = total_cpu_ns
            get_process_cpu_percent_windows._last_sample_time_ns = now_ns
            get_process_cpu_percent_windows._last_percent = cpu_percent
            return cpu_percent
        finally:
            try:
...
def update_performance_display():
    ...
    try:
        ...
        # Process CPU usage percentage (Windows process-time based)
        process_cpu_percent = get_process_cpu_percent_windows()
        ...
        # Network Stats (throttled to reduce overhead)
        now = time.monotonic()
        if not hasattr(update_performance_display, "_last_network_check"):
            update_performance_display._last_network_check = 0.0
            update_performance_display._last_network_status = None
            update_performance_display._last_network_color = None

        if now - update_performance_display._last_network_check >= 3.0:
            update_performance_display._last_network_check = now
            try:
                import socket
            ...
            update_performance_display._last_network_status = status
            update_performance_display._last_network_color = color

        if update_performance_display._last_network_status is not None:
            dpg.set_value("perf_network_status", update_performance_display._last_network_status)
            dpg.configure_item("perf_network_status", color=update_performance_display._last_network_color)
...
def create_settings_tab_cheat():
    ...
    with dpg.tab(label="Miscellaneous"):
        ...
        dpg.add_checkbox(
            ...
            default_value=Active_Config.get("show_spectator_list_overlay", False),
            tag="chk_show_spectator_list",
            callback=on_show_spectator_list_toggle
        )
        with dpg.tooltip("chk_show_spectator_list", tag="tooltip_show_spectator_list", show=show_tips):
            dpg.add_text("Shows a draggable overlay window listing players currently spectating you")
            dpg.add_text("Auto-refreshes while CS2 is running")
        ALL_TOOLTIP_TAGS.append("tooltip_show_spectator_list")

        dpg.add_text("Spectator Protections:")
        dpg.add_checkbox(
            label="Hide ESP",
            default_value=Active_Config.get("hide_while_spectated", False),
            tag="chk_hide_while_spectated",
            callback=on_hide_while_spectated_toggle
        )
        with dpg.tooltip("chk_hide_while_spectated", tag="tooltip_hide_while_spectated", show=show_tips):
            dpg.add_text("Hide ESP while spectators are present")
            dpg.add_text("ESP returns once the spectator count reaches zero")
        ALL_TOOLTIP_TAGS.append("tooltip_hide_while_spectated")

        dpg.add_checkbox(
            label="Disable Anti-Flash",
            default_value=Active_Config.get("spectator_protections_disable_anti_flash", False),
            tag="chk_spectator_disable_anti_flash",
            callback=on_spectator_disable_anti_flash_toggle
        )
        with dpg.tooltip("chk_spectator_disable_anti_flash", tag="tooltip_spectator_disable_anti_flash", show=show_tips):
            dpg.add_text("Temporarily disable Anti-Flash while spectators are present")
        ALL_TOOLTIP_TAGS.append("tooltip_spectator_disable_anti_flash")

        dpg.add_checkbox(
            label="Disable Aimbot",
            default_value=Active_Config.get("spectator_protections_disable_aimbot", False),
            tag="chk_spectator_disable_aimbot",
            callback=on_spectator_disable_aimbot_toggle
        )
        with dpg.tooltip("chk_spectator_disable_aimbot", tag="tooltip_spectator_disable_aimbot", show=show_tips):
            dpg.add_text("Temporarily disable Aimbot while spectators are present")
        ALL_TOOLTIP_TAGS.append("tooltip_spectator_disable_aimbot")

        dpg.add_checkbox(
            label="Disable Recoil Control",
            default_value=Active_Config.get("spectator_protections_disable_rcs", False),
            tag="chk_spectator_disable_rcs",
            callback=on_spectator_disable_rcs_toggle
        )
        with dpg.tooltip("chk_spectator_disable_rcs", tag="tooltip_spectator_disable_rcs", show=show_tips):
            dpg.add_text("Temporarily disable RCS while spectators are present")
        ALL_TOOLTIP_TAGS.append("tooltip_spectator_disable_rcs")
        ...
        dpg.add_checkbox(
            label="Flashed Indicator",
            default_value=Active_Config.get("local_flashed_indicator_enabled", False),
            tag="chk_local_flashed_indicator",
            callback=on_local_flashed_indicator_toggle
        )
        with dpg.tooltip("chk_local_flashed_indicator", tag="tooltip_local_flashed_indicator", show=show_tips):
            dpg.add_text("Show FLASHED and the remaining duration while you are flashed")
        ALL_TOOLTIP_TAGS.append("tooltip_local_flashed_indicator")
        ...
        dpg.add_checkbox(
            label="Apply While Scoped",
            default_value=Active_Config.get("fov_apply_while_scoped", False),
            tag="chk_fov_apply_while_scoped",
            callback=on_fov_apply_while_scoped_toggle,
            show=True,
            enabled=True
        )
        with dpg.tooltip("chk_fov_apply_while_scoped", tag="tooltip_fov_apply_while_scoped", show=show_tips):
            dpg.add_text("If enabled, FOV changes are applied even while scoped")
            dpg.add_text("If disabled, scoped zoom is preserved")
        ALL_TOOLTIP_TAGS.append("tooltip_fov_apply_while_scoped")
        ...
        with dpg.tooltip("chk_bhop_enabled", tag="tooltip_bhop", show=show_tips):
            dpg.add_text("Auto-jump while holding the selected key")
            dpg.add_text("Works only when CS2 is the active window")
        ALL_TOOLTIP_TAGS.append("tooltip_bhop")

        with dpg.group(horizontal=True):
            dpg.add_text("Bhop Key:")
            dpg.add_button(
                label=f"{Keybinds_Config.get('bhop_key', 'space').upper() or 'NONE'}",
        ...
        with dpg.tooltip("btn_bind_bhop_cheat", tag="tooltip_bhop_key", show=show_tips):
            dpg.add_text("Key or mouse button to trigger bunnyhop")
        ...
        dpg.add_checkbox(
            label="Snipers Only",
            default_value=Active_Config.get("custom_crosshair_snipers_only", False),
            tag="chk_custom_crosshair_snipers_only",
            callback=on_custom_crosshair_snipers_only_toggle
        )
        with dpg.tooltip("chk_custom_crosshair_snipers_only", tag="tooltip_custom_crosshair_snipers_only", show=show_tips):
            dpg.add_text("Show the custom crosshair only while holding AWP or SSG 08")
            dpg.add_text("and while not scoped")
        ALL_TOOLTIP_TAGS.append("tooltip_custom_crosshair_snipers_only")
        ...
        crosshair_scale_show = show_tips
        ...
        crosshair_thickness_show = show_tips
        ...
        crosshair_shape_show = show_tips
        ...
        crosshair_targeting_show = show_tips
        ...
        dpg.add_combo(
            ...
            show=True,
            enabled=hitsound_enabled
        )
        hitsound_combo_show = show_tips
...
def _update_loading_overlay(progress_value, status_text):
    """Safely update the loading overlay when the UI items already exist."""
    try:
        if dpg.does_item_exist("loading_progress"):
            dpg.set_value("loading_progress", progress_value)
...
def create_loading_overlay(window_type="loader"):
    """Create a modal overlay that blocks interaction until the cheat UI is fully built."""
    ...
    with dpg.window(
        ...
        dpg.add_text("Initializing UI...", tag="loading_title")
        dpg.add_spacer(height=6)
        dpg.add_text("Please wait while the menu finishes loading.", tag="loading_message")
        dpg.add_spacer(height=10)
        dpg.add_progress_bar(tag="loading_progress", default_value=0.0, width=260)
        dpg.add_spacer(height=8)
        dpg.add_text("Preparing interface...", tag="loading_status")


def finish_ui_loading(window_type="loader"):
    """Remove the loading gate and enable the UI once the initial build is complete."""
    app_state["ui_initialization_complete"] = True

    try:
        if dpg.does_item_exist("loading_status"):
            dpg.set_value("loading_status", "Ready")
        if dpg.does_item_exist("loading_progress"):
    ...
    if window_type == "cheat":
        start_cheat_feature_threads()


def start_cheat_feature_threads():
    """Start cheat runtime threads after the UI has fully loaded."""
    if not app_state.get("ui_initialization_complete", False):
        return
    ...
    if Active_Config.get("acs_enabled", False):
        start_acs_thread()
    if Active_Config.get("rcs_enabled", False):
        start_rcs_thread()
    update_spectator_protections()
...
def create_main_window(title, window_type="loader"):
    ...
    with dpg.window(label="Main Window", tag="main_window", no_title_bar=True):
        create_loading_overlay(window_type)
        ...
        with dpg.tab_bar():
            if window_type == "loader":
                ...
                # Cheat window has ESP controls, aimbot controls, and settings
                create_esp_tab()
                create_aimbot_tab()
...
def apply_window_styles(title):
    ...
    if hwnd:
        ...
        # Loader window should always use rounded corners on Windows 11
        if "loader" in title.lower():
            enable_rounded_corners(hwnd)
        else:
            # Cheat window follows user setting
            if Active_Config.get("rounded_window_corners", True):
                enable_rounded_corners(hwnd)
            else:
...
def cleanup_current_window():
    """Hide and destroy the previous viewport window after its loop has ended."""
    hwnd_candidates = []
    current_hwnd = drag_state.get("hwnd")
    if current_hwnd:
        hwnd_candidates.append(current_hwnd)

    app_title = app_state.get("app_title") or get_app_title()
    candidate_titles = []
    if app_title:
        candidate_titles.extend([app_title, f"{app_title} - Loader"])
    candidate_titles.extend(["loader", "cheat"])

    for title in candidate_titles:
        try:
            hwnd = win32gui.FindWindow(None, title)
            if hwnd and hwnd not in hwnd_candidates:
...
def request_window_switch(window_type="cheat"):
    """Request a controlled transition to the requested window type."""
    app_state["pending_window_transition"] = True
    app_state["requested_window"] = window_type
    app_state["switch_to_cheat"] = (window_type == "cheat")
    app_state["switch_to_loader"] = (window_type == "loader")
...
def run_window(window_type="loader"):
    ...
    dpg.create_context()
    app_state["ui_initialization_complete"] = False
    ...
    # Finalize the loading gate and only then start cheat features
    if window_type == "cheat":
        finish_ui_loading(window_type)
    
    # FPS update counter for cheat window
    fps_update_counter = 0

    # If requested, auto-resume the loader launch flow and wait for CS2 restart.
    auto_resume_wait_for_cs2 = (
        window_type == "loader" and app_state.get("resume_loader_wait_for_cs2", False)
    )
    ...
    spectator_protection_check_counter = 0
    cs2_check_counter = 0
    ...
    while dpg.is_dearpygui_running():
        if auto_resume_wait_for_cs2:
            auto_resume_wait_for_cs2 = False
            app_state["resume_loader_wait_for_cs2"] = False
            try:
                on_test_clicked()
            except Exception as e:
                debug_log(f"Failed to auto-resume loader launch flow: {str(e)}", "ERROR")
        ...
        if window_type == "cheat" and not app_state.get("ui_initialization_complete", False):
            # Keep the loading gate active until the UI has fully finished building.
            app_state["ui_initialization_complete"] = True
            continue
        ...
        if window_type == "cheat":
            try:
                performance_tab_active = app_state.get("active_debug_subtab") == "tab_performance"
                if dpg.does_item_exist("debug_subtabs"):
                    selected_subtab = dpg.get_value("debug_subtabs")
                    if selected_subtab == "tab_performance":
                        performance_tab_active = True
                    elif isinstance(selected_subtab, str):
                        performance_tab_active = selected_subtab.strip().lower() == "performance"
                    else:
                        try:
                            performance_tab_active = dpg.get_item_label(selected_subtab) == "Performance"
                        except Exception:
                            pass

                    if performance_tab_active:
                        app_state["active_debug_subtab"] = "tab_performance"
                ...
                spectator_protection_check_counter += 1
                if spectator_protection_check_counter >= 30:
                    update_spectator_protections()
                    spectator_protection_check_counter = 0
            except:
                pass
        ...
        if window_type == "cheat":
            ...
            if cs2_check_counter >= 60:  # Check every ~60 frames
                ...
                if not is_cs2_running():
                    title = app_state.get("app_title") or FALLBACK_TITLE
                    hwnd = drag_state["hwnd"] if drag_state.get("hwnd") else 0

                    # Close/hide the cheat window before showing the restart prompt.
                    if hwnd:
                        try:
                            win32gui.ShowWindow(hwnd, win32con.SW_HIDE)
                            app_state["cheat_window_visible"] = False
                        except Exception:
                            pass

                    result = ctypes.windll.user32.MessageBoxW(
                        hwnd,
                        "CS2 Process ended. Would you like to wait for restart?",
                        title,
                        0x00000004 | 0x00000020 | 0x00040000  # MB_YESNO | MB_ICONQUESTION | MB_TOPMOST
                    )

                    if result == 6:  # IDYES
                        debug_log("CS2 closed - returning to loader and waiting for restart", "INFO")
                        app_state["resume_loader_wait_for_cs2"] = True
                        request_window_switch("loader")
                    else:
            ...
            if foreground_check_counter >= 10:
                ...
                if cs2_is_foreground and not cs2_was_foreground:
                    ...
                    if menu_hidden_by_auto and window_visible:
                        ...
                        app_state["cheat_window_visible"] = True
                    cs2_was_foreground = True
            ...
            if menu_key != "none":
                ...
                if key_pressed and not key_was_pressed:
                    ...
                    if window_visible:
                        ...
                        win32gui.ShowWindow(drag_state["hwnd"], win32con.SW_HIDE)
                    app_state["cheat_window_visible"] = window_visible
        ...
        if window_type == "cheat" and not keybind_listener["listening"]:
            ...
            if esp_key != "none":
                ...
                if esp_key_pressed and not run_window.esp_key_was_pressed:
                    # Key just pressed - toggle ESP using the same logic as the checkbox
                    ...
                    try:
                        ...
                        pass
                    on_esp_toggle(None, new_esp_state)
            ...
            if aimbot_toggle_key and aimbot_toggle_key != "none":
                ...
                if aimbot_toggle_vk_code:
                    ...
                    if aimbot_toggle_key_pressed and not run_window.aimbot_toggle_key_was_pressed:
                        # Key just pressed - toggle aimbot using the same logic as the checkbox
                        ...
                        try:
                            ...
                            pass
                        on_aimbot_toggle(None, new_aimbot_state)
        ...
        # Exit Key handling is available only in the cheat window.
        if window_type == "cheat":
    ...
    # Clean up the old viewport only after the current loop has exited.
    cleanup_current_window()
    
    # Cleanup
    dpg.destroy_context()
    
    # Return the requested next window target after a transition.
    # If the window closed normally without a transition request, return None.
    if app_state.get("pending_window_transition", False):
        return app_state.get("requested_window")
    return None
...
def initialize_config_folders():
    ...
    Creates the main settings and custom sounds folders if they don't exist.
    Keybind data is persisted inside the settings payload instead of a separate folder.
    Called at application startup.
    """
    try:
...
def cleanup_temp_folder():
    ...
    """
    # This function is called both explicitly after the window loop and by
    # atexit. Each cleanup step must be independent so a failed optional
    # shutdown operation cannot prevent removal of TEMP_FOLDER.
    for cleanup_step in (
        save_settings,
        save_keybinds,
        stop_esp_overlay,
    ...
    # Windows can briefly retain handles while worker threads and overlay
    # windows finish shutting down. Retry the removal after the UI loop has
    # stopped, which covers both standalone and launcher-launched execution.
    for _ in range(5):
...
def check_runtime_compatibility():
    """Final compatibility guard before starting the app runtime."""
    if os.name != "nt":
        print("[COMPAT] Unsupported OS. This script requires Windows.")
        sys.exit(1)

    major = sys.version_info.major
    minor = sys.version_info.minor
    if major != SUPPORTED_PYTHON_MAJOR or minor not in SUPPORTED_PYTHON_MINORS:
        print(f"[COMPAT] Incompatible Python version: {major}.{minor}")
        print("[COMPAT] Supported version: 3.11")
        sys.exit(1)

    if not GLFW_AVAILABLE:
        print(f"[COMPAT] Missing or broken GPU overlay dependencies: {GLFW_IMPORT_ERROR}")
        print("[COMPAT] Re-run setup and ensure these packages are installed: glfw imgui[glfw] PyOpenGL PyOpenGL_accelerate")
        sys.exit(1)


def perform_graphics_reset():
    """Trigger Windows graphics driver reset safely without crashing app flow."""
    # Prefer Win32 key events; this avoids PyAutoGUI fail-safe interruptions.
    try:
        key_sequence = [win32con.VK_CONTROL, win32con.VK_SHIFT, win32con.VK_LWIN, ord('B')]

        for vk_code in key_sequence:
            win32api.keybd_event(vk_code, 0, 0, 0)
        for vk_code in reversed(key_sequence):
            win32api.keybd_event(vk_code, 0, win32con.KEYEVENTF_KEYUP, 0)

        debug_log("Graphics reset hotkey sent via Win32", "INFO")
        return True
    except Exception as e:
        debug_log(f"Win32 graphics reset failed: {str(e)}", "WARNING")

    # Fallback to PyAutoGUI; temporarily disable fail-safe for this single chord.
    try:
        ...
        debug_log("Graphics reset hotkey sent via PyAutoGUI fallback", "INFO")
        return True
    except Exception as e:
        debug_log(f"PyAutoGUI graphics reset failed: {str(e)}", "ERROR")
        return False


def resolve_startup_config_path(args):
    """
    Resolve optional --config argument into an absolute settings-file path.

    The launcher may pass --config <file>. We try both expected loader layouts
    to remain compatible with existing launch/batch scripts.
    """
    if len(args) <= 2 or args[1] != "--config":
        return None
    ...
    candidate_paths = (
        os.path.join(loader_dir, "configs", "Settings", config_name),
        os.path.join(loader_dir, "configs", config_name),
    )
    ...
    debug_log(f"Specified config file not found: {candidate_paths[0]}", "ERROR")
    return None


def download_skip_loader_sound_pack():
    """Download the default sound pack used by SKIP_LOADER startup mode."""
    sound_files = [
        (HITMARKER_SOUND_URL, "hitmarker.wav"),
        (METALHIT_SOUND_URL, "metalhit.wav"),
        (CRITICALHIT_SOUND_URL, "criticalhit.wav"),
...
def main():
    ...
    """
    # Final runtime compatibility check after setup/launcher checks.
    ...
    # Detect first launch if the main config folder is missing.
    global FIRST_LAUNCH
    FIRST_LAUNCH = not os.path.exists(CONFIGS_FOLDER)

    # Initialize config folders
    initialize_config_folders()
    
    # Resolve optional startup config from launcher args.
    config_file = resolve_startup_config_path(sys.argv)
    ...
    if SKIP_LOADER:
        ...
        # Wait indefinitely for CS2. Detection errors are treated as
        # "not found" rather than allowing the startup flow to terminate.
        while True:
            try:
                if is_cs2_running():
                    break
        ...
        if RESET_GRAPHICS and LAUNCHED_BY_LAUNCHER:
            perform_graphics_reset()
            time.sleep(random.randint(RESET_GRAPHICS_DELAY_MIN, RESET_GRAPHICS_DELAY_MAX))
        
        # Run cheat window initially, then allow unload to return to the loader.
        debug_log("Starting cheat window (skipping loader)...", "INFO")
        next_window = "cheat"
        while True:
            requested_window = run_window(next_window)
            if requested_window == "loader":
                debug_log("Returning to loader window...", "INFO")
                next_window = "loader"
                continue
            if requested_window == "cheat":
        ...
        while True:
            # Run the current window and honor any requested transition.
            requested_window = run_window("loader")
            if requested_window == "cheat":
                debug_log("Switching to cheat window...", "INFO")
                if RESET_GRAPHICS and LAUNCHED_BY_LAUNCHER:
                    perform_graphics_reset()
                    time.sleep(random.randint(RESET_GRAPHICS_DELAY_MIN, RESET_GRAPHICS_DELAY_MAX))
                requested_window = run_window("cheat")
                if requested_window == "loader":
                    debug_log("Returning to loader window...", "INFO")
                    continue
                if requested_window == "cheat":
                    debug_log("Cheat window requested another transition...", "INFO")
                    continue
            elif requested_window == "loader":
                debug_log("Returning to loader window...", "INFO")
                continue
            break

    # Do this after the DearPyGui loop and its window have fully shut down.
    # Calling it from inside the restart message-box branch can leave Windows
    # handles open and make shutil.rmtree silently fail.
    cleanup_temp_folder()
