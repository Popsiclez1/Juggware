...
def fov_changer_thread():
    ...
    while fov_changer_state["running"]:
        try:
            ...
            # Write the configured FOV every 1ms while enabled.
...
def on_fov_changer_toggle(sender, value):
    ...
    # Reset FOV to 90 when disabled, keep the thread running briefly, then stop it.
    if not value:
        if fov_changer_state["settings"]:
        ...
        # Keep the FOV thread alive and applying the slider value for a short grace period.
        reset_fov_to_default()
        time.sleep(3.0)
...
def create_info_tab():
    ...
    with dpg.tab(label="Info"):
        ...
        # Add warning about offsets being outdated
