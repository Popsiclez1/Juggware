...
def initialize_offset_globals():
    ...
    global m_iFOV, m_flFOVRate, m_iFOVStart, m_iDefaultFOV, m_iDefaultFOV_source, m_flFlashMaxAlpha
    ...
    try:
        ...
        # The current provider does not expose m_iDefaultFOV explicitly.
        # Use m_iDesiredFOV from CBasePlayerController as the closest available equivalent.
        m_iDefaultFOV = 0
        m_iDefaultFOV_source = None
        try:
            m_iDefaultFOV = client_dll['client.dll']['classes']['CCSPlayerBase_CameraServices']['fields']['m_iDefaultFOV']
            m_iDefaultFOV_source = 'camera_services'
        except KeyError:
            pass
        if not m_iDefaultFOV:
            try:
                m_iDefaultFOV = client_dll['client.dll']['classes']['CBasePlayerController']['fields']['m_iDesiredFOV']
                m_iDefaultFOV_source = 'controller'
            except KeyError:
                pass
        if not m_iDefaultFOV:
            missing_count += 1
            missing_offsets.append('CCSPlayerBase_CameraServices::m_iDefaultFOV|CBasePlayerController::m_iDesiredFOV')
            debug_log('Missing FOV default offset candidates: CCSPlayerBase_CameraServices::m_iDefaultFOV, CBasePlayerController::m_iDesiredFOV', 'WARNING')
...
def _get_spectator_panel_theme(settings=None):
    ...
    live_cfg = dict(Active_Config)
    if isinstance(settings, dict):
        for key, value in settings.items():
            if key not in live_cfg:
                live_cfg[key] = value

    colorway_name = live_cfg.get("menu_colorway", "Default") or "Default"
    ...
    if use_rainbow:
        ...
        return {
            ...
            "text": (255, 255, 255),
            "border": border,
            "font_name": live_cfg.get("menu_font", "Default") or "Default",
            "accent": accent,
        }
...
def _apply_fov_value(pm, client, fov_value, apply_while_scoped=False):
    ...
    try:
        ...
        if m_flFOVRate:
            pm.write_float(camera_services + m_flFOVRate, float(fov_value))

        if m_iFOVStart:
            pm.write_int(camera_services + m_iFOVStart, target_fov)

        if m_iDefaultFOV:
            if m_iDefaultFOV_source == 'camera_services':
                pm.write_int(camera_services + m_iDefaultFOV, target_fov)
            elif m_iDefaultFOV_source == 'controller' and dwLocalPlayerController:
                local_controller = pm.read_longlong(client + dwLocalPlayerController)
                if local_controller:
                    pm.write_int(local_controller + m_iDefaultFOV, target_fov)
