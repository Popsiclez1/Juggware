...
def on_unload_clicked():
    ...
    # If this process was started by the external `launcher`, try to reopen it
    # so the user returns to the loader UI instead of fully exiting.
    if LAUNCHED_BY_LAUNCHER:
        try:
            launcher_py = os.path.join(SCRIPT_DIR, "launcher.py")
            launcher_exe = os.path.join(SCRIPT_DIR, "launcher.exe")
            # Prefer a bundled exe if available, otherwise run the python script.
            if os.path.exists(launcher_exe):
                subprocess.Popen([launcher_exe], cwd=SCRIPT_DIR, creationflags=subprocess.CREATE_NEW_CONSOLE)
            elif os.path.exists(launcher_py):
                subprocess.Popen([sys.executable, launcher_py], cwd=SCRIPT_DIR, creationflags=subprocess.CREATE_NEW_CONSOLE)
            else:
                debug_log("Launcher not found to reopen", "WARNING")
        except Exception as e:
            debug_log(f"Failed to relaunch loader: {e}", "ERROR")
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="ESP"):
                ...
                with dpg.tooltip("chk_box_esp", tag="tooltip_box_esp", show=show_tips):
                    dpg.add_text("Draw player boxes for easier target tracking.")
                ...
                with dpg.tooltip("combo_box_type", tag="tooltip_box_type", show=show_tips):
                    dpg.add_text("Switch between a flat 2D box and a 3D box.")
                ...
                with dpg.tooltip("slider_box_thickness", tag="tooltip_box_thickness", show=show_tips):
                    dpg.add_text("Adjust the box outline thickness for better visibility.")
                ...
                with dpg.tooltip("combo_lines_pos", tag="tooltip_lines_pos", show=show_tips):
                    dpg.add_text("Choose where the snaplines start from.")
                ...
                with dpg.tooltip("chk_skeleton_esp", tag="tooltip_skeleton_esp", show=show_tips):
                    dpg.add_text("Show the player skeleton overlay for easier positioning.")
                ...
                with dpg.tooltip("chk_head_hitbox", tag="tooltip_head_hitbox", show=show_tips):
                    dpg.add_text("Show a marker on the head hitbox for easier aiming.")
...
def main():
    ...
    if SKIP_LOADER:
        ...
        # If Launch was clicked, run cheat window
        if should_switch == "cheat":
            debug_log("Switching to cheat window...", "INFO")
            if RESET_GRAPHICS and LAUNCHED_BY_LAUNCHER:
                perform_graphics_reset()
                time.sleep(random.randint(RESET_GRAPHICS_DELAY_MIN, RESET_GRAPHICS_DELAY_MAX))
        ...
        # If unload was requested from the cheat window, return to loader
        if should_switch == "loader":
            debug_log("Returning to loader window...", "INFO")
            run_window("loader")
