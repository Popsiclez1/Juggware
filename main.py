...
# GitHub offsets URLs
GITHUB_OFFSETS_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/offsets/offsets.json"
GITHUB_CLIENT_DLL_URL = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/offsets/client_dll.json"
    ...
    while esp_overlay["running"] and overlay.running:
        try:
            ...
            if Active_Config.get("show_status_labels", True):
                ...
                overlay.draw_text(5, 55, anti_afk_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if anti_afk_status == "ON" else (255, 0, 0)
                overlay.draw_text(anti_afk_status_x, 55, anti_afk_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
...
def aimbot_thread():
    ...
    # Wait for shared memory thread to provide pm/client
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[AIMBOT] Failed to get pm/client from shared memory", "ERROR")
        return
...
def triggerbot_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[TRIGGERBOT] Failed to get pm/client from shared memory", "ERROR")
        return
...
def anti_flash_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[ANTI FLASH] Failed to get pm/client from shared memory", "ERROR")
        return
...
def hitsound_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[HITSOUND] Failed to get pm/client from shared memory", "ERROR")
        return
...
def fov_changer_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[FOV CHANGER] Failed to get pm/client from shared memory", "ERROR")
        return
...
def acs_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[ACP] Failed to get pm/client from shared memory", "ERROR")
        return
...
def rcs_thread():
    ...
    if not esp_overlay.get("pm") or not esp_overlay.get("client"):
        debug_log("[RCS] Failed to get pm/client from shared memory", "ERROR")
        return
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Overlay"):
                ...
                with dpg.tooltip("chk_footstep_esp", tag="tooltip_footstep_esp", show=show_tips):
                    dpg.add_text("Show expanding rings when enemies move")
