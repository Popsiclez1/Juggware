...
def apply_config_to_ui():
    ...
    try:
        ...
        # Keep FOV slider always visible and update enabled state
        if dpg.does_item_exist("slider_fov_value"):
            fov_enabled = Active_Config.get("fov_changer_enabled", False)
            dpg.configure_item("slider_fov_value", show=True, enabled=fov_enabled)
...
def compute_humanized_aimbot_movement(dx, dy, aimbot_state):
    """Advanced humanized movement that prioritizes reliable target lock with highly realistic motion including reaction delay and overshoot."""
    ...
    human_state.setdefault("target_locked_frames", 0)
    human_state.setdefault("reaction_delay_frames", 0)
    human_state.setdefault("overshoot_x", 0.0)
    human_state.setdefault("overshoot_y", 0.0)
    ...
    if dist <= 0.001:
        human_state["velocity_x"] *= 0.45
        human_state["velocity_y"] *= 0.45
        human_state["drift_x"] *= 0.60
        human_state["drift_y"] *= 0.60
        human_state["overshoot_x"] *= 0.50
        human_state["overshoot_y"] *= 0.50
        human_state["stabilize_frames"] = max(0, human_state.get("stabilize_frames", 0) - 1)
        human_state["target_locked_frames"] = 0
        return 0.0, 0.0

    human_state["frame_counter"] += 1

    # Track locked/on-target frames for progressive precision matching Fitts's Law
    if dist < 12.0:
        human_state["target_locked_frames"] += 1
    else:
        human_state["target_locked_frames"] = max(0, human_state["target_locked_frames"] - 2)

    # Biological Reaction Latency upon sudden direction changes
    prev_dx = human_state.get("last_dx", dx)
    prev_dy = human_state.get("last_dy", dy)
    prev_dist = math.hypot(prev_dx, prev_dy)
    if prev_dist > 5.0 and dist > 5.0:
        dot_product = (dx * prev_dx + dy * prev_dy) / (dist * prev_dist)
        if dot_product < 0.5:  # Direction shifted by more than 60 degrees
            delay_chance = 0.55 * human_strength
            if random.random() < delay_chance:
                human_state["reaction_delay_frames"] = max(human_state["reaction_delay_frames"], random.randint(2, 5))

    reaction_delay = human_state["reaction_delay_frames"]
    if reaction_delay > 0:
        human_state["reaction_delay_frames"] -= 1
        human_state["velocity_x"] *= (0.65 - 0.25 * human_strength)
        human_state["velocity_y"] *= (0.65 - 0.25 * human_strength)
        return human_state["velocity_x"], human_state["velocity_y"]

    # Target motion estimation & prediction
    raw_target_vel_x = dx - prev_dx
    raw_target_vel_y = dy - prev_dy
    target_vel_alpha = 0.25 + 0.15 * (1.0 - human_strength)
    human_state["target_vel_x"] += (raw_target_vel_x - human_state["target_vel_x"]) * target_vel_alpha
    human_state["target_vel_y"] += (raw_target_vel_y - human_state["target_vel_y"]) * target_vel_alpha

    # Apply adaptive prediction lead (dampened prediction for high humanization)
    lead_frames = 0.05 + 0.20 * (1.0 - human_strength)
    lead_dx = dx + human_state["target_vel_x"] * lead_frames
    lead_dy = dy + human_state["target_vel_y"] * lead_frames
    lead_dist = max(0.001, math.hypot(lead_dx, lead_dy))
    nx = lead_dx / lead_dist
    ny = lead_dy / lead_dist

    # Dynamic Overshoot and Correction Patterns
    if dist > 85.0 and random.random() < (0.42 * human_strength) and math.hypot(human_state["overshoot_x"], human_state["overshoot_y"]) < 1e-3:
        overshoot_mag = dist * (0.04 + 0.06 * human_strength)
        human_state["overshoot_x"] = nx * overshoot_mag
        human_state["overshoot_y"] = ny * overshoot_mag

    # Decay overshoot and apply correctives
    overshoot_x = human_state["overshoot_x"]
    overshoot_y = human_state["overshoot_y"]
    human_state["overshoot_x"] *= (0.82 - 0.12 * human_strength)
    human_state["overshoot_y"] *= (0.82 - 0.12 * human_strength)

    # Offset lead coordinates by overshoot delta
    adj_lead_dx = lead_dx + overshoot_x
    adj_lead_dy = lead_dy + overshoot_y
    adj_lead_dist = max(0.001, math.hypot(adj_lead_dx, adj_lead_dy))
    nx = adj_lead_dx / adj_lead_dist
    ny = adj_lead_dy / adj_lead_dist

    # Adaptive speed curve with Fitts's Law Target Friction
    min_speed = 0.85 + (1.15 * (1.0 - human_strength))
    max_speed = 10.8 - (2.4 * human_strength)
    distance_gain = 0.060 + 0.042 * (1.0 - human_strength)
    desired_speed = min(max_speed, min_speed + adj_lead_dist * distance_gain)

    # Sticky target deceleration friction
    if adj_lead_dist < 28.0:
        friction_factor = 0.50 + 0.35 * (1.0 - human_strength)
        if human_state["target_locked_frames"] > 5:
            friction_factor *= (0.75 - 0.15 * human_strength)
        desired_speed *= friction_factor

    desired_vx = nx * desired_speed
    desired_vy = ny * desired_speed

    # Inertial acceleration and muscle tension filtering
    velocity_x = human_state.get("velocity_x", 0.0)
    velocity_y = human_state.get("velocity_y", 0.0)

    accel_base = 0.38 - 0.18 * human_strength
    if adj_lead_dist < 15.0:
        accel_base *= (0.80 - 0.20 * human_strength)

    velocity_x += (desired_vx - velocity_x) * accel_base
    velocity_y += (desired_vy - velocity_y) * accel_base

    braking = 0.05 + 0.12 * human_strength
    velocity_x *= (1.0 - braking)
    velocity_y *= (1.0 - braking)

    # Biomechanic drift (perpendicular path curvature)
    perp_x = -ny
    perp_y = nx
    drift_pull = 0.25 + 0.25 * human_strength
    drift_noise = (0.08 + 0.22 * human_strength) * min(1.2, adj_lead_dist / 85.0)
    drift_x = human_state.get("drift_x", 0.0)
    drift_y = human_state.get("drift_y", 0.0)
    drift_x += (-drift_x * drift_pull) + random.uniform(-1.0, 1.0) * drift_noise * perp_x
    drift_y += (-drift_y * drift_pull) * random.uniform(-1.0, 1.0) * drift_noise * perp_y

    # Biological muscular tremor (micro-shivers)
    human_state["micro_phase"] += 0.65 + random.random() * 0.30
    human_state["coarse_phase"] += 0.22 + random.random() * 0.12
    tremor_wave = (math.sin(human_state["micro_phase"]) * 0.70) + (math.sin(human_state["coarse_phase"]) * 0.30)

    tremor_amplitude = (0.04 + 0.22 * human_strength) * min(1.0, adj_lead_dist / 75.0)
    if adj_lead_dist < 12.0:
        tremor_amplitude *= 0.25
    tremor_x = perp_x * tremor_wave * tremor_amplitude
    tremor_y = perp_y * tremor_wave * tremor_amplitude

    move_x = velocity_x + drift_x + tremor_x
    move_y = velocity_y + drift_y + tremor_y

    if dist < 12.0:
        settle_scale = 0.42 + 0.18 * (1.0 - human_strength)
        move_x *= settle_scale
        move_y *= settle_scale
        human_state["stabilize_frames"] = 3
    elif human_state.get("stabilize_frames", 0) > 0:
        move_x *= 0.68
        move_y *= 0.68
        human_state["stabilize_frames"] -= 1

    # Anti-stall logic on medium/large inputs
    if dist > 25.0:
        min_axis_step = 0.55 + 0.70 * (1.0 - human_strength)
        if abs(move_x) < min_axis_step and abs(dx) > min_axis_step:
            move_x = math.copysign(min_axis_step, dx)
        if abs(move_y) < min_axis_step and abs(dy) > min_axis_step:
            move_y = math.copysign(min_axis_step, dy)
    ...
    human_state["drift_x"] = drift_x
    human_state["drift_y"] = drift_y
...
def _apply_fov_value(pm, client, fov_value):
...
def fov_changer_thread():
    ...
    while fov_changer_state["running"]:
        try:
            ...
            fov_value = 90 if not settings.get("fov_changer_enabled", False) else int(settings.get("fov_value", 90))
            _apply_fov_value(pm, client, fov_value)
            time.sleep(0.001)
...
def on_fov_changer_toggle(sender, value):
    ...
    # Keep the slider visible at all times and enable it only when FOV changer is on
    if dpg.does_item_exist("slider_fov_value"):
        dpg.configure_item("slider_fov_value", show=True, enabled=value)
    if dpg.does_item_exist("tooltip_fov_value"):
        dpg.configure_item("tooltip_fov_value", show=True)

    if not value:
        if fov_changer_state["settings"]:
            fov_changer_state["settings"]["fov_value"] = 90
        Active_Config["fov_value"] = 90
        save_settings()
        if dpg.does_item_exist("slider_fov_value"):
            dpg.set_value("slider_fov_value", 90)
    else:
...
def cleanup_temp_folder():
    ...
    # Reset FOV to 90 before stopping the FOV changer thread
    if Active_Config.get("fov_changer_enabled", False):
        Active_Config["fov_value"] = 90
        if fov_changer_state.get("settings"):
            fov_changer_state["settings"]["fov_value"] = 90
