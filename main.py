...
def initialize_offset_globals():
    ...
    global m_iFOV, m_flFOVRate, m_iFOVStart, m_iDesiredFOV , m_flFlashMaxAlpha
...
def should_hide_spotted_player(settings, is_spotted):
    """Return True when spotted-player hiding is enabled and the entity is spotted."""
...
def render_esp_frame(overlay, pm, client, settings):
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state only when hide-spotted-players is active.
            is_spotted = False
            if hide_spotted_players:
                is_spotted = read_spotted_state(
                    pm,
                    entity_pawn_addr,
...
def render_snaplines_frame(overlay, pm, client, settings):
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state only when hide-spotted is active.
            if hide_spotted_players:
                is_spotted = read_spotted_state(
                    pm,
                    entity_pawn_addr,
    ...
    while esp_overlay["running"] and overlay.running:
        try:
            ...
            if Active_Config.get("show_status_labels", True):
                ...
                # ACS status
                acs_status = "ON" if Active_Config.get("acs_enabled", False) else "OFF"
                acs_label = "ACS: "
                acs_status_x = 5 + len(acs_label) * 7 + 8
                overlay.draw_text(5, 70, acs_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if acs_status == "ON" else (255, 0, 0)
                overlay.draw_text(acs_status_x, 70, acs_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
                
                # Bhop status
                bhop_status = "ON" if Active_Config.get("bhop_enabled", False) else "OFF"
                bhop_label = "Bhop: "
                bhop_status_x = 5 + len(bhop_label) * 7 + 8
                overlay.draw_text(5, 85, bhop_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if bhop_status == "ON" else (255, 0, 0)
                overlay.draw_text(bhop_status_x, 85, bhop_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
                ...
                overlay.draw_text(5, 115, triggerbot_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if triggerbot_status == "ON" else (255, 0, 0)
                overlay.draw_text(triggerbot_status_x, 115, triggerbot_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
                ...
                overlay.draw_text(5, 130, anti_flash_label, 255, 255, 255, size=12.0, stroke=True)
                status_color = (0, 255, 0) if anti_flash_status == "ON" else (255, 0, 0)
                overlay.draw_text(anti_flash_status_x, 130, anti_flash_status, status_color[0], status_color[1], status_color[2], size=12.0, stroke=True)
...
def on_test_clicked():
    ...
    # Wait for CS2 while keeping the loader responsive/draggable.
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="ESP"):
                ...
                with dpg.tooltip("chk_hide_spotted_players", tag="tooltip_hide_spotted_players", show=show_tips):
                    dpg.add_text("Hide ESP details for players that are visible to opposite team")
...
def create_settings_tab_cheat():
    ...
    with dpg.tab(label="Miscellaneous"):
        ...
        dpg.add_text("Automatic feature protections when spectators are present.")
        dpg.add_checkbox(
            label="Hide ESP",
