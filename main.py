...
aimbot_state = {
    ...
    "humanization": {           # Stateful human-like movement profile for aimbot
        "velocity_x": 0.0,
        "velocity_y": 0.0,
        "frame_counter": 0,
        "cooldown": 0,
...
def render_esp_frame(overlay, pm, client, settings):
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state for spotted ESP feature and hide-spotted behavior
            ...
            if should_hide_spotted_player(settings, is_spotted):
                continue
            ...
            # OPTIMIZATION: Cache game_scene read to eliminate redundancy
            # This is read once here and reused throughout entity rendering
            ...
            if settings.get('draw_head_hitbox', False):
                ...
                if world_hitbox:
                    ...
                    thickness = settings.get('head_hitbox_thickness', 2.0)
                    draw_projected_sphere(overlay, rings, head_hitbox_color, thickness)
...
def render_snaplines_frame(overlay, pm, client, settings):
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state for hide-spotted behavior
            is_spotted = read_spotted_state(
                pm,
                entity_pawn_addr,
                m_entitySpottedState,
...
def draw_skeleton(overlay, pm, bone_matrix, view_matrix, width, height, skeleton_color=(255, 255, 255), thickness=1.5):
    ...
    try:
        bones_to_read = {bone_name for bone_pair in BONE_CONNECTIONS for bone_name in bone_pair}
        bone_ids = {name: BONE_IDS[name] for name in bones_to_read if name in BONE_IDS}
        if not bone_ids:
            return
        
        min_id = min(bone_ids.values())
        max_id = max(bone_ids.values())
        block_size = (max_id - min_id + 1) * 0x20
        bone_block = pm.read_bytes(bone_matrix + min_id * 0x20, block_size)
        
        for bone_name, bone_id in bone_ids.items():
            offset = (bone_id - min_id) * 0x20
...
def compute_humanized_aimbot_movement(dx, dy, aimbot_state):
    """Generate fast, human-like mouse movement while ignoring standard smoothing settings."""
    human_state = aimbot_state.setdefault("humanization", {
        "velocity_x": 0.0,
        "velocity_y": 0.0,
        "frame_counter": 0,
        "cooldown": 0,
    ...
    dist = math.sqrt(dx * dx + dy * dy)
    if dist <= 0:
        human_state["velocity_x"] = 0.0
        human_state["velocity_y"] = 0.0
        human_state["cooldown"] = max(0, human_state.get("cooldown", 0) - 1)
        human_state["reaction_timer"] = max(0, human_state.get("reaction_timer", 0) - 1)
        human_state["settle_timer"] = max(0, human_state.get("settle_timer", 0) - 1)
        return 0.0, 0.0

    human_state["frame_counter"] = human_state.get("frame_counter", 0) + 1
    frame_counter = human_state["frame_counter"]
    nx = dx / dist
    ny = dy / dist

    prev_dx = human_state.get("last_dx", dx)
    prev_dy = human_state.get("last_dy", dy)
    prev_mag = math.sqrt(prev_dx * prev_dx + prev_dy * prev_dy)
    if prev_mag > 0:
        dot = (dx * prev_dx + dy * prev_dy) / (dist * prev_mag)
        direction_change = max(0.0, 1.0 - dot)
    else:
        direction_change = 0.0

    # Human reaction: sudden direction changes introduce a slight delay and a tiny hesitation.
    if direction_change > 0.25:
        human_state["reaction_timer"] = max(human_state.get("reaction_timer", 0), random.randint(1, 2))
    elif frame_counter % 11 == 0 and random.random() < 0.16:
        human_state["reaction_timer"] = max(human_state.get("reaction_timer", 0), 1)

    reaction_timer = max(0, human_state.get("reaction_timer", 0) - 1)
    human_state["reaction_timer"] = reaction_timer

    # Humanization strength scales pauses, drift, jitter, and settling.
    strength = max(0.0, min(100.0, settings.get("aimbot_humanization_strength", 75.0))) if (settings := aimbot_state.get("settings")) else 75.0
    human_strength = (strength / 100.0) ** 2.1

    # Build a slightly nonlinear speed curve so far targets are handled faster, close targets softer.
    base_speed = 4.2 + min(6.5, dist / 24.0)
    if dist < 35:
        base_speed *= 0.62 + 0.38 * (1.0 - human_strength)
    elif dist > 140:
        # Slow down the far-distance flick more strongly when humanization is high.
        base_speed *= 0.90 + 0.10 * human_strength

    cooldown = human_state.get("cooldown", 0)
    if cooldown > 0:
        human_state["cooldown"] = cooldown - 1
    elif frame_counter % 22 == 0 and random.random() < 0.16 + 0.28 * human_strength:
        human_state["cooldown"] = random.randint(1, 4 if human_strength > 0.5 else 3)
        cooldown = human_state["cooldown"]

    if reaction_timer > 0:
        speed_scale = 0.44 + random.random() * 0.24
    else:
        speed_scale = 0.86 - 0.22 * human_strength + random.random() * (0.14 + 0.18 * (1.0 - human_strength))

    if frame_counter % 7 == 0 and random.random() < 0.18 + 0.38 * human_strength:
        speed_scale *= 1.10 + 0.06 * human_strength

    if dist < 24:
        speed_scale *= 0.72 + 0.28 * (1.0 - human_strength)

    if cooldown > 0:
        speed_scale *= 0.10 + 0.10 * (1.0 - human_strength)

    desired_vx = nx * base_speed * speed_scale
    desired_vy = ny * base_speed * speed_scale

    velocity_x = human_state.get("velocity_x", 0.0)
    velocity_y = human_state.get("velocity_y", 0.0)
    accel = 0.16 + min(0.18, dist / 700.0)
    velocity_x += (desired_vx - velocity_x) * accel
    velocity_y += (desired_vy - velocity_y) * accel

    # Add motion that feels like real tracking: a slight drift, tiny correction, and a wobble.
    phase = human_state.get("phase", 0.0) + 0.75 + random.random() * 0.15
    human_state["phase"] = phase
    wave = math.sin(phase + frame_counter * 0.22) * (0.12 + random.random() * 0.06) * (0.30 + 0.70 * human_strength)
    if dist < 45:
        wave *= 0.50 + 0.30 * (1.0 - human_strength)
    if reaction_timer > 0:
        wave *= 0.36 + 0.32 * (1.0 - human_strength)
    if cooldown > 0:
        wave *= 0.18 + 0.06 * (1.0 - human_strength)

    perpendicular_x = -ny
    perpendicular_y = nx
    jitter_strength = 0.10 + random.random() * 0.24 * human_strength
    if dist > 140:
        jitter_strength *= 1.12
    if dist < 45:
        jitter_strength *= 0.54 + 0.20 * (1.0 - human_strength)
    if reaction_timer > 0 or cooldown > 0:
        jitter_strength *= 0.50 + 0.20 * (1.0 - human_strength)

    # Add a more noticeable natural drift along the path with a subtle perpendicular bend.
    drift_factor = (0.08 + random.random() * 0.10) * human_strength
    drift_x = perpendicular_x * drift_factor * (dist / 80.0)
    drift_y = perpendicular_y * drift_factor * (dist / 80.0)

    jitter_x = perpendicular_x * random.uniform(-jitter_strength, jitter_strength)
    jitter_y = perpendicular_y * random.uniform(-jitter_strength, jitter_strength)

    correction_bias = 0.0
    if frame_counter % 5 == 0 and random.random() < 0.32 + 0.40 * human_strength:
        correction_bias = random.uniform(-0.55, 0.55) * human_strength

    move_x = velocity_x + jitter_x + wave * nx + correction_bias * ny + drift_x
    move_y = velocity_y + jitter_y + wave * ny - correction_bias * nx + drift_y

    # Near the target, settle instead of over-correcting.
    if dist < 16:
        move_x *= 0.48
        move_y *= 0.48
        human_state["settle_timer"] = 2
    elif human_state.get("settle_timer", 0) > 0:
        move_x *= 0.75
        move_y *= 0.75
        human_state["settle_timer"] -= 1
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Aimbot"):
                ...
                with dpg.tooltip("chk_aimbot_use_humanization", tag="tooltip_aimbot_use_humanization", show=show_tips):
                    dpg.add_text("Make aimbot look human")    
...
def cleanup_current_window():
    """Hide and destroy any active DearPyGui viewport window before switching windows."""
    hwnd_candidates = []
...
def run_window(window_type="loader"):
    ...
    """
    # Destroy any stale viewport windows from previous runs before creating a new one.
    cleanup_current_window()
