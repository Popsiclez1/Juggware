...
Default_Config = {
    ...
    "name_esp": True,
    "weapon_esp": False,                 # Show weapon name label above players
    "distance_esp": False,
    "distance_text_size": 12.0,          # Font size for distance text labels
    ...
    "weapon_text_size": 12.0,               # Weapon label text font size
    # FPS Cap
    ...
    "rcs_horizontal_smoothing": 0.0,       # Horizontal smoothing (0 = full compensation, 10 = barely any)
    "rcs_vertical_smoothing": 0.0,         # Vertical smoothing (0 = full compensation, 10 = barely any)
...
def apply_config_to_ui():
    ...
    try:
        ...
        # Custom crosshair option visibility depends on custom_crosshair_enabled
        if dpg.does_item_exist("slider_custom_crosshair_scale"):
            ...
            for option in crosshair_options:
                if dpg.does_item_exist(option):
                    dpg.configure_item(option, show=custom_crosshair_enabled)
            ...
            for option in tooltip_options:
                if dpg.does_item_exist(option):
                    dpg.configure_item(option, show=custom_crosshair_enabled and show_tips)
...
def bhop_loop():
    ...
    debug_log("Bhop: Shared memory connected, starting bhop loop", "SUCCESS")
...
def initialize_offset_globals():
    ...
    try:
        ...
        if dwForceJump == 0:
            ...
            try:
                url = "https://raw.githubusercontent.com/Popsiclez1/Juggware/refs/heads/main/offsets/buttons.hpp"
                response = requests.get(url)
                if response.status_code == 200:
                    match = re.search(r'constexpr\s+std::ptrdiff_t\s+jump\s*=\s*(0x[0-9A-Fa-f]+);', response.text)
                    if match:
                        dwForceJump = int(match.group(1), 16)
                        debug_log(f"dwForceJump loaded from online fallback: 0x{dwForceJump:X}", "SUCCESS")
            except Exception as e:
                debug_log(f"Online dwForceJump fallback failed: {e}", "ERROR")
...
def render_esp_frame(overlay, pm, client, settings):
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state for spotted ESP feature
            is_spotted = False
            ...
            if head_pos[0] == -999 or leg_pos[0] == -999:
                if settings.get('line_esp', True) and head_snap_pos[0] != -999:
            ...
            if settings.get('box_esp', True):
                ...
                if box_type == '3D':
                    ...
                    try:
                        # OPTIMIZATION: Reuse cached game_scene instead of re-reading
            ...
            if settings.get('weapon_esp', False):
                try:
                    ...
                    if player_weapon_name and player_weapon_name != "Unknown":
                        font_size = settings.get('weapon_text_size', 12.0)
                        selected_font = settings.get('menu_font', 'Default')
            ...
            if settings.get('distance_esp', True):
                ...
                text_size = settings.get('distance_text_size', 12.0)
                selected_font = settings.get('menu_font', 'Default')
...
def draw_skeleton(overlay, pm, bone_matrix, view_matrix, width, height, skeleton_color=(255, 255, 255), thickness=1.5):
    ...
    OPTIMIZED: Only reads bones that are actually connected (not all 16 bones).
    
    Process:
    1. Read only bones used in BONE_CONNECTIONS
    2. Convert 3D bone positions to 2D screen coordinates
    3. Draw lines between connected bones
    ...
    try:
        # OPTIMIZATION: Only read bones used in connections, not all BONE_IDS
        bones_to_read = set()
        for bone1_name, bone2_name in BONE_CONNECTIONS:
            bones_to_read.add(bone1_name)
            bones_to_read.add(bone2_name)
        
        # Read only necessary bones - batch read with 12 bytes per bone (X, Y, Z as floats)
        for bone_name in bones_to_read:
            try:
        ...
        # Draw bone connections - early exit if either endpoint invalid
...
def render_aimbot_snaplines(overlay, pm, client, settings):
    ...
    try:
        ...
        bone_map = {'Head': 7, 'Neck': 6, 'Chest': 4}
        ...
        try:
            ...
            for i in range(1, 64):
                try:
                    ...
                    # Apply head level adjustment (add a few Z units if targeting head)
                    if target_bone_name == "Head":
                        bone_z += 2.0
...
def start_esp_overlay():
    ...
    if not GLFW_AVAILABLE:
        error_msg = (
            "ESP overlay failed to initialize: GLFW runtime is missing. "
    ...
    while triggerbot_state["running"]:
        try:
            ...
            if previous_entity_id != entity_id and previous_entity_id != -1:
                # New target gained - reset first shot pending
                first_shot_pending = True
            ...
            try:
                ...
                for i in range(1, 64):
                    try:
                        ...
                        # Get target bone ID based on setting
                        target_bone_name = settings.get("acs_target_bone", "Head")
                        bone_map = {"Head": 6, "Neck": 5, "Chest": 4}
...
def rcs_thread():
    ...
    while rcs_state["running"]:
        try:
            ...
            # Apply per-axis smoothing (0 = full compensation, 10 = minimal compensation).
            # Map 0..10 to a divisor of 1..20.
            ...
            horizontal_divisor = 1.0 + (horizontal_smoothing * 1.9)
            vertical_divisor = 1.0 + (vertical_smoothing * 1.9)
            move_x /= horizontal_divisor
            move_y /= vertical_divisor
...
def create_settings_tab():
    ...
    with dpg.tab(label="Loader"):
        ...
        with dpg.tooltip("chk_use_local_offsets", tag="tooltip_use_local_offsets", show=show_tips):
            ...
            dpg.add_text("-Allows cheat to function without updating")
    ...
    with dpg.tab(label="Overlay"):
        with dpg.tab_bar():
            ...
            with dpg.tab(label="ESP"):
                ...
                dpg.add_text("Global ESP Toggle")
                dpg.add_separator()
                ...
                with dpg.tooltip("chk_esp_enabled", tag="tooltip_esp_enabled", show=show_tips):
                    dpg.add_text("Global toggle for all ESP features")
                ...
                with dpg.tooltip("combo_esp_targeting_type", tag="tooltip_esp_targeting_type", show=show_tips):
                    dpg.add_text("ESP and radar target filtering")
                ...
                with dpg.tooltip("chk_weapon_esp", tag="tooltip_weapon_esp", show=show_tips):
                    dpg.add_text("Display detected weapon name above boxes")
                ...
                with dpg.tooltip("slider_weapon_text_size", tag="tooltip_weapon_text_size", show=show_tips):
                    dpg.add_text("Font size for player weapon text")
                ...
                with dpg.tooltip("slider_distance_text_size", tag="tooltip_distance_text_size", show=show_tips):
                    dpg.add_text("Font size for distance text labels")
                ...
                with dpg.tooltip("chk_head_hitbox", tag="tooltip_head_hitbox", show=show_tips):
                    dpg.add_text("Draw sphere indicating head hitbox")
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Triggerbot"):
                ...
                with dpg.tooltip("slider_triggerbot_first_shot_delay", tag="tooltip_triggerbot_first_shot_delay", show=show_tips):
                    dpg.add_text("Delay before first shot fires (applies to all presets)")
            ...
            with dpg.tab(label="Recoil Control"):
                ...
                with dpg.tooltip("chk_rcs_enabled", tag="tooltip_rcs_enabled", show=show_tips):
                    dpg.add_text("Compensate weapon recoil using real aim punch angles from the game engine")
                ...
                with dpg.tooltip("slider_rcs_horizontal_smoothing", tag="tooltip_rcs_horizontal_smoothing", show=show_tips):
                    dpg.add_text("Horizontal axis: 0 = full compensation, 10 = barely any")
                ...
                with dpg.tooltip("slider_rcs_vertical_smoothing", tag="tooltip_rcs_vertical_smoothing", show=show_tips):
                    dpg.add_text("Vertical axis: 0 = full compensation, 10 = barely any")
...
def update_performance_display():
    ...
    try:
        ...
        esp_keybind = (Keybinds_Config.get("esp_toggle_key", "") or "").upper()
        esp_status_text = "NONE" if not esp_keybind or esp_keybind == "NONE" else ("Running" if esp_running else "Stopped")
        esp_color = (180, 180, 180) if esp_status_text == "NONE" else ((100, 255, 100) if esp_running else (255, 100, 100))
        ...
        aimbot_keybind = (Keybinds_Config.get("aimbot_toggle_key", "") or "").upper()
        aimbot_status_text = "NONE" if not aimbot_keybind or aimbot_keybind == "NONE" else ("Running" if aimbot_running else "Stopped")
        aimbot_color = (180, 180, 180) if aimbot_status_text == "NONE" else ((100, 255, 100) if aimbot_running else (255, 100, 100))
