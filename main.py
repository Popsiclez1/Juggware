...
def on_unload_clicked():
    ...
    # Hide DearPyGui viewport first, then the Win32 window handle as a fallback.
    try:
        # Hide the viewport so the window disappears immediately
        try:
            dpg.hide_viewport()
        except Exception:
            pass

        # Also try to mark the viewport invisible via configure_viewport
        try:
            dpg.configure_viewport(0, visible=False)
        except Exception:
            pass

        # Hide the cheat HWND if available
        try:
            if drag_state.get("hwnd"):
                win32gui.ShowWindow(drag_state["hwnd"], win32con.SW_HIDE)
        except Exception:
        ...
        # Ensure we attempt to stop DearPyGui even if hiding failed
        try:
            dpg.stop_dearpygui()
        except Exception:
            pass
...
def run_window(window_type="loader"):
    ...
    # Cleanup
    dpg.destroy_context()
    # Ensure cheat window is hidden when switching back to the loader
    if window_type == "cheat" and app_state.get("requested_window") == "loader":
        try:
            if drag_state.get("hwnd"):
                win32gui.ShowWindow(drag_state["hwnd"], win32con.SW_HIDE)
        except Exception:
...
def main():
    ...
    if SKIP_LOADER:
        ...
        # Run loader/cheat windows and handle transitions until user exits.
        next_window = "loader"
        while True:
            requested_window = run_window(next_window)
            if requested_window == "cheat":
            ...
            if requested_window == "loader":
                ...
                next_window = "loader"
                continue
            # No transition requested -> exit application
            break
