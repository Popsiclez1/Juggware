...
Default_Config = {
    ...
    # Spotted ESP
    "spotted_esp": False,                    # Show spotted/visibility indicator
    ...
    "spotted_text_size": 14.0,              # Spotted text font size
...
def wait_for_feature_memory_connection(feature_name, max_wait_seconds=10):
    """Wait for shared memory to become available for a feature thread."""
    pm, client = wait_for_shared_memory_connection(max_wait_seconds=max_wait_seconds)
    if pm and client:
        return pm, client

    debug_log(f"[{feature_name}] Failed to get pm/client from shared memory", "ERROR")
    return None, None
...
def apply_config_to_ui():
    ...
    try:
        ...
        if dpg.does_item_exist("combo_acs_target_bone"):
            bone = normalize_target_bone_value(Active_Config.get("acs_target_bone", "Head"))
            Active_Config["acs_target_bone"] = bone
...
def initialize_offset_globals():
    ...
    global m_hPlayerPawn, m_iszPlayerName
...
def render_esp_frame(overlay, pm, client, settings):
    ...
    spotted_text_size = settings.get('spotted_text_size', 12.0)
    ...
    for i in range(1, 64):
        try:
            ...
            # Check spotted state only when a spotted-dependent feature is active.
            is_spotted = False
            if spotted_enabled or hide_spotted_players:
                ...
                if hide_spotted_players and is_spotted:
                    continue
            ...
            if settings.get('weapon_esp', False):
                try:
                    ...
                    if player_weapon_name and player_weapon_name != "Unknown":
                        ...
                        overlay.draw_text(text_x, text_y_offset - font_size, player_weapon_name, 255, 255, 255, size=font_size, stroke=True, font_name=selected_font)
        ...
        try:
            ...
            for i in range(1, 64):
                try:
                    ...
                    # Check team based on ACP targeting mode
                    entity_team = pm.read_int(entity_pawn + m_iTeamNum)
                    if entity_team == local_team and targeting_type == 0:
                        continue
...
def should_run_esp_overlay(config=None):
    ...
    return any([
        ...
        acs_enabled and cfg.get("acs_draw_deadzone_lines", False),
    ])
...
def acs_thread():
    ...
    pm, client = wait_for_feature_memory_connection("ACP", max_wait_seconds=10)
    if not pm or not client:
        return
    
    debug_log("[ACP] Connected to CS2 via shared memory", "INFO")
            ...
            try:
                ...
                for i in range(1, 64):
                    try:
                        ...
                        screen_pos = apply_head_target_bone_offset(
                            target_bone_name,
                            w2s_aimbot(view_matrix, bone_x, bone_y, bone_z, screen_width, screen_height)
                        )
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="ESP"):
                ...
                with dpg.tooltip("combo_box_type", tag="tooltip_box_type", show=show_tips):
                    dpg.add_text("Box dimension type")
                ...
                dpg.add_spacer(height=UI_SPACING_MEDIUM)
                dpg.add_text("Spotted Status Settings")
                dpg.add_separator()

                dpg.add_checkbox(
                    label="Render Spotted Status", 
                ...
                with dpg.tooltip("chk_spotted_esp", tag="tooltip_spotted_esp", show=show_tips):
                    dpg.add_text("Show whether a player is spotted by your team.")
                ALL_TOOLTIP_TAGS.append("tooltip_spotted_esp")

                dpg.add_slider_float(
                    label="Spotted Status Size",
                ...
                with dpg.tooltip("slider_spotted_text_size", tag="tooltip_spotted_text_size", show=show_tips):
                    dpg.add_text("Adjust the spotted-status label size.")
    ...
    with dpg.tab(label="Aim"):
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Auto Crosshair Placement"):
                ...
                dpg.add_combo(
                    items=["Enemies Only", "All Players"],
                    default_value="All Players" if Active_Config.get("acs_targeting_type", 0) == 1 else "Enemies Only",
                    label="Targeting",
                    tag="combo_acs_targeting_type",
                    callback=on_acs_targeting_type_change
                )
                with dpg.tooltip("combo_acs_targeting_type", tag="tooltip_acs_targeting_type", show=show_tips):
                    dpg.add_text("ACP target filtering")
                ALL_TOOLTIP_TAGS.append("tooltip_acs_targeting_type")
                ...
                # Always show deadzone lines toggle
                dpg.add_checkbox(
                ...
                with dpg.tooltip("chk_acs_always_show_deadzone_lines", tag="tooltip_acs_always_show_deadzone_lines", show=show_tips):
